...
// $Id: ViewRss.class.php,v 1.18 2006/01/04 22:32:48 matthieu_ Exp $
...
class ViewRss extends ViewModule
{
	...
	{	
		...
		if ((!$allData = $Cache_Lite->get($uniqCacheId)) || SMARTY_DEBUG) 
		{
			...
			foreach($allSiteArchive as $id => $infoSite)
			{
				...
				foreach($allArchives as $date => $o_archive)
				{
				    ...
				    $item['guid'] = $url;//"http://www.phpmyvisites.net/".md5($dateRss);
					$item['link'] = $url;
