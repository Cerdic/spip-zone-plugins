...
// $Id: ArchivePeriod.class.php,v 1.13 2006/01/09 11:46:52 matthieu_ Exp $
...
class ArchivePeriod extends Archive
{
	...
	{			
		...
		*/
		...
		$this->deleteOldRecords($inIdArchives);
	}
	// TODO protéger le champ pmv_sum des mots clés	
	
	function deleteOldRecords($inIdArchives)
	{
		parent::deleteOldRecords( false );
		
		// delete records in VISIT that are useless (mean when week AND month using these records are done)
		if($this->periodType === DB_ARCHIVES_PERIOD_MONTH
		&& $this->state !== DB_ARCHIVES_TEMP)
		{
			// select all weeks and months begin and end
			$r = query("SELECT date1, date2, idsite, period
						FROM ".T_ARCHIVES."
			...
			while($l = mysql_fetch_assoc($r))
			{
				...
				//print( $l['date1'].",". $l['date2']." : ");
				//print_r(sizeof($dateBetween)); print("<br>");
					...
					if($hits > 1)
					{
						$r = query("DELETE
									...
									WHERE server_date = '".$date."'
										AND idsite = $idsite
										");
						//print("deleted ".mysql_affected_rows($r));
					}
				}
			}
