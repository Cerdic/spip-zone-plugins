...
// $Id: Request.class.php,v 1.61 2006/01/13 01:24:49 matthieu_ Exp $
...
class Request
{
	...
	{
		return isset($GLOBALS['currentModuleIsLogModule']); // quick hack...
		return ereg( '/phpmyvisites.php', Request::getCurrentCompleteUrl() );
	}
	...
	function getArraySort($name, $totalPopulation)
	{
		...
		$sorting_limit = isset($sort[3])?$sort[3]:$totalPopulation;
		
		//print("$name : total= $totalPopulation <br>");
		if(!isset($sort[4]))
		{
			$sorting_percent_limit = $this->sorting_percent_limit_and_population_index[$name][0]; 
		}
		else
		{
	...
	function getCurrentCompletePath()
	{
		...
		$phpmv_url_chemin= Request::getHostUrl() . $phpmv_url_chemin;
	...
	{
		//print(Request::getCurrentCompletePath());
		//print("<br>");
	...
	function getCurrentUrl()
	{
		...
		{
			$toUse = $_SERVER["SCRIPT_URL"];
		}
		else
		{
			$toUse = $_SERVER["SCRIPT_NAME"];
		}
