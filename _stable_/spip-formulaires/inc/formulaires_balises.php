<?php
	...
	function calculer_REPONSE($id_application, $id_question) {
		if (intval($id_application) == 0)
			return '';
		list($valeur) = spip_fetch_array(spip_query('SELECT valeur FROM spip_reponses WHERE id_application="'.$id_application.'" AND id_question="'.$id_question.'"'), SPIP_NUM);
		return $valeur;
	}
	...
	function calculer_REPONSES($id_application, $id_question) {
		...
		if (intval($id_application) == 0)
			return $valeurs;
		$res = spip_query('SELECT valeur FROM spip_reponses WHERE id_application="'.$id_application.'" AND id_question="'.$id_question.'"');
	...
	function calculer_ABONNEMENTS($id_application) {
		$valeurs = array();
		$res_email_applicant = spip_query('SELECT A.email 
											FROM spip_applicants AS A
											INNER JOIN spip_applications AS APP ON APP.id_applicant=A.id_applicant
											WHERE APP.id_application="'.$id_application.'"');
		list($email_applicant) = spip_fetch_array($res_email_applicant, SPIP_NUM);
