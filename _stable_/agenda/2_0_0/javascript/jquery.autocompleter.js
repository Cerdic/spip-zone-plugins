...
// ameliore par Cedric Morin 
// - correction flickering bug sous FF 2
jQuery.autocomplete = function(input, options) {
	...
	.focus(function(){
		...
		hasFocus = true;
		if (options.delay==0 && options.minChars==0)
			onChange()
	})
	.blur(function() {
	...
	function onChange() {
		...
		var v = '';
		// si pas encore de saisie, on ne prend pas en compte la valeur du champ
		if (lastKeyPressCode){
			v = $input.val();
			if (v == prev) return;
		}
	...
	function dataToDom(data) {
		...
		for (var i=0; i < num; i++) {
			var row = data[i];
			var value = (typeof row == 'string')?row:row[0];
			if (!row) continue;
			var li = $(ul).append('<li></li>').children("li:last").get(0);
			if (options.formatItem) {
				li.innerHTML = options.formatItem(row, i, num);
				li.selectValue = value;
			} else {
				li.innerHTML = value;
				li.selectValue = value;
			}
	...
	function loadFromCache(q) {
		if (!q && options.minChars>0) return null;
		if (!q) return options.data; // on retourne tout si pas de saisie et minchars=0
