	...
	function exec_lettres_statistiques() {
		...
		if (empty($_GET['id_lettre'])) {
			...
			for ($i = 1; $i <= 7; $i++) {
				...
				list($nb_inscriptions) = spip_fetch_array(spip_query('SELECT COUNT(id_lettre) FROM spip_lettres_statistiques WHERE date LIKE "'.$date_mysql.'%" AND type="inscription"'),SPIP_NUM);
				$tableau_inscriptions_7j[$nom_jour] = $nb_inscriptions;
				list($nb_desinscriptions) = spip_fetch_array(spip_query('SELECT COUNT(id_lettre) FROM spip_lettres_statistiques WHERE date LIKE "'.$date_mysql.'%" AND type="desinscription"'),SPIP_NUM);
			...
			for ($i = 1; $i <= 7; $i++) {
				...
				$date_mysql = date('Y-m-d', $timestamp);
				$nom_jour = nom_jour(date('Y-m-d h:i:s', $timestamp));
				list($nb_inscriptions) = spip_fetch_array(spip_query('SELECT COUNT(id_lettre) FROM spip_lettres_statistiques WHERE id_lettre="'.$id_lettre.'" AND date LIKE "'.$date_mysql.'%" AND type="inscription"'),SPIP_NUM);
				$tableau_inscriptions_7j[$nom_jour] = $nb_inscriptions;
				list($nb_desinscriptions) = spip_fetch_array(spip_query('SELECT COUNT(id_lettre) FROM spip_lettres_statistiques WHERE id_lettre="'.$id_lettre.'" AND date LIKE "'.$date_mysql.'%" AND type="desinscription"'),SPIP_NUM);
				$tableau_desinscriptions_7j[$nom_jour] = $nb_desinscriptions;
			}
