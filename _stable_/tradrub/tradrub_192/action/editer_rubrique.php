...
function rubrique_referent ($id_rubrique, $c) {
	...
	spip_query("SELECT id_trad FROM spip_rubriques WHERE id_rubrique=$lier_trad AND NOT(id_rubrique=$id_rubrique)")))
	{
		spip_log("echec lien de trad vers rubrique inexistant ($lier_trad)");
		return '&trad_err=1';
	}
	...
	// Si la rubrique vise n'est pas deja traduit, son identifiant devient
	// le nouvel id_trad de ce nouveau groupe et on l'affecte aux deux
	// rubriques
	if ($id_lier == 0) {
		spip_query("UPDATE spip_rubriques SET id_trad = $lier_trad WHERE id_rubrique IN ($lier_trad, $id_rubrique)");
	}
	// sinon on ajouter notre rubrique dans le groupe
	else {
		spip_query("UPDATE spip_rubriques SET id_trad = $id_lier WHERE id_rubrique = $id_rubrique");
	}
