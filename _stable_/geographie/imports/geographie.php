			...
			foreach($table as $region=>$departements){
				...
				foreach($departements as $departement=>$communes) {
					...
					foreach($communes as $commune) {
						sql_insertq('spip_geo_communes',array('nom'=>$commune['nom'],'id_commune'=>$commune['insee'],'code_postal'=>$commune['cp'],'latitude'=>$commune['lat'],'longitude'=>$commune['long'],'id_departement'=>$id_departement));
					}
				}
			}
		}
