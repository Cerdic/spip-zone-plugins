<?php
	...
	include_spip('notifications_fonctions');
	...
	class Notification extends PHPMailer {
		...
		public function Notification($email, $objet, $message_html, $message_texte) {
			...
			if ($GLOBALS['meta']['spip_notifications_adresse_envoi'] == 'oui') {
				$this->From		= $GLOBALS['meta']['spip_notifications_adresse_envoi_email'];
				$this->FromName	= $GLOBALS['meta']['spip_notifications_adresse_envoi_nom'];
			} else {
			...
			if (isset($GLOBALS['meta']['spip_notifications_smtp_sender'])) {
	       		$this->Sender = $GLOBALS['meta']['spip_notifications_smtp_sender'];
	       		$this->AddCustomHeader("Errors-To: ".$this->Sender);
			}

			if (isset($GLOBALS['meta']['spip_notifications_smtp']) AND $GLOBALS['meta']['spip_notifications_smtp'] == 'oui') {
	    		$this->Mailer	= 'smtp';
			    $this->Host 	= $GLOBALS['meta']['spip_notifications_smtp_host'];
			    $this->Port 	= $GLOBALS['meta']['spip_notifications_smtp_port'];
				if ($GLOBALS['meta']['spip_notifications_smtp_auth'] == 'oui') {
				    $this->SMTPAuth = true;
				    $this->Username = $GLOBALS['meta']['spip_notifications_smtp_username'];
				    $this->Password = $GLOBALS['meta']['spip_notifications_smtp_password'];
				} else {
				    $this->SMTPAuth = false;
				}
				if ($GLOBALS['meta']['spip_notifications_smtp_secure'] == 'ssl')
				    $this->SMTPSecure = 'ssl';
				if ($GLOBALS['meta']['spip_notifications_smtp_secure'] == 'tls')
				    $this->SMTPSecure = 'tls';
			}
			...
			if (!empty($message_html)) {
	     		...
	     		$this->IsHTML(true);
				if ($GLOBALS['meta']['spip_notifications_filtre_css'])
					$this->ConvertirStylesEnligne();
				if ($GLOBALS['meta']['spip_notifications_filtre_images'])
			...
			if ($GLOBALS['meta']['spip_notifications_filtre_iso_8859'])
			...
			foreach ($styles[1] as $i => $type) {
				if ($type=="" && $styles[3][$i]=="") {
					...
					if (!preg_match("/ /", $styletag) && !preg_match("/#/", $styletag)) {
						$pattern = "!<(".$styletag.")([^>]*(?= /)|[^>]*)( /)?>!mi";
						$this->Body = preg_replace_callback ($pattern, 'notifications_addstyle' , $this->Body);
