...
function numero_affiche_droite($flux){
	if ($flux['args']['exec']=='naviguer'){
		...
		$url = $securiser_action('renumeroter',"rubrique-$id_rubrique",_DIR_RESTREINT_ABS . str_replace('&amp;','&',self(true)));
		$out .= "<div><a href='$url'><img src='"._DIR_PLUGIN_NUMERO."img_pack/numerote.gif' width='48' height='24' alt='Re-numeroter' /></a>";
		$url = $securiser_action('denumeroter',"rubrique-$id_rubrique",_DIR_RESTREINT_ABS . str_replace('&amp;','&',self(true)));
		...
		$url = $securiser_action('renumeroter',"article-$id_rubrique",_DIR_RESTREINT_ABS . str_replace('&amp;','&',self(true)));
		$out .= "<div><a href='$url'><img src='"._DIR_PLUGIN_NUMERO."img_pack/numerote.gif' width='48' height='24' alt='Re-numeroter' /></a>";
		$url = $securiser_action('denumeroter',"article-$id_rubrique",_DIR_RESTREINT_ABS . str_replace('&amp;','&',self(true)));
		$out .= "&nbsp;<a href='$url'><img src='"._DIR_PLUGIN_NUMERO."img_pack/denumerote.gif' width='48' height='24' alt='Enlever la numerotation' /></a></div>";
