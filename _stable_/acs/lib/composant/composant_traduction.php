...
function composant_traduction($c='fond', $l='fr', $module='') {
  ...
  $r = cache('affiche_composant_traduction', 'ct_'.$GLOBALS['meta']['acsModel'].'_'.$c.'_'.(($module != '') ? $module.'_' : '').$l, array($c, $l, $module));
  return $r[0];
}
 ...
 */
function affiche_composant_traduction($c, $l, $module) {
  $tableau = lecture_composant_traduction($c, $l, $module);
  ksort($tableau);
  $nb = count($tableau);
  $r = '<div class="acs_box">';
  ...
  foreach ($tableau as $raccourci => $val) {
    ...
    $r .= '</b></td><td class="arial2"><table  cellpadding="2px;" cellspacing="0" style="table-layout: fixed; width: 100%; overflow: hidden"><tr><td class="crayon crayon_'.$c.'_'.$raccourci.' type_traduction traduction-'.$c.'_'.$raccourci.'-0 lang_'.$l.($module ? '_'.$module : '').'">'.$val."</td></tr></table></td></tr>";
  }
 ...
 */
function lecture_composant_traduction($c, $l, $module) {
