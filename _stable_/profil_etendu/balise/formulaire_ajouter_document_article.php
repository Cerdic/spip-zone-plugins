...
function balise_FORMULAIRE_AJOUTER_DOCUMENT_ARTICLE($p) {
  return calculer_balise_dynamique($p,'FORMULAIRE_AJOUTER_DOCUMENT_ARTICLE',array('id_article','id_rubrique','statut'));
}
...
function balise_FORMULAIRE_AJOUTER_DOCUMENT_ARTICLE_dyn($id_article,$id_rubrique,$statut_article) {
	...
	if (!$GLOBALS["auteur_session"]) 
		return array('formulaires/formulaire_ajouter_document_login', 0, array('var_login'=>($_REQUEST['var_login']),'block_visible' => ' '));
...
//	OR ($flag_auteur AND ($statut_article == 'publie') and ($droit_modif=='Oui')));
	...
	if ($flag_modif){
		...
		return array('formulaires/formulaire_ajouter_document', 0,
			array(
						'id_chose' => $id_article,
						'type' => 'article',
