...
/*  
  // liste_des_champs() ne retourne que les champs compatibles avec la recherche "classique"
  // on veut *tous* les champs indexables FULLTEXT 
  global $tables_principales;
  $champs_exclus = array('alea_futur','alea_actuel','pass','low_sec','htpass','prefs','cookie_oubli');
  foreach ($tables as $table=>$champs) {
      $nom_table = $GLOBALS['table_prefix'].'_'.($table == 'site'? 'syndic' : $table.($table == 'forum'? '' : 's'));
      foreach($tables_principales[$nom_table]['field'] as $champ=>$desc) {
          if (!in_array($champ, $champs_exclus) AND !array_key_exists($champ, $champs) AND substr_count($desc, 'text') > 0)
            $tables[$table][$champ] = 1;            
      }
echo '<br><br>$table '.$table.'<br>';
