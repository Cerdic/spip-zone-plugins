...
function jeux_ajouter_resultat($id_jeu, $resultat, $total, $resultat_detaille=''){
	...
	$requete = spip_fetch_array(spip_query("SELECT enregistrer_resultat,resultat_unique FROM spip_jeux WHERE id_jeu =".$id_jeu));
	...
	if (!$insert) {
		...
		if (!$requete){
			...
			$resultat_unique == 'oui' ? $insert=false : $insert = true; //on n'insert pas si on doit avoir affaire à un resultat unique 
		}
