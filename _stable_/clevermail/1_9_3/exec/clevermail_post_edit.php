...
function exec_clevermail_post_edit() {
	...
	if (isset($_POST['pst_subject'])) {
		...
		if ($post['pst_subject'] != '') {
			...
			$post['pst_html'] = addslashes(recuperer_page($list['lst_url_html']),dirname($list['lst_url_html']));
			$post['pst_text'] = addslashes(wordwrap(recuperer_page($list['lst_url_text']),dirname($list['lst_url_text']), 70));
