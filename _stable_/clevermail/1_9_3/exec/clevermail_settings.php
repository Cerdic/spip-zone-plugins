...
function exec_clevermail_settings() {
	if($_POST) {
		...
		spip_query("UPDATE cm_settings SET set_value = '".$_POST['CM_MAIL_RETURN']."' WHERE set_name='CM_MAIL_RETURN'");
...
?>
		<form action="<?php echo generer_url_ecrire('clevermail_settings',''); ?>" method="post">
			...
			<label><?php echo _T('clevermail:email_return_path') ?> :</label><br />
			<input type="text" name="CM_MAIL_RETURN" value="<?php echo $cm_mail_return['set_value'] ?>" class="formo" /><br />
