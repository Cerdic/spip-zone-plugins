...
exec_odb_notes ()
{
  ...
  global $connect_statut, $connect_toutes_rubriques, $debug, $txt_gauche,
    $txt_droite, $txt_debug, $tab_referentiel, $odb_referentiel, $odb_mapping;
  ...
  echo "<SCRIPT SRC='".DIR_ODB_COMMUN.
    "contrib/boxover/boxover.js'></SCRIPT>\n";
  $imgInfo =
    "<img src='".DIR_ODB_COMMUN.
    "contrib/boxover/info.gif' style='vertical-align:middle'>";
  ...
  $gauche =
    "<IMG SRC='"._DIR_PLUGIN_ODB_NOTES.
    "/img_pack/logo_odb_150.png' alt='Office du bac' ALIGN='absmiddle'><br/><br/>\n";
  ...
  if ($r_jury != '' && !in_array ($r_jury, $tJurys))
    die (KO.
	 ' - Vous n\'&ecirc;tes pas autoris&eacute;(e) &agrave; saisir le jury '.
	 $r_jury.', veuillez <A HREF="'.generer_url_ecrire ('odb_notes').
	 '">recommencer</A>');
  if (isset ($_REQUEST['imprimer']))
	...
	$serie = $tRefSerie[$id_serie];
      ...
      $isPleinEcran = false;
      $titrePrincipal =
	"Impressions pour le jury $jury s&eacute;rie $serie $annee";
      $selectJury =
	"<SELECT NAME='jury' class='fondo' onChange=\"document.forms['form_resultats'].submit();\">\n";
	...
	die (KO." - Aucun jury d&eacute;fini pour vous en $annee");
      $selectSerie =
	"<SELECT NAME='id_serie' class='fondo' onChange=\"document.forms['form_resultats'].submit();\">\n\t<OPTION VALUE=''>-=[S&eacute;rie]=-</OPTION>\n";
      foreach ($tSeries as $iIdSerie = >$sSerie)
      {
	if ($id_serie == $iIdSerie)
	  $selected = 'selected';
	else
	  $selected = '';
	$selectSerie. =
	  "\t<OPTION VALUE='$iIdSerie' $selected>$sSerie</OPTION>\n";
      }
      $selectSerie. = "</SELECT>\n";
      $msg =
	"<table class='spip'>\n<form name='form_resultats' action='".
	generer_url_ecrire ('odb_notes').
	"' method='get' class='spip_xx-small'/>\n".
	...
	"<input type='hidden' name='imprimer' value='auto'/>\n</FORM>\n";
      $msg. =
	"<tr><th colspan=2>Retour aux <A HREF='".
	generer_url_ecrire ('odb_notes').
	"&jury=$jury&annee=$annee&resultats'>r&eacute;sultats</A></th></tr>\n";
      $msg. = "</TABLE>\n";
      ...
      else
	{
	  $deliberation = 1;
	  $msg. =
	    "Vous n'avez pas acc&egrave;s aux impressions du jury $jury, veuillez choisir un autre jury svp.";
	}
    }
  elseif (isset ($_REQUEST['historique']))
  {
	...
	$msg. = afficherHistorique ($id, $annee);
      ...
      $msg. = KO." - Erreur inconnue - ne peut afficher l'historique\n";
    if ($msgRetour != '')
      {
	$msg. =
	  "Navigation rapide : <b><A HREF='$urlRetour'>$msgRetour</A></b>\n<br/>\n";
      }
  }
  elseif (isset ($_REQUEST['resultats']))
  {
    ...
    $isPleinEcran = true;
    $titrePrincipal =
      "R&eacute;sultats des notes du jury $jury s&eacute;rie $serie";
    $selectJury =
      "<SELECT NAME='jury' class='fondo' onChange=\"document.forms['form_resultats'].id_serie.value='';document.forms['form_resultats'].submit();\">\n";
    foreach ($tJurys as $sJury)
    {
      if ($jury == $sJury)
	$selected = 'selected';
	...
	if (isset ($_REQUEST['total']))
	  {
	    ...
	    if ($id_serie != '')
	      $extra =
		"<hr size=1/><A HREF='".generer_url_ecrire ('odb_notes').
		"&jury=$jury&serie=$serie&resultats'><b>Filtrer les notes</b> &agrave; la d&eacute;lib&eacute;ration en cours</A>\n";
	  }
	else
	  {
	    $nbCandidatsOral = getNbCandidats ($annee, $r_jury, 'Oral');
	    if ($nbCandidatsOral > 1)
	      $deliberation = 3;
	    else
	      $deliberation = 2;
	    if ($id_serie != '')
	      $extra =
		"<hr size=1/><A HREF='".generer_url_ecrire ('odb_notes').
		"&jury=$jury&serie=$serie&resultats&total'>Afficher <b>toutes</b> les notes</A>\n";
	  }
	include_once (DIR_ODB_COMMUN.'inc-html.php');
	$msg. =
	  "<tr><td>".vignette ('pdf').
	  "</td><td colspan='2'>Vous &ecirc;tes autoris&eacute;(e) &agrave; <b><A HREF='".
	  generer_url_ecrire ('odb_notes').
	  "&jury=$r_jury&serie=$serie&annee=$annee&imprimer'>acc&eacute;der aux impressions PDF</A></b><br/><b>Note :</b> acc&eacute;dez &agrave; ce module si les r&eacute;sultats vous semblent incorrects$extra</td></tr>\n";
      }
    else
      {
	$deliberation = 1;
    ...
    if ($id_serie != '')
      {
	$msg. =
	  afficherNotes ($jury, $id_serie, $annee, '', $deliberation, true);
	$msg. =
	  "Navigation rapide : <b><A HREF='".generer_url_ecrire ('odb_notes').
	  "&jury=$jury&serie=$serie'>mati&egrave;res du jury $jury, s&eacute;rie $serie</A></b>\n";
      }
    else
      $msg. = "Veuillez choisir une s&eacute;rie pour le jury $jury\n";
  }
  elseif (isset ($_REQUEST['step3']) || isset ($_REQUEST['step4']))
  {
    //////////////////////////////////////////////// step 4 + step 3 : formulaire de saisie des notes
    foreach (array
	     ('type', 'id_anonyme', 'note', 'jury', 'serie', 'id_serie',
	      'coeff', 'matiere', 'id_matiere') as $col) $$col =
      urldecode ($_REQUEST[$col]);
    $r_type = $type;
    $msg = '';

    $gauche. =
      "<br/><br/>\nNavigation rapide\n<ul class='tout-site'>\n<li class='sec'><A class='titre' href='".
      generer_url_ecrire ('odb_notes').
      "&jury=$jury&serie=$serie&id_serie=$id_serie&step2=manuel'>Mati&egrave;re</A></li>\n".
      "<li class='sec'><A class='titre' href='".
      generer_url_ecrire ('odb_notes').
      "&jury=$jury&serie=$serie&id_serie=$id_serie&resultats=manuel'>R&eacute;sultats jury $jury</A></li>\n".
      "<br/><li class='sec'><b><A class='titre' href='../spip.php?action=logout&logout=prive'>Se d&eacute;connecter</A></B></li>\n</ul>\n";
    $gauche2 =
      "- Pour enregistrer une note, tapez [<b>Entr&eacute;e</b>] au lieu de cliquer sur le bouton pour gagner du temps<br/><br/>- Saisissez un tiret [<b>-</b>] comme note pour signifier qu'un candidat n'a pas compos&eacute;";
    //echo $id_anonyme;
	...
	if (substr_count ($id, '-') > 0)
	  {
	    //TODO mon dieu quelle horreur
	    $sql =
	      "SELECT DECODE(id_anonyme,'$pass') id_anonyme from odb_repartition where id_table='$id'";
	    $result = odb_query ($sql, __FILE__, __LINE__);
	    $row = mysql_fetch_array ($result);
	    $id_anonyme = $row['id_anonyme'];
	    $idTableOuRien =
	      "(SELECT id_table from odb_repartition where DECODE(id_anonyme,'$pass')='$id_anonyme' and annee=$annee)";
	    //print_r($row);
	  }
	//FIXME supprimer ligne suivante pour ne plus enregistrer les id_table dans la base
	$idTableOuRien =
	  "(SELECT id_table from odb_repartition where DECODE(id_anonyme,'$pass')='$id_anonyme' and annee=$annee)";

	$sql1 =
	  "INSERT into odb_histo_notes (id_table, id_anonyme, annee, id_matiere, note, type, coeff, operateur, maj)  \n".
	  "\t(SELECT id_table, id_anonyme, annee, id_matiere, note, type, coeff, operateur, maj from odb_notes \n\twhere id_anonyme='$id_anonyme' and annee=$annee and id_matiere=$id_matiere and type='$type')";
	$sql =
	  "REPLACE into odb_notes (id_table, id_anonyme, annee, id_matiere, note, type, coeff, operateur, maj) VALUES (\n".
	  "\t$idTableOuRien, $id_anonyme, $annee, $id_matiere, $note, '$type', $coeff,'".
	  $GLOBALS['auteur_session']['login']."',NOW()\n)";
	//echo "<pre>$sql</pre>";
	if ($note != '')
	  {
	    if ($note > 20)
	      $msg. =
		'<small>'.KO.
		" - Vous avez essay&eacute; de saisir la note <b>$note</b>, qui est sup&eacute;rieure &agrave; 20. <b>Saisie ignor&eacute;e</b></small><br/>\n";
	    elseif ($note == '') $msg. =
	      '<small>'.KO.
	      " - Note inconnue. <b>Saisie ignor&eacute;e</b></small><br/>\n";
	    else
	    {
	      ...
	      //echo "<pre>$sql</pre>";
	      $msg. =
		'<small>'.OK." - Le candidat <b>$id</b> a eu ".($coeff ==
								0 ?
								"+<b>$note</b>"
								:
								"<b>$note</b><small>/20</small> <small>(soit ".
								($note *
								 $coeff).
								'/'.(20 *
								     $coeff).
								")</small>").
		" en <b>$matiere</b> <small>($type)</small></small><br/>\n";
	    }
	  }
      }
    ///// fin step 4
    $msg. = "<TABLE class='spip' width='100%'>\n<tr class='row_even'>\n".
      "<th>Jury $jury";
    if ($isOperateur)
      $msg. = " (Centre $deliberationCentre)";
    $msg. =
      "</th><th>S&eacute;rie $serie</th><th style='text-align:right;'>Mati&egrave;re : $matiere</th></tr></table>\n";
    $msg. =
      ...
      if ($id_matiere > 0)
	{
	  if ($type == 'Oral')
	    {
	      $sql =
		"SELECT rep.jury, dep.departement, eta.etablissement centre, ser.serie, rep.id_table, DECODE(rep.id_anonyme,'$pass') id_anonyme, notes.note, exa.coeff\n".
		...
		" where exa.id_matiere=$id_matiere and exa.annee=$annee and exa.id_serie=$id_serie and notes.type='$type'\n".
	...
	if ($type == 'Divers')
	  {
		...
		$where =
		  "and can.ef1 = ef.id and decis.id_table=rep.id_table and decis.delib1='Admissible' and decis.annee=$annee";
		break;
	      case ID_MATIERE_EF2:
		$champ = 'ef.ef,';
		$from = "odb_ref_ef ef, odb_decisions decis,";
		$where =
		  "and can.ef2 = ef.id and decis.id_table=rep.id_table and decis.delib1='Admissible' and decis.annee=$annee";
		break;
	      case ID_MATIERE_EPS:
		$champ = 'eps.eps,';
		$from = "odb_ref_eps eps, odb_decisions decis,";
		$where =
		  "and can.eps=eps.id and eps.eps='Apte' and decis.id_table=rep.id_table and decis.delib1='Admissible' and decis.annee=$annee";
		break;
	      default:
		die (KO.
		     " - Cas impr&eacute;vu : id_matiere=$id_matiere dans les divers");
	      }
	    $sql =
	      "SELECT $champ rep.jury, dep.departement, eta.etablissement centre, ser.serie, rep.id_table, DECODE(rep.id_anonyme,'$pass') id_anonyme, notes.note, 1 coeff\n".
	  ...
	  $sql = "SELECT 1 from odb_param where 1=0";
      }
      $result = odb_query ($sql, __FILE__, __LINE__);
      //echo "<hr/>$type<br/><pre>$sql</pre><br/>";
      while ($row = mysql_fetch_array ($result))
	{
	  foreach (array ('ef', 'id_anonyme', 'id_table', 'note', 'coeff') as
		   $col)
	  {
	    $$col = $row[$col];
	  }
      ...
      )
	{
	  ...
	  {
	  "select$type"}
	  = "<SELECT name='id_anonyme' class='fondo' onChange=\"document.forms['form_$type'].note.value='';document.forms['form_$type'].note.focus();\">"."<OPTION value=''>-=[Candidat]=-</OPTION>";
	  foreach ($
		   {
		   "t$type"}
		   as $id_anonyme = >$note)
	  {
	    ...
	    "select$type"}
	    . =
	      "<OPTION $selected value='$id_anonyme'>$id_anonyme $tEf[$id_anonyme] $aff_note</OPTION>\n";
	  }
	  $
	  {
	  "select$type"}
	  ...
	  if ($isEf)
	    {
	      $inputNote =
		"<INPUT name='note' size=2 maxlength=2 onKeyUp=\"if(isNaN(parseInt(this.value))) this.value='';if(this.value>5) {alert('Un candidat ne peut avoir plus de 5 points de bonus par EF, veuillez saisir cette note de nouveau');this.value='';}document.forms['form_$type'].note_coeff.value='+'+this.value\" value='$note_selected[$type]' class='fondo'/>\n";
	      $inputNoteCoeff =
		"<INPUT name='note_coeff' size=2 maxlength=2 value='+".((int)
									$note_selected
									[$type]).
		"' style='text-align:right;border:0px;none;#fff;' onFocus='blur();'/>\n";
	    }
	  else
	    {
	      $inputNote =
		"<INPUT name='note' size=2 maxlength=2 onKeyUp=\"if(this.value!='-' && isNaN(parseInt(this.value))) this.value='';if(this.value>20) {alert('Un candidat ne peut avoir plus de 20/20, veuillez saisir cette note de nouveau');this.value='';}document.forms['form_$type'].note_coeff.value=this.value*$tCoeff[$type]\" value='$note_selected[$type]' class='fondo'/>\n";
	      $inputNoteCoeff =
		"<INPUT name='note_coeff' size=2 maxlength=3 value='".((int)
								       $note_selected
								       [$type]
								       *
								       $tCoeff
	  ...
	  "select$type"}."</td>\n".
	    ...
	    "<tr>\n\t<td>Note</td>\n\t<td>$inputNote".($isEf ? '' :
						       '<small>/20</small>').
	    $inputNoteCoeff.($isEf ? '' : "<small>/".($tCoeff[$type] * 20).
			     "</small>")."</td>\n"."</tr>\n</table>\n";
	  //"<tr>\n\t<td colspan=2><INPUT TYPE='submit' name='step4' value='Enregistrer $type' class='forml'/></td>\n</tr>\n</table>\n";
	  $
	  {
	  "t$type"} = array_reverse ($
				     {
				     "t$type"}, true);
	  $msg. = "<br/>Notes $type (ordre invers&eacute;)<br/>\n".
	    "<table class='spip' width='90%'>\n<tr><th>Candidat</th><th>".
	    ($isEf ? 'Bonus' : 'Note <small>/20</small>').
	    "</th><th>".($isEf ? '&Eacute;p. Fac.' :
			 "Note <small>/".(20 *
					  $tCoeff[$type])."</small>").
	    "</th>\n</tr>\n";
	  foreach ($
		   {
		   "t$type"} as $id_anonyme = >$note)
	  {
	    if ($note != '')
	      {
		if ($note < 0)
		  {
		    $note =
		      '<span style="color:#f00;font-weight:bold;" title="Non Connue">N/C</span>';
		    $note_coeff = '';
		  }
		else
		  {
			...
			$note_coeff = $tEf[$id_anonyme];
		      ...
		      {
			$note_coeff =
			  ($note * $tCoeff[$type])."/".(20 * $tCoeff[$type]);
	  ...
	  $coeff = $tCoeff[$type];
	  foreach (array
		   ('matiere', 'id_matiere', 'jury', 'serie', 'id_serie',
		    'type', 'coeff') as $var) $msg. =
  ...
  else
  {
    ...
    $gauche. = "<br/><br/>\nNavigation rapide\n<ul class='tout-site'>\n".
      "<li class='sec'><A class='titre' href='".
      generer_url_ecrire ('odb_notes').
      "&jury=$jury_aff&serie=$r_serie&resultats=manuel'>R&eacute;sultats $msgTmp</A></li>\n".
      "<br/><li class='sec'><b><A class='titre' href='../spip.php?action=logout&logout=prive'>Se d&eacute;connecter</A></B></li>\n</ul>\n";

    $tdJurys =
      "<INPUT type='hidden' name='jury' value='".$_REQUEST['jury']."'/>\n";
    $styleBouton = 'font-weight:normal;border-width:1px;';
    $styleBoutonActif = 'font-weight:bold;border-width:2px;';

    $sql =
      "SELECT DISTINCT jury, ser.id id_serie, ser.serie\n FROM odb_ref_serie ser, odb_repartition rep, odb_candidats can\n".
    ...
    if ($_REQUEST['jury'] > 0)
      {
	// on commence par la pour pouvoir affiche la bulle des series au survol des jurys
	$r_jury = $_REQUEST['jury'];
	$r_serie = $_REQUEST['serie'];
	...
	{
	  if ($serie == $r_serie)
	    $style = $styleBoutonActif;
	  else
	    $style = $styleBouton;
	  $tdSeries. =
	    "<INPUT type='submit' class='forml' name='step2' value='$serie' style='$style' ".
	    "onClick=\"document.forms['form_notes'].serie.value='$serie';document.forms['form_notes'].matiere.value='';\"/><br/>\n";
	  //"<INPUT type='submit' class='fondo' name='resultats' value='R&eacute;sultats $serie'".
	  //"onClick=\"document.forms['form_notes'].serie.value='$serie';\"/><br/>&nbsp;\n";
	}
	if ($r_serie != '')
	  {
	    odb_maj_decisions ($annee, $r_jury, 3, 1);
	    odb_maj_decisions ($annee, $r_jury, 3, 2);

	    $r_matiere = $_REQUEST['matiere'];
	    $sql =
	      "SELECT DISTINCT id_matiere, matiere, id_serie from odb_ref_examen exa, odb_ref_serie ser, odb_ref_matiere mat\n where exa.annee=$annee and ser.serie='$r_serie' and ser.id=exa.id_serie and mat.id=exa.id_matiere\n ORDER BY matiere";
	    $result = odb_query ($sql, __FILE__, __LINE__);
	    $id_serie = 0;
	    while ($row = mysql_fetch_array ($result))
	      {
		$id_matiere = $row['id_matiere'];
		$tMatieres[$id_matiere] = $row['matiere'];
		$id_serie = $row['id_serie'];
	      }
	    $tMatieres[ID_MATIERE_EF1] = LIB_MATIERE_EF1;
	    $tMatieres[ID_MATIERE_EF2] = LIB_MATIERE_EF2;
	    $tMatieres[ID_MATIERE_EPS] = LIB_MATIERE_EPS;
	    //echo $sql;
	    $tdMatieres =
	      "<INPUT type='hidden' name='matiere' value='$r_matiere'/>\n".
	      "<INPUT type='hidden' name='id_matiere' value='$id_matiere'/>\n".
	      "<INPUT type='hidden' name='id_serie' value='$id_serie'/>\n";
	    foreach ($tMatieres as $id_matiere = >$matiere)
	    {
	      if ($matiere == $r_matiere)
		...
		$style. = 'background-color:#ece;';
	      $tdMatieres. =
		"<INPUT type='submit' class='forml' name='step3' value=\"$matiere\" style='$style' ".
		"onClick=\"document.forms['form_notes'].id_matiere.value='$id_matiere';document.forms['form_notes'].matiere.value='".
		urlencode ($matiere)."';\"/><br/>\n";
	    }
	  }
	else
	  $tdMatieres = "Veuillez choisir une s&eacute;rie";
      }
    else
      $tdSeries = 'Veuillez s&eacute;lectionner un jury';

    foreach ($tJurys as $jury)
    {
      if ($jury == $_REQUEST['jury'])
	$style = $styleBoutonActif;
      else
	$style = $styleBouton;
      if ($isAdmin)
	...
	$s = '';
      $titleJury =
	"header=[Jury $jury]	body=[S&eacute;rie$s ".implode (', ',
							     $tSeries[$jury]).
	"]	fade=[on] fadespeed=[0.5]";
      $tdJurys. =
	"<INPUT title=\"$titleJury\" type='submit' class='$class'	name='step2' value='Jury ".
	str_pad ($jury, 3, '0',
		 PADDING_LEFT)."' style='$style' ".
    ...
    $formNotes =
      "<form name='form_notes' action='".generer_url_ecrire ('odb_notes').
      "' method='get' class='spip_xx-small'>\n".
      "<INPUT type='hidden' name='exec' value='odb_notes'/>\n".
  ...
  debut_droite ();
  if ($titrePrincipal == '')
    $titrePrincipal = "Saisie des notes $annee ($deliberationCentre)";
  if ($isDessineCadrePrincipal)
    debut_cadre_relief ("", false, "", $titre = _T ($titrePrincipal));
  else
    debut_boite_info ();
  echo $msg;
  if ($isDessineCadrePrincipal)
