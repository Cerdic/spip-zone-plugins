...
function exec_config_chercher_squelettes_mots() {
  ...
  echo $commencer_page('&laquo; '._T('squelettesmots:titre_page').' &raquo;', 'configurations', 'mots_partout');
  ...
  if ($connect_statut == '0minirezo' AND $connect_toutes_rubriques ) {
	...
	echo debut_boite_info(true), propre(_T('squelettesmots:help')), fin_boite_info(true);

	echo debut_droite('', true);
	
	
	
	include_spip('ecrire/inc_config');
	...
	foreach($fonds as $fond => $a) {
	  ...
	  while ($r = sql_fetch($rez)) {
		include_spip("ecrire/inc_charsets");
	  ...
	  echo '<div class="possible"><br />';
	  echo _T('squelettesmots:possibilites',array('total_actif' => $cnt_actif, 'total_inactif'=>$cnt_inactif));
	  if($cnt_actif+$cnt_inactif > 0) echo bouton_block_depliable(_T("regle$index"),true,"regle$index");
	  if ($cnt_actif+$cnt_inactif > 0) {
		echo debut_block_depliable(true, "regle$index");
		echo $liste_squel;
		echo fin_block();
	  }
	  echo '</div>';
