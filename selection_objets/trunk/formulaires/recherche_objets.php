...
function formulaires_recherche_objets_charger_dist($objet_dest='rubrique', $id_objet_dest, $lang='') {
	...
	if (in_array($objet_dest,$special)) {
		$choisies = picker_selected(lire_config('selection_objet/selection_'.$objet_dest.'_dest',array()),$objet_dest);
	} else {
		$choisies = lire_config('selection_objet/selection_'.$objet_dest.'_dest',array());
	}
	...
	if (!$types=lire_config('selection_objet/type_liens_'.$objet_dest_original,array())) {
...
function formulaires_recherche_objets_verifier_dist($objet_dest='rubrique',$id_objet_dest,$lang=''){
	...
	if (!_request('objet_sel')) {
		...
		if (!isset($config['choix_illimite']) AND $id = sql_getfetsel('id_selection_objet', 'spip_selection_objets', $where)) {
			$erreurs['objet_sel']=_T("selection_objet:erreur_deja_selectionne");
		}
	}
