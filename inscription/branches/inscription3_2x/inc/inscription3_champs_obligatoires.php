 ...
 */
function inc_inscription3_champs_obligatoires_dist($id_auteur=null) {
	...
	foreach ($config_i3 as $clef => $valeur) {
		...
		if ((!empty($resultat[0])) && ($config_i3[$resultat[1].$suffixe] == 'on') && ($config_i3[$resultat[1].'_obligatoire'] == 'on') && (!in_array($resultat[1],$exceptions_des_champs_auteurs_elargis))) {
			$valeurs[] = $resultat[1];
		}
	}
	if($config_i3['reglement']){
		$valeurs[] = 'reglement';
	}
