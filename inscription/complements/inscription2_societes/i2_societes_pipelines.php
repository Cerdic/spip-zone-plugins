...
function i2_societes_affiche_droite($flux) {
	if (((preg_match('/^inscription2/', $flux['args']['exec']))
		or (preg_match('/^auteurs/', $flux['args']['exec']))
		or (preg_match('/^i2_/', $flux['args']['exec']))
		or (($flux['args']['exec'] == 'cfg') and ((_request('cfg') == 'inscription2') or preg_match('/^i2_/', _request('cfg'))))
		)
		and ($flux['args']['exec'] != 'inscription2_adherents')) {
