...
function inscription_import_table_fields() {
	...
	foreach ($config as $clef => $val) {
		...
		if (!in_array($cle, $champ_presents) and ($val == 'on')) {
			if (!in_array($cle, $exceptions_des_champs_auteurs_elargis)
				and in_array($cle, $spip_auteurs)
...
function inscription_import_array_visu_assoc($data, $table_fields, $assoc_field, $nombre_lignes = 0) {
	...
	if ($data != false) {
		foreach ($data as $key => $ligne) {
			...
			$output .= "</tr>\n";
			if (($nombre_lignes > 0)  and (++$data_count >= $nombre_lignes)) {
				break;
			}
		}
	}
...
function inscription_import_field_associate($data, $table_fields, $assoc_field) {
	...
	foreach ($assoc as $key => $value) {
		...
		if ((isset($table_fields[$value])) or ($value==-1)) {
			$good_value = true;
		}
		if (($good_key == false) or ($good_value == false)) {
			unset($assoc[$key]);
		} else {
			unset($csvfield[$key]);
			if ($value!=-1) {
				unset($table_fields[$value]);
			}
		}
	}
				...
				if ((isset($assoc[$kc])) && (isset($ligne[$assoc[$kc]]))) {
					...
					if (!isset($erreurs[$count_lignes][$key])) {
						if (in_array($key, $table_fields)) {
							if (in_array($key, $auteurs_obligatoires) and (strlen($ligne[$assoc[$kc]]) == 0)) {
			...
			if (count($erreurs[$count_lignes], COUNT_RECURSIVE) == 0) {
				if (count($auteurs_insert)) {
					...
					if (!isset($auteurs_insert['statut'])) {
						$auteurs_insert['statut'] = lire_config('inscription3/statut_nouveau') ?
							lire_config('inscription3/statut_nouveau'):'6forum';
					}
