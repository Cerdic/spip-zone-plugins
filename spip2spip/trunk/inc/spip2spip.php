    ...
    if ($row_site = sql_fetsel("*","spip_spip2spips","id_spip2spip=".intval($id_site))) {    
      ...
      if (!$rss) {
        ...
        if (is_array($articles)) { 
            foreach ($articles as $article) {
              ...
              if (isset($article['link'])) {    		  	    
                ...
                if ($nb_reponses>0) {                                   
                   ...
                   $log_html .= "<li>[<span style='color:#999'>"._T('spip2spip:imported_already')."</span>] <a href='$current_link'>$current_titre</a></li>\n";
                   spip_log("spip2spip: deja importe: ".$current_link);                    
                } else {
                   // nouvel article à importer
                   $log_html .= "<li>[<span style='color:#090'>"._T('spip2spip:imported_new')."</span>] <a href='$current_link'>$current_titre</a>\n";
                   
                   // on cherche la rubrique destination
                   $target = spip2spip_get_id_rubrique($article['keyword']);
                   if (!$target) {  // pas de destination
                      $log_html .= "<div style='color:#009'>"._T('spip2spip:no_target')." <strong>".$article['keyword']."</strong></div></li>\n";                    
                   } else {   
                     ...
                     if  ($_trad) {                     
                      ...
                      if ($_evenements!="") {  
                        ...
                        foreach($_evenements as $_evenement) {                  
                            ...
                            $log_html .= "<div style='padding:2px 5px;border-bottom:1px solid #5DA7C5;background:#eee;display:block;'>"._T('spip2spip:event_ok')." : $datedeb  $lieu</div>";
          ...
          $log_html .= "<div style='color:purple'>"._T('spip2spip:aucun_article')."</div>";
        }
        // #analyse article
        //----*************
      } 	
