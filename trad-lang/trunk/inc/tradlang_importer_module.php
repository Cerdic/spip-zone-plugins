...
function inc_tradlang_importer_module($module, $dir_lang = false, $new_only = false) {
	...
	foreach ($langues_module as $langue) {
		...
		if ($mode == 'update' and sql_count($res) > 0) {
		...
		foreach (array_unique(array_merge(array_keys($existant), array_keys($chs))) as $id) {
			if (isset($chs[$id]) and !isset($existant[$id])) {
				unset($md5);
				if ($orig) {
					$md5 = md5($chs[$id]);
				} elseif (!isset($liste_id_orig[$id])) {
			...
			} elseif (isset($chs[$id]) and isset($existant[$id])) {
				// * chaine existante
			...
			} else if (!isset($chs[$id]) and isset($existant[$id])) {
				// * chaine supprimee
				spip_log('cas 3', 'tradlang');
				// mettre au grenier
				sql_updateq('spip_tradlangs', array('id' => $id, 'statut' => 'attic'), 'id = ' . sql_quote($id) . ' AND module = '.sql_quote($nom_mod));
				$supprimees++;
			}
	...
	if (count($fichiers_lang) > 0) {
		...
		foreach ($fichiers_lang as $fichier => $chemin) {
			...
			if (preg_match('/^([a-z]*)_([a-z_]*)\.php$/i', $fichier, $module)) {
				...
				} elseif (preg_match('/^([a-z]*_[a-z]*)_([a-z_]*)\.php$/i', $fichier, $module)) {
					if (array_key_exists($module[2], $GLOBALS['codes_langues'])) {
						if (test_espace_prive()) {
							if (strpos($chemin, _DIR_RACINE) !== false) {
								$search = _DIR_RACINE;
...
function tradlang_select_langues_module($module, $name = 'langue_mere', $selected = '', $option_vide = false) {
	...
	if (is_array($tous_modules[$module]) and (count(explode(',', $langues)) > 0)) {
