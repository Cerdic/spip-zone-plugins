...
/* STOCKER LES DOCUMENTS INCONNUS AU FORMAT .ZIP */
	...
	if (!$id_document) {
		...
		//#EXIF
                $date = 'NOW()';
                //on prend la date exif que si 
		//c'est un document local, 
 		//extension jpg ou tiff et 
		//que ce n'est pas juste la vignette d'un autre document
		if($distant == 'non' && ($ext == 'jpg' || $ext == 'tiff') && !$id_document_lie) {
                   include_spip('inc/exif');
                   $date_exif = date_exif($fichier);
                   if($date_exif) $date = "'$date_exif'";
                }
                
		$id_document = spip_abstract_insert("spip_documents",
		"(id_type, titre, date, distant)",
		"($id_type, '".addslashes($titre)."', $date, '$distant')");
                //#EXIF
		if ($id_lien
