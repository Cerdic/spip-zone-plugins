...
// Si cette constante est vraie le fil d'Ariane est plus court et se termine sur le parent
if (!defined('_FIL_ARIANE_COURT')) define('_FIL_ARIANE_COURT', false);
...
function construire_FIL_ARIANE($fil)
{
    ...
    // si le fil d'Ariane est court, on retire le dernier élément
    if (_FIL_ARIANE_COURT)
        array_pop($fil);
    ...
    foreach ($fil as $titre => $lien) {
        ...
        elseif ($passe >= $nb - 1) {
            if (_FIL_ARIANE_COURT) { // si court, lien off dans tous les cas
                $fil_ariane .= "<a href='$lien'>$titre</a>";
            } else { // sinon, lien ou pas selon conf
                if (_FIL_ARIANE_LIEN)
