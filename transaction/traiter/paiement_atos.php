...
function traiter_paiement_atos_dist($args, $retours){
	...
	if ($_REQUEST['montant_selection_1'] OR $_REQUEST['montant_fixe_1'] OR $_REQUEST['montant_1']) $retours['message_ok'] .=  "<div class='transaction_ok atos' style='background: url(".find_in_path('paiement/atos/logo.png').") no-repeat top left'>"._T('transaction:traiter_message_atos').'<p><a href="'.find_in_path("paiement/atos/paiement.php").'"  class="valider"><span>'._T('transaction:valider_paiement').'</span></a></p></div>';
