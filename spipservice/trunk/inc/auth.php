...
/***************************************************************************\
*                      SPIP SERVICE : IMPORTANT                           *
* Pour pouvoir fonctionner, ce plugin surcharge l'authentification de Spip*
* Cette surcharge permet seulement de récupérer PHP_AUTH_USER et          *
* PHP_AUTH_PW depuis REMOTE_USER                                          *
* ET SEULEMENT si l'action demandee est 'spipservice'                     *
* DONC : aucune modification du fonctionnement normal!                    *
...
function inc_auth_dist() {
	...
	spip_log("UTILISATION de l'authentification SpipService...");
...
function auth_mode()
{
	...
	if (!$ignore_auth_http) {
		...
		// --- @spipservice
		if (_request('action')=='spipservice'){
			// ce bout de code permet de recuperer $_SERVER['PHP_AUTH_USER'] et $_SERVER['PHP_AUTH_PW'] depuis $_SERVER['REMOTE_USER']
			// j'ai eu ce probleme avec un hebergement OVH PHP5
...
function auth_init_droits($row)
{
	...
	// --- @spipservice modification, voir commentaires plus bas
	if (_request('action')=='spipservice'){
		$session = charger_fonction('session', 'inc');
	}
	// --- @spipservice fin
	...
	if (!isset($_COOKIE['spip_session'])) {
		...
		// --- @spipservice
		if (_request('action')=='spipservice'){
			//cette methode est placee au debut de la fonction
			// avant de fixer $GLOBALS car lors du chargement du
			// fichier session.php, $GLOBALS est re-initialisee
			// $session = charger_fonction('session', 'inc');
		}else{
			$session = charger_fonction('session', 'inc');
		}
