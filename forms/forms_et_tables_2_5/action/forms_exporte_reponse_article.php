	...
	if (verifier_action_auteur("forms_exporte_reponse_article-$id_donnee",$hash,$id_auteur)==TRUE){
		...
		if ( $row = spip_fetch_array($res) )
		{
			...
			if( $id_champ == "null" || $id_champ == "" ) {
				$titre_article =utf8_encode(html_entity_decode(_T("forms:reponse", array('id_reponse' => $id_donnee)))); //-- correction des accents.
			} else {
		...
		if ($id_article!=0)
			...
			redirige_par_entete(generer_url_ecrire('article_edit',"id_article=$id_article",true));
		else
