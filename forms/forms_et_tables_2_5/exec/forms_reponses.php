...
function exec_forms_reponses(){
  ...
  _Forms_install();
	...
	if ($id_form) {
		...
		$contexteBT = array(
			'style'=>'horizontale ',
			'titre'=>_T("forms:Formulaire"),
			'url'=>"?exec=forms_edit&id_form=$id_form",
			'icon_base'=>'form-24.png',
			'icon_action'=>'rien.gif'
		);
		echo recuperer_fond("fonds/bouton_url",$contexteBT);
		...
		$contexteBT = array(
			...
			'url'=>"?exec=donnees_tous&id_form=$id_form&retour=$nretour",
		...
		if (autoriser('administrer','form',$id_form)) {
			...
			// echo icone_horizontale(_T("forms:telecharger_reponses"),generer_url_ecrire("forms_telecharger","id_form=$id_form&retour=$retour"), _DIR_PLUGIN_FORMS. "img_pack/donnees-exporter-24.png", "rien.gif",false);
			$contexteBT = array(
				'style'=>'horizontale ',
				'titre'=>_T("forms:telecharger_reponses"),
				'url'=>generer_url_ecrire("forms_telecharger","id_form=$id_form&retour=$retour"),
	...
	if (substr(sql_version(), 0, 1) == 3) {
		...
		$query = "SELECT r.*, a.nom, f.titre FROM spip_forms_donnees AS r LEFT JOIN spip_auteurs AS a USING (id_auteur), spip_forms AS f  
		WHERE r.id_form=f.id_form AND r.confirmation ='valide' AND r.statut <> 'poubelle' 
		ORDER BY r.date DESC LIMIT "._q($debut).", "._q($tranche);//-- suppression de la ligne qui n'affiche plus les enregistrements de plus de 6 mois
	}
	else {
	...
	*/
	...
	while ($row = spip_fetch_array($result)) {
		...
		if ($id_article_export==0){
			...
			$contexteBT = array(
				'style'=>'horizontale donnees-24',
				'titre'=>_T("forms:exporter_article"),
				'url'=>generer_action_auteur('forms_exporte_reponse_article',"$id_donnee",self()),
				'icon_base'=>'article-new-24.png',
				'icon_action'=>'rien.gif'
			);
			echo recuperer_fond("fonds/bouton_url",$contexteBT);
		}
			...
			$contexteBT = array(
				'style'=>'horizontale donnees-24',
				'titre'=>_T("forms:voir_article"),
				'url'=>generer_url_ecrire('articles',"id_article="._q($id_article_export)),
				'icon_base'=>'article-24.png',
				'icon_action'=>'rien.gif'
			);
			echo recuperer_fond("fonds/bouton_url",$contexteBT);
		}
