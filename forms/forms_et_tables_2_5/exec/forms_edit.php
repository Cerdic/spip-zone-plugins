...
function contenu_boite_resume($id_form, $row, &$apercu){
	...
	/*$liste = afficher_articles(_T("$prefixei18n:articles_utilisant"),
		...
		'ORDER BY' => "titre"));*/
// SB201208 Affichage de la liste des articles utilisant ce formulaire
	// $liste = afficher_objets('article',_T("$prefixei18n:articles_utilisant"),
		// array('FROM' => 'spip_articles AS articles, spip_forms_articles AS lien',
		// 'WHERE' => "lien.id_article=articles.id_article AND id_form="._q($id_form)." AND statut!='poubelle'",
		// 'ORDER BY' => "titre"));
$liste = "
<div style=\"\" class=\"cadre cadre-liste\">
<img width=\"24\" height=\"24\" class=\"cadre-icone\" alt=\"\" src=\"../prive/images/article-24.gif\">
<div onmouseover=\"jQuery(this).depliant('#t75ea3bc1');\" class=\"titrem depliant deplie blocdeplie\" id=\"b900444f3\">
	<a class=\"titremancre\" onclick=\"return jQuery(this).depliant_clicancre('#t75ea3bc1');\" href=\"#\"></a>
	<b>Articles utilisant ce formulaire</b>
</div>
<div>
<div class=\"bloc_depliable blocdeplie\" id=\"t75ea3bc1\" style=\"display: block; overflow: visible;\">
<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\">
<tbody>";
	if ($resultats = sql_select(
							'*', 
							array('spip_articles AS articles','spip_forms_articles AS lien'),
							array('lien.id_article=articles.id_article',"id_form="._q($id_form)."", "statut!='poubelle'"),
							'',
							array("titre")
					)
		) 
	{
		...
		while ($res = sql_fetch($resultats)) 
		{
			// utiliser les resultats
			 // $res['colonne']
			 // print_r($res);
			 // $liste .=var_export($res,true);
			$liste .= "
<tr class=\"tr_liste\">
	<td class=\"verdana12\">
		<div><a dir=\"ltr\" href=\"?exec=article&id_article=".$res['id_article']."\">#".$res['id_article']." - ".$res['titre']."</a>
		</div>
	</td>
	</td>
</tr>";
...
function exec_forms_edit(){
	...
	if ($retour) {
		...
		$contexteBT = array(
			'style'=>'horizontale ',
			'titre'=>_T('icone_retour'),
			'url'=>$retour,
			'icon_base'=>'form-24.png',
			'icon_action'=>'rien.gif'
		);
		echo recuperer_fond("fonds/bouton_url",$contexteBT);
	}
	if (autoriser('administrer','form',$id_form)) {
		$nretour = urlencode(self());
		// echo icone_horizontale(_T("forms:suivi_reponses")."<br />".(($nb_reponses==0)?_T("forms:aucune_reponse"):(($nb_reponses==1)?_T("forms:une_reponse"):_T("forms:nombre_reponses",array('nombre'=>$nb_reponses)))),generer_url_ecrire('forms_reponses',"id_form=$id_form"), _DIR_PLUGIN_FORMS."img_pack/donnees-24.png", "rien.gif",false);
		$contexteBT = array(
			'style'=>'horizontale ',
			'titre'=>_T("forms:suivi_reponses")."\n\r".(($nb_reponses==0)?_T("forms:aucune_reponse"):(($nb_reponses==1)?_T("forms:une_reponse"):_T("forms:nombre_reponses",array('nombre'=>$nb_reponses)))),
			'url'=>generer_url_ecrire('forms_reponses',"id_form=$id_form"),
			'icon_base'=>'donnees-24.png',
			'icon_action'=>'rien.gif'
		);
		echo recuperer_fond("fonds/bouton_url",$contexteBT);
		
		// echo icone_horizontale(_T("forms:Tableau_des_reponses"),generer_url_ecrire('donnees_tous',"id_form=$id_form&retour=$nretour"), _DIR_PLUGIN_FORMS."img_pack/donnees-24.png", "rien.gif",false);
		$contexteBT = array(
			'style'=>'horizontale ',
			'titre'=>_T("forms:Tableau_des_reponses"),
			'url'=>generer_url_ecrire('donnees_tous',"id_form=$id_form&retour=$nretour"),
			'icon_base'=>'donnees-24.png',
			'icon_action'=>'rien.gif'
		);
		echo recuperer_fond("fonds/bouton_url",$contexteBT);
		...
		if ($nb_reponses){
			// echo icone_horizontale(_T("forms:telecharger_reponses"),generer_url_ecrire('forms_telecharger',"id_form=$id_form&retour=$nretour"), _DIR_PLUGIN_FORMS."img_pack/donnees-exporter-24.png", "rien.gif",false);
			$contexteBT = array(
				'style'=>'horizontale ',
				'titre'=>_T("forms:telecharger_reponses"),
				'url'=>generer_url_ecrire('forms_telecharger',"id_form=$id_form&retour=$nretour"),
				'icon_base'=>'donnees-exporter-24.png',
				'icon_action'=>'rien.gif'
			);
			echo recuperer_fond("fonds/bouton_url",$contexteBT);
		}
		...
		$contexteBT = array(
			'style'=>'horizontale ',
			'titre'=>_T("$prefixei18n:supprimer_formulaire"),
			'url'=>$link,
			'icon_base'=>'supprimer-24.png',
			'icon_action'=>'rien.gif'
		);
		echo recuperer_fond("fonds/bouton_url",$contexteBT);
		/*echo "</p>";*/
