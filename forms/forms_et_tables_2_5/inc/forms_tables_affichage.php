...
function affichage_donnees_tous_corps($type_form,$id_form,$retour=false, $titre_page=false, $contexte = array()){
  ...
  if (!$icone)
	...
	if ($retour){
		...
		$retour=str_replace(".%2F%3Fexec%3D","?exec=",$retour);//--- sparadrap
		$contexteBT = array(
				'style'=>'verticale ',
	...
	if (autoriser('administrer','form',$id_form)) {
		...
		$contexteBT = array(
				...
				'url_destination'=>in_array($row['type_form'],array('','sondage'))?'forms_reponses':'donnees_tous',
				'url_parametres'=>"id_form=$id_form",
				'icon_base'=>'donnees-24.png',
				'icon_action'=>'rien.gif'
			);
		$out .=  recuperer_fond("fonds/bouton_cree",$contexteBT);
		$out .=  "</div>";
		...
		if ($row2 = spip_fetch_array($res2)){
			...
			// $out .=  icone(_T("$prefix:telecharger_reponses"),
				...
				// _DIR_PLUGIN_FORMS. "img_pack/donnees-exporter-24.png", "rien.gif","",false);
				
			$contexteBT = array(
				'style'=>'verticale ',
				'titre'=>_T("forms:telecharger_reponses"),
				'url_destination'=>'forms_telecharger',
				'url_parametres'=>"id_form=$id_form&retour=$retour",
		...
		if (defined('_DIR_PLUGIN_CSVIMPORT')){
			...
			$contexteBT = array(
				'style'=>'verticale ',
				'titre'=>_T("forms:importer_donnees_csv"),
				'url_destination'=>'csvimport_import',
				'url_parametres'=>"id_form=$id_form&retour=$retour",
				'icon_base'=>'donnees-importer-24.png',
				'icon_action'=>'rien.gif'
			);
			$out .=  recuperer_fond("fonds/bouton_cree",$contexteBT);
			$out .=  "</div>";
...
function affichage_donnee_edit($type_form){
  ...
  $statut = $row['statut'];
	...
	echo "<div class='verdana2'><form action=\"\" method=\"POST\">$formulaire</form></div>";
