...
function forms_reponses(){
  ...
  include_ecrire("inc_presentation");
	...
	if ($id_form) {
		...
		icone_horizontale(_L("Aller au formulaire"), "?plug=forms_edit&id_form=$id_form", _DIR_THIS_PLUGIN_IMG_PACK. "/form-24.png", "rien.gif");
	...
	while ($row = spip_fetch_array($result)) {
		...
		while ($row2 = spip_fetch_array($result2)) {
			...
			if ($types[$id_form][$champ] == 'fichier') {
				$valeurs[$champ][] = "<a href='?plug=forms_telecharger.php&id_reponse=$id_reponse&champ=$champ'>".
					$row2['valeur']."</a>";
			}
		...
		icone(_L("Supprimer cette r&eacute;ponse"), $link->getUrl(), _DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png", "supprimer.gif", "right");

		echo _L("R&eacute;ponse envoy&eacute;e le ").affdate($date)."<br />\n";
		if (!$form_unique) {
			//echo "<div style='padding: 2px; background-color: $couleur_claire;'>";
			echo _L("&agrave;")." ";
			echo "<strong class='verdana3' style='font-weight: bold;'><a href='?plug=forms_reponses&id_form=$id_form'>"
