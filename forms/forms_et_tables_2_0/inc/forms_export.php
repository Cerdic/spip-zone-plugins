...
function Forms_formater_ligne($ligne, $format, $separateur){
	...
	// SI une fonction correspondant au format existe ...
	...
	foreach ($structure as $champ => $t) {
		...
		if ( !isset($valeurs[$champ]) ) {
			...
			if ( $t['type']=='multiple' ){
				...
				// (??Pourquoi une colonne vide avant??)
				$ligne[$champ][] = "";
...
// INFO : $separateur = $delim 
// (?? Pourquoi '&$' dans "&$fichiers" et "&$filename" ??)
function Forms_formater_les_reponses($id_form, $format, $separateur, &$fichiers, &$filename, $head=true, $traduit=true){
	...
	if ($head) {
		...
		// SI 'Import_CSV', on ne veut que la 2ème ligne (ligne des titres (libellés) des champs)
		if( $format != "Import_CSV" ){
			$ligne1[] = 'id_donnee';
		}
		$ligne2[] = 'id_donnee';
		
		
		// SI 'Import_CSV', on ne veut pas ces 2 colonnes! (ni 'date', ni 'url')
		if( $format != "Import_CSV" ){
			$ligne1[] = 'date';
		...
		// SI 'Import_CSV', on ne veut que la 2ème ligne (ligne des titres (libellés) des champs)
		if( $format != "Import_CSV" ){
			$s .= Forms_formater_ligne($ligne1, $format, $separateur); // (1ère ligne)
		}
		
		if ($traduit)
			$s .= Forms_formater_ligne($ligne2, $format, $separateur); // (2ème ligne)
