...
function forms_edit(){
  ...
  include_ecrire("inc_presentation");
	...
	$form_link = new Link("?plug=forms_edit&id_form=$id_form&new=$new");
	...
	if ($id_form && form_editable($id_form)) {
		...
		// Ajout d'un champ
		if (($type = $ajout_champ) && types_champs_autorises($type)) {
			$n = 1;
	...
	if (form_administrable($id_form) && $nb_reponses) {
		debut_boite_info();

		icone_horizontale(_L("Suivi des r&eacute;ponses")."<br />".$nb_reponses." "._L("r&eacute;ponses"),
			"?plug=forms_reponses&id_form=$id_form", "forum-public-24.gif", "rien.gif");
		icone_horizontale(_L("T&eacute;l&eacute;charger les r&eacute;ponses"),
			"?plug=forms_telecharger&id_form=$id_form", "doc-24.gif", "rien.gif");
	...
	if ($id_form) {
		debut_cadre_relief(_DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png");
		...
		if (count($schema)) {
			...
			echo afficher_formulaire_schema($schema);
			echo "</div>\n";
			echo fin_block();
	...
	if ($retour) {
		...
		icone(_T('icone_retour'), $retour, _DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png", "rien.gif");
		echo "</div>\n";
	}
	if ($id_form && form_administrable($id_form)) {
		echo "<br />\n";
		echo "<div align='$spip_lang_right'>";
		$link = new link();
		...
		icone(_L("Supprimer ce formulaire"), $link->getUrl(), _DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png", "supprimer.gif");
		echo "</div>\n";
	}
	...
	if (form_editable($id_form)) {
		...
		if ($id_form) {
			...
			foreach ($schema as $index => $t) {
				...
				icone_horizontale(_L("Supprimer ce champ"), $link->getUrl('champs'), _DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png", "supprimer.gif");
				echo "</div>\n";
			...
			foreach ($types as $type) {
				echo "<option value='$type'>".nom_type_champ($type)."</option>\n";
			}
			echo "</select>\n";
