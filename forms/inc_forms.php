...
define_once('_DIR_THIS_PLUGIN',(_DIR_PLUGINS . basename(dirname(__FILE__))));
define_once('_DIR_THIS_PLUGIN_IMG_PACK',(_DIR_THIS_PLUGIN . '/img_pack'));
...
function afficher_insertion_formulaire($id_article) {
	...
	echo "\n<p>";
	debut_cadre_relief(_DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png", false);
	...
	if (form_editable()) {
		echo "\n<br />";
		$link = new Link("?plug=forms_edit&new=oui");
		$link->addVar('retour', $GLOBALS['clean_link']->getUrl());
		icone_horizontale(_L("Cr&eacute;er un nouveau formulaire"),
			$link->getUrl(), _DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png", "creer.gif");
	}
...
function afficher_champ_formulaire($t, $attributs = '', $erreur = false) {
	...
	$r .= "</strong>";
	if ($obligatoire) $r .= " "._T('info_obligatoire_02');
	$r .= " :<br />\n";
...
function generer_mail_reponse_formulaire($id_form, $id_reponse, $mailconfirm){
	...
	if ($row = spip_fetch_array($result)) {
		...
		if ($email != '') {
			...
			$fullurl .= _DIR_RESTREINT_ABS ."?plug=forms_reponses";
	...
	if (!$erreur) {
		...
		if ($ok) {
			...
			if ($row['sondage'] != 'non') {
				...
				$link = new Link('plug.php?plug=spip_sondage');
...
function afficher_forms($titre_table, $requete, $icone = '') {
	...
	if (!$icone) $icone = _DIR_THIS_PLUGIN_IMG_PACK . "/form-24.png";
