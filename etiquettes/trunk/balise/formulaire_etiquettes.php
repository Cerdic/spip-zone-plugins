...
function balise_FORMULAIRE_ETIQUETTES_stat($args, $filtres){
	...
	$params_ok = array('groupe', 'name', 'aide', 'proposer_login', 'remplacer', 'objet', 'uniquement_champ', 'explication', 'nuage', 'label');
	...
	if (
		$type_objet
		AND $desc = $trouver_table(table_objet_sql($type_objet))
		AND !in_array('mots',$desc['tables_jointures'])
	)
		return erreur_squelette(
