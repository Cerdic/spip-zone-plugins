...
function svp_actualiser_stats() {
	...
	foreach (array_keys($GLOBALS['infos_branches_spip']) as $_branche) {
		...
		if (!$stats = $infos->plugins) {
			...
			foreach ($stats as $_stat) {
				$prefixe = strtoupper($_stat->nom);
				$where = array('prefixe='. sql_quote($prefixe), 'branche=' . sql_quote($_branche));
				// Suivant que l'enregistrement du plugin pour la branche donnée existe ou pas, on met à jour ou on insère les statistiques.
				$historique = array();
				if ($historique_existant = getfetsel('historique', 'spip_plugins_stats', $where)) {
