...
function spip_proprio_exporter(){
	...
	if (empty($ok_export) || $ok_export!='oui') return false;
...
function spip_proprio_importer(){
	...
	if (empty($ok_import) || $ok_import!='oui') return false;
...
//	    var_export($php); exit;
	...
	if( $ok ) return _T('spipproprio:ok_import');
	else return _T('spipproprio:erreur_import');
}
