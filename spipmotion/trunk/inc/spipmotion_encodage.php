...
function inc_spipmotion_encodage_dist($id_document, $options = array()) {
	...
	if (($retour_int == 1) and (count($retour) >= 3)) {
	...
	if ($id_document = sql_getfetsel(
		...
		'lien.id_objet='.intval($source['id_document']).'
			AND lien.objet='.sql_quote('document').'
			AND document.extension='.sql_quote($format).'
			AND document.mode='.sql_quote('conversion')
	)) {
		 ...
		 */
		$liste[] = $v['id_document'];
		if (($nb = sql_countsel('spip_documents', 'fichier='.sql_quote($v['fichier'])) == '1')
			and @file_exists($f = get_spip_doc($v['fichier']))) {
			supprimer_fichier($f);
		}
		...
		if ($v['id_vignette'] > 0) {
			...
			if (($nb = sql_countsel('spip_documents', 'fichier='.sql_quote($fichier)) == '1')
				and @file_exists($f = get_spip_doc($fichier))) {
				supprimer_fichier($f);
			}
		}
		...
		if (is_array($liste)) {
			...
			sql_delete(
				'spip_facd_conversions',
				'id_document='.intval($id_document).'
					AND statut != '.sql_quote('oui').'
					AND extension='.sql_quote($format).'
					AND id_facd_conversion!='.intval($options['id_facd_conversion'])
			);
		}
	}
	/**
...
function encodage($source, $options) {
	...
	sous_repertoire(_DIR_VAR, 'cache-spipmotion');
	...
	if ($source['hasaudio'] == 'oui') {
		...
		if (in_array($codec_audio, array('libvorbis'))) {
			 ...
			 */
			if (intval($source['audiobitrate'])
				and (intval($source['audiobitrate']) < (lire_config("spipmotion/bitrate_audio_$extension_attente", '64')*1000))) {
		 ...
		 */
		if (intval($source['audiosamplerate'])
			and (intval($source['audiosamplerate']) < lire_config("spipmotion/frequence_audio_$extension_attente", '22050'))) {
			/**
		 ...
		 */
		if (in_array($extension_attente, array('ogg','ogv','oga')) and ($source['audiochannels'] < 2)) {
			$audiochannels = 2;
		} else {
	...
	if (in_array($source['extension'], lire_config('spipmotion/fichiers_videos_encodage', array()))) {
		 ...
		 */
		if ($format == 'ipod' and ($width_finale > 640)) {
			$width_finale = 640;
		}
		 ...
		 */
		$texte .= lire_config("spipmotion/vcodec_$extension_attente") ?
			'vcodec='.lire_config("spipmotion/vcodec_$extension_attente")."\n" : '';
		$vcodec .= lire_config("spipmotion/vcodec_$extension_attente") ?
			'--vcodec '.lire_config("spipmotion/vcodec_$extension_attente") : '';

		$fps_conf = (intval(lire_config("spipmotion/fps_$extension_attente", '30')) > 0) ?
			lire_config("spipmotion/fps_$extension_attente", '30') :
			((intval($source['framerate']) > 0) ? intval($source['framerate']) : 24);
		if (intval($source['framerate']) and (intval($source['framerate']) < $fps_conf)) {
		 ...
		 */
		if (intval($source['videobitrate'])
			and (intval($source['videobitrate']) < (lire_config("spipmotion/bitrate_$extension_attente", '600'))*1000)) {
		...
		if (($passes == '2') and ((($vcodec == '--vcodec libx264') and ($preset_quality != 'hq'))
			or ($vcodec == '--vcodec flv')
			or ($vcodec == '--vcodec libtheora')
			or ($extension_attente == 'webm'))) {
	...
	if ($ret['success'] and file_exists(get_spip_doc($source['fichier']))) {
		/**
	...
	} elseif (!file_exists(get_spip_doc($source['fichier']))) {
