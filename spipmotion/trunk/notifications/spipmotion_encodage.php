...
function notifications_spipmotion_encodage_dist($quoi, $id, $options) {
	...
	$en_cours = sql_countsel(
		'spip_facd_conversions',
		'id_document = '.intval($options['source']['id_document']).'
			AND statut IN ("non","en_cours","erreur")'
	);
	...
	if (lire_config('spipmotion/debug_mode') == 'on') {
		...
		$msg_mail = recuperer_fond(
			'notifications/spipmotion_encodage_webmestre',
			array(
				'id_facd_conversion' => $id,
				'fichier_log' => $options['fichier_log']
			)
		);
	...
	if ($en_cours == 0) {
		$msg_mail = recuperer_fond(
			'notifications/spipmotion_encodage_termine',
			array('id_facd_conversion'=>$id)
		);
