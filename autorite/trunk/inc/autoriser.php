...
if (
	isset($GLOBALS['spip_version_code']) and $GLOBALS['spip_version_code'] < '1.93'
...
if (
	...
	if (!function_exists('autoriser_article_modifier')) {
		function autoriser_article_modifier($faire, $type, $id, $qui, $opt) {
			...
			$s = spip_query("SELECT id_rubrique,id_secteur,statut FROM spip_articles WHERE id_article="._q($id));
			$r = sql_fetch($s);
	...
	if (!function_exists('autoriser_rubrique_publierdans')) {
		function autoriser_rubrique_publierdans($faire, $type, $id, $qui, $opt) {
			...
			if (isset($GLOBALS['autorite']['espace_publieur'])) {
				...
				if (autorisation_publie_visiteur($qui, $r['id_secteur']) and ($qui['statut'])) {
					return true;
				}
			}
