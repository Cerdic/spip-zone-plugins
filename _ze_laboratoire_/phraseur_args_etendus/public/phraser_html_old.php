...
// https://code.spip.net/@phraser_arguments_inclure
...
// https://code.spip.net/@phraser_idiomes
...
// https://code.spip.net/@phraser_champs
...
function phraser_champs_exterieurs($texte, $ligne, $sep, $nested) {
	$res = array();
	$texte = phraser_champs_args_etendus($texte,$ligne,$sep,$nested);
...
function phraser_champs_args_etendus($texte,$ligne,$sep,&$result) {
	$i = count($result);
	while (preg_match('/'.NOM_DE_CHAMP.'(\{[^{}]*%'.preg_quote($sep).'([0-9]+)@[^{}]*\})/',$texte,$m)) {
		$champ = new Champ;
		$champ->ligne = $ligne;
		$ligne += substr_count($m[0], "\n");
		$champ->nom_boucle = $m[2];
		$champ->nom_champ = $m[3];
		$champ->etoile = $m[5];

	  $result = phraser_args($m[6], '}',$sep, $result, $champ);
	  $result[$i] = $champ;
		
		$texte = str_replace($m[0],"%{$sep}$i@",$texte);
		$i++;
	}
	return $texte;
}
...
// https://code.spip.net/@phraser_criteres
...
// https://code.spip.net/@public_phraser_html
