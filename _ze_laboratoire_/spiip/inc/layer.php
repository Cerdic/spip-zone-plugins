...
function verif_butineur() {
	...
	if (!eregi("opera", $browser_description)&&eregi("opera", $browser_name)) {
		$browser_name = "Opera";
		$browser_version = $match[2];
		$browser_layer = (($browser_version < 7) ? '' :  http_script('', find_in_path(_DIR_IMG_PACK.'layer.js')));
		...
		$browser_name = "MSIE";
		$browser_version = $match[1];
		$browser_layer = (($browser_version < 5) ? '' :  http_script('', find_in_path(_DIR_IMG_PACK.'layer.js')));
		...
		$browser_name = "Safari";
		$browser_version = $match[1];
		$browser_layer = http_script('', find_in_path(_DIR_IMG_PACK.'layer.js'));
