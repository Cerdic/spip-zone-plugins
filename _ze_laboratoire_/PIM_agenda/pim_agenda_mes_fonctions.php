...
function PIMAgenda_affiche_evenements($texte){
	...
	if ($flag_editable)
		PIMAgenda_ajoute_creneaux_horaires($urlbase,$ts_start,$ts_fin,$type,$partie_cal,$echelle);
	...
	if($id_auteur){
		...
		$res = spip_query("SELECT * 
					 ...
					 LEFT JOIN spip_pim_agenda_auteurs ON spip_pim_agenda.id_agenda=spip_pim_agenda_auteurs.id_agenda 
							 WHERE spip_pim_agenda_auteurs.id_auteur=$auteur_session
		...
		$res = spip_query("SELECT * 
								FROM spip_pim_agenda AS agenda
					 LEFT JOIN spip_pim_agenda_invites ON spip_pim_agenda.id_agenda=spip_pim_agenda_invites.id_agenda 
							 WHERE spip_pim_agenda_invites.id_auteur=$auteur_session
							 	 AND ((agenda.date_debut>='$datestart' AND agenda.date_debut<='$datefin')
							 				OR (agenda.date_fin>='$datestart' AND agenda.date_fin<='$datefin')
							 				OR (agenda.date_debut<'$datestart' AND agenda.date_fin>'$datefin'))
							 ORDER BY agenda.date_debut;");
