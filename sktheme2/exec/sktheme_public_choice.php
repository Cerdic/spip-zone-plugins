...
include_spip('inc/xml');         // xml function
...
function exec_sktheme_public_choice() {
  ...
  $habillage_public_name = _request('habillage_public_name');
	$squelette_public_name = _request('squelette_public_name');
...
// fin_cadre(’gris_clair’);
  ...
  if (is_dir($s_dir)) {
	...
	if ( (is_dir($s_dir."/".$dir)) AND ($dir[0]!=".") ) {
	  ...
	  if (is_file($s_dir."/".$dir."/theme.xml")) {
		...
		$type = (isset($s_info['type'])) ? $s_info['type'] : "";
	    if ($type == 'squelettes') {
  ...
  foreach( $squelettes_list as $key => $value) {
	...
	echo bouton_block_depliable($key,'',"$key");
    //echo "<strong>$key</strong>";
    //echo debut_block_invisible("$key");
	echo debut_block_depliable('',"$key");
  ...
  if (is_dir($h_dir)) {
	...
	if ( (is_dir($h_dir."/".$dir)) AND ($dir[0]!=".") ) {
	  ...
	  if (is_file($h_dir."/".$dir."/theme.xml")) {
		...
		$type = (isset($h_info['type'])) ? $h_info['type'] : "";
	    if ($type == 'themes') { 
