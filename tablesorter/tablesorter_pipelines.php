...
function tablesorter_insert_head_css($flux){
	...
	if (!$done) {
		$done = true;
		$flux .= '<link rel="stylesheet" href="'.url_absolue(find_in_path('css/tablesorter.css')).'" type="text/css" />';
	}
...
function tablesorter_insert_head($flux){
	...
	$flux .='<script src="'.url_absolue(find_in_path('javascript/jquery.tablesorter.js')).'" type="text/javascript"></script>';
	// Init de tablesorter
	$flux .='
