...
function polyhier_affiche_enfants($flux) {
	if (isset($flux['args']['id_rubrique']) and $id_rubrique = $flux['args']['id_rubrique']) {
		...
		$flux['data'] .= recuperer_fond("prive/squelettes/inclure/rubrique-enfants-indirects",$_GET, array('ajax'=>true));
	}
	return $flux;
}
...
function polyhier_formulaire_charger($flux){
	$form = $flux['args']['form'];
	if (
		(isset($flux['data']['_polyhier']) AND $objet = $flux['data']['_polyhier'] AND in_array($objet,array('article','rubrique')))
		OR (is_array($flux['data']) and $objet = substr($form,7) AND in_array($form,array('editer_article','editer_rubrique')))
		){
