...
/**
 * Adaptation du plugin openPublishing à un mode de fonctionnement moins merdique
 * qu'Indy pour l'agenda (pas d'usage des brèves),
 * et à la possibilité d'utiliser un style "non-Indy".
 */
...
if($valider) {
  ...
  // Calcul de la date complète
  $tableau = split('[:]', $heure);
  $heure = $tableau[0];
  $minute = $tableau[1];

  $time = mktime($heure, $minute, 0, $mois, $jour, $annee);
  // Remplacement de la date de publication par la date de l'évènement annoncé si c'est futur
  if ($time < time())
    $date_complete = 'NOW()';
  else
    $date_complete = date('Y-m-d H:i:s', $time);

	// Sauvegarde en bdd
  if ($flag_ok== 'ok') { // soit il s'agit d'un article, soit d'une breve. Les deux à la fois ne sont pas possible
		...
		$retour = spip_query('UPDATE spip_articles SET titre = ' . spip_abstract_quote($titre) .
				...
				',	date = '. spip_abstract_quote($date_complete) .
        ',  date_modif = '. spip_abstract_quote($date_complete) .
				',	date_redac = NOW()' .
