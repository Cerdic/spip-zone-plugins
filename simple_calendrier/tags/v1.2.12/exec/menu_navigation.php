<?php
//--------------------------------------------------------------------------
// Fichier de la dist personnalisé
// Modif apportée :
// - 'exec_menu_navigation_dist' renommé en 'exec_menu_navigation' 
// - ajout de la liste des évènements proposés à l'évaluation (cf. plus bas)
//--------------------------------------------------------------------------
...
// https://code.spip.net/@exec_menu_navigation_dist
function exec_menu_navigation_dist() {
    ...
    //------------------------------------------------------
    // ici : customisation par le plugin Simple-calendrier
    //------------------------------------------------------
    include_spip('inc/simplecal_utils');
    $where_rub_exclure = simplecal_get_where_rubrique_exclure(); // Acces restreint

    $vos_evenements = sql_select("id_evenement,titre,statut", "spip_evenements", "statut='prop'".$where_rub_exclure, "date", "date DESC", "5");
	$vos = '';
	while($row = sql_fetch($vos_evenements)) {
		$id_evenement = $row['id_evenement'];
		$titre = typo(sinon($row['titre'], _T('ecrire:info_sans_titre')));
		$statut = $row['statut'];
		$vos .= "<a class='$statut spip_xx-small' href='" . generer_url_ecrire("evenement_voir","id_evenement=$id_evenement") . "'>$titre</a>";
	}
	if ($vos) {
		$gadget .= debut_cadre('bandeau-rubriques', _DIR_SIMPLECAL_IMG_PACK."simplecal-logo-24.png",'',afficher_plus(generer_url_ecrire("evenement_tous"))._T('simplecal:liste_evenements_a_valider'))
		. "<div class='plan-articles'>"
		. $vos
		. "</div>"
		. fin_cadre('bandeau-rubriques');
