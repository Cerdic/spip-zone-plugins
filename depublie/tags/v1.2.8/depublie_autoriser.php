...
function autoriser_depublierafficher($faire, $type, $id, $qui, $opt) {
	...
	if (lire_config('depublie/rubrique_depublie') != '' || lire_config('depublie/secteur_depublie') != '') {
		...
		if (in_array($infos_rubrique['id_rubrique'], explode(',', lire_config('depublie/rubrique_depublie'))) 
			|| in_array($infos_rubrique['id_secteur'], explode(',', lire_config('depublie/secteur_depublie')))) {
