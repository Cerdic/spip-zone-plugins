...
idf_escape($t);}function
connect(){global$b;$h=new
Min_DB;$Ab=$b->credentials();if($h->connect($Ab[0],$Ab[1],$Ab[2])){$h->query("SET sql_quote_show_create = 1, autocommit = 1");return$h;}$J=$h->error;if(function_exists('iconv')&&!is__utf8($J)&&strlen($eg=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$eg;return$J;}function
...
selectVal($X,$_,$n,$We){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$n["type"])&&!preg_match("~var~",$n["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$n["type"])&&!is__utf8($X))$J=lang(34,strlen($We));return($_?"<a href='".h($_)."'>$J</a>":$J);}function
