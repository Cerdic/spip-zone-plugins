...
function exec_admin_tech()
{
	...
	if(_request('mes_fichiers') == 'sauve_ok') {
		echo debut_cadre_trait_couleur('',true,'',_L('Plugin "Mes fichiers": résultat de la sauvegarde'),'sauvegarder') .
			_L('le fichier zip est cr&eacute;e, tout s\'est bien pass&eacute;');
		echo ' => <a href="../?action=telecharger_mes_fichiers">'._L('Télécharger le fichier zip').'</a>';
		echo fin_cadre_trait_couleur();
	}
	else {
		echo debut_cadre_trait_couleur('',true,'',_L('Plugin "Mes fichiers"'),'sauvegarder');
		if (count(preg_files(_DIR_TMP . 'mes_fichiers_')) > 0) {
			echo '<a href="../?action=telecharger_mes_fichiers">'._L('Télécharger le dernier zip sauvegardé').'</a>';
			echo fin_cadre_trait_couleur();
		}
	}
...
// https://code.spip.net/@generer_form_ecrire
