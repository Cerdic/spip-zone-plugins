...
function action_export_all_dist()
{
	...
	if ($quoi =='start'){
		...
		ecrire_meta("status_dump", "$gz::$archive::1::0::$zip",'non');
		ecrire_metas();
	...
	// qu'annonce' si certains etaient vides
			...
			$corps = "<p style='text-align: $spip_lang_left'>".
			  _T('mesfichiers:info_sauvegarde_reussi_02',
			     array('archive' => ':<br /><b>'.joli_repertoire($file)."</b> ($n)")) .
			  " <a href='" .  ($zip ? generer_url_action("mes_fichiers") : generer_url_ecrire()) . "'>".
			_T('mesfichiers:info_sauvegarde_reussi_03')
			. "</a> "
			._T('mesfichiers:info_sauvegarde_reussi_04', array('fichier' => joli_repertoire($file.$zip)))
			. "</p>\n";
