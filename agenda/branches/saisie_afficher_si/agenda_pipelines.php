...
/**
 * Pour la saisie de type événement, indique le type de test js que l'on doit faire
 * @param $flux
 * @return $flux
**/
function agenda_saisies_afficher_si_js_type($flux) {
	$saisie = $flux['args']['saisie'];
	if ($saisie['saisie'] != 'evenements') {
		return $flux;
	}
	$flux['data'] = isset($saisie['options']['type_choix']) ? $saisie['options']['type_choix'] : 'radio';
	return $flux;
}
