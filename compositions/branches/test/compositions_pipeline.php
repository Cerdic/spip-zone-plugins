...
/**
 * Pipeline pre_insertion pour appliquer la composition par defaut a un nouvel article
 *
 * @param array $flux
 * @return array
 */

function compositions_pre_insertion($flux){
	if ($flux['args']['table']=='spip_articles'){
			// on recupere la composition par defaut dans la rubrique de l'article
			$type = 'rubrique';
			$table = table_objet($type);
			$table_sql = table_objet_sql($type);
			$_id_table = id_table_objet($type);
			...
			if (isset($desc['field']['composition_articles'])
				AND $composition_articles = sql_getfetsel('composition_articles',$table_sql,"$_id_table=".intval($id_rubrique))){
				$flux['data']['composition']=$composition_articles;
			} 
	}
