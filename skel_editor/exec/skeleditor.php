...
function editor_addfile() {
  ...
  $output .= "<img src='../plugins/skel_editor/img_pack/action_add.png' alt='new' />Nouveau fichier";
  ...
  $output .= "</select><br /><input type='submit' name='sub' value='Cr&eacute;er' />";
  $output .= "</form>\n";
  $output .= fin_block();
  return $output;        
}
...
;
	...
	if (isset($_POST['editor'])) {      // save file ?
	     ...
	     if (is_writable($file_name)) {
             if (!$handle = fopen($file_name, 'w')) {
                 ...
                 $log = "<span style='color:green'>fichier sauvegardé @ ".date('H:i')."</span>";
                 fclose($handle);
             }  
          ...
          if ($action=="delete") {                // delete the file
            ...
            if (isset($_GET['target'])) {
               ...
               if (substr($target,3, strlen($dossier_squelettes))== $dossier_squelettes) { // security 2: right place ?                  
                   ...
                   if (is_file($file_name)) { // security 3: overight ?
                    $log = "<span style='color:red'>erreur: ce fichier existe d&eacute;j&agrave;</span>";
                   } else {
                     if (!$handle = fopen($file_name, 'w')) {
                         $log = "<span style='color:red'>erreur: impossible de cr&eacute;er le fichier. V&eacute;rifier les droits en &eacute;critutre de votre r&eacute;pertoire squelette</span>";
                     } else if (fwrite($handle, "...") === FALSE) {
                         $log = "<span style='color:red'>erreur: impossible de cr&eacute;er le fichier. V&eacute;rifier les droits en &eacute;critutre de votre r&eacute;pertoire squelette</span>";           
                     } else {
                         $log = "<span style='color:green'>fichier cr&eacute;e @ ".date('H:i')."</span>";
                         fclose($handle);
                     }
                    }
               }               	
  ...
  fin_boite_info();
	...
	if ($file_name!="") {   
       ...
       if ($action=="delete") {
         echo "<p style='color:green'>Fichier effacé avec succès!</p>\n";
       } else { 
           // tools bar
           echo "<div id='skel_toolbar' style='width:100%;text-align:right;'>\n";
           echo "<img src='../plugins/skel_editor/img_pack/action_dl.png' alt='download' /><a href=\"?exec=skeleditor&amp;f=".urlencode($file_name)."&amp;action=download\">télécharger</a>";
           echo "<img src='../plugins/skel_editor/img_pack/action_del.png' alt='delete' /><a href=\"?exec=skeleditor&amp;f=".urlencode($file_name)."&amp;action=delete\" onclick=\"javascript:return confirm('Voulez vous vraiment effacer ce fichier ?');\">effacer</a>";
           echo "</div>\n";
           // img or text ?
