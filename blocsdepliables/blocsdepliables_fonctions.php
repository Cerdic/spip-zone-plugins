...
function blocdepliable_balise_titre() {
	static $h=null;
	if (is_null($h)) {
		include_spip('inc/config');
		$h = lire_config('blocsdepliables/balise_titre', 'h4');
	}
	return $h;
}
...
function balise_BLOC_VISIBLE_TITRE_NUM(&$p) {
	return balise_BLOC_VISIBLE_TITRE($p);
}
...
function bloc_deplier_script($num = 0) {
	...
	return $num<0?'':http_script("jQuery(document).ready(function() { jQuery('{$h}.blocs_titre').eq($num).click(); });");
}
...
function blocs_balises($type, $args = array()) {
	...
	$id = $numero = '';
	switch ($type) {
		case 'titre_debut':// id pour le bloc distant
			$id = ' id="cs_bloc_id_' . array_shift($args) . '"';
			$distant = 10;
			break;
		case 'titre':
			foreach ($args as $a) {
				if (is_numeric($a = trim($a))) {
					$numero = ' cs_bloc'.$a.'" id="deplier_num'.$a;
				} elseif ($a=='visible') {
					$replie = -1;
				} elseif ($a=='invisible') {
					$replie = 1;
		...
		case 'resume':
			...
			$h = blocdepliable_balise_titre();
			return "</a></$h><div class=\"blocs_resume$class\">";
		case 'debut':
			if (count($args)) {
			...
			if ($stade == 3) {
				// on arrive du resume, fermer la div resume seulement
				...
				$h = blocdepliable_balise_titre();
				return "</a></$h><div class=\"blocs_destination$class\">";
			}
			break;
		case 'titre_fin':
			...
			$h = blocdepliable_balise_titre();
			if ($k==10 || $k==20) {
				// on DOIT arriver de #BLOC_TITRE_DEBUT
				return "</a></$h></div>";
			}
		...
		case 'fin':
			...
			switch ($k) {
				...
				case 11:
					return '</div>';
				default:
					die("Erreur : #BLOC_FIN sans #BLOC_DEBUT ($k)");
			}
	}
}
