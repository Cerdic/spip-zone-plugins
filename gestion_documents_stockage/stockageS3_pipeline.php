...
function stockageS3_document_desc_actions($flux){
	if ($id_document = intval($flux['args']['id_document'])
		AND stockageS3_actif()){
		$flux['data'] .= recuperer_fond('modeles/stockageS3_actions', array('id_document'=>$id_document));
	}
	return $flux;
}
...
function stockageS3_editer_document_actions($flux){
	if ($id_document = intval($flux['args']['id_document'])
		...
		AND $distant == 'non'){
		$flux['data'] .=
			"<input type='submit' class='submit' name='stockageS3_envoyer' value='".attribut_html(_T('stockageS3:envoyer_s3'))."' />";
	}
	return $flux;
}
