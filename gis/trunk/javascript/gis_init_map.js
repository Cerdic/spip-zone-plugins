...
gis_init_map = function(mapcfg) {
	...
	// Appeler l'�ventuelle fonction de callback et trigger "load"
	...
	// D�terminer la position initiale de la carte
	...
	// Fond de carte par d�faut (layer)
	...
	if (mapcfg['control_type'] && !mapcfg['no_control'] && mapcfg['affiche_layers'].length>1){
		...
		// ajouter l'objet du controle de layers � la carte pour permettre d'y acc�der depuis le callback
		map.layersControl = layers_control;
		// classe noajax sur le layer_control pour �viter l'ajout de hidden par SPIP
	...
	// Ajout des contr�les de la carte
	...
	map.setGeoJsonFeatureIcon = function (feature, layer) {
		// D�clarer l'icone du points, si d�fini
	...
	map.setGeoJsonFeaturePopup = function (feature, layer) {
		// D�clarer le contenu de la popup s'il y en a
	...
	/*
		Il y a pour le moment 2 fa�ons d'analyser le GeoJson calcul�
		en fonction de si on veut faire du clustering (regrouper les points proches)
		ou non. Il y a certainement moyen de regrouper en un seul �l�ment
		la plupart du code, en se passant du js L.geoJson m�me hors clustering.
		� r�fl�chir.
	*/
	// API parseGeoJson
	if (!mapcfg['cluster']){
		// Analyse des points et d�claration (sans regroupement des points en cluster)
		map.parseGeoJson = function(data) {
			if (data.features.length > 0) {
				var geojson = new L.geoJson('', {
					style: mapcfg['path_styles'],
					onEachFeature: function (feature, layer) {
						// D�clarer l'icone du point
		...
		// Analyse des points et d�claration (en regroupant les points en cluster)
		map.parseGeoJson = function(data) {
			var options = {
			...
			/* Pour chaque points pr�sents, on cr�e un marqueur */
	...
	if (mapcfg['affiche_points']
		...
		// R�cup�ration des points � mettre sur la carte, via json externe
		var args = {};
		...
		jQuery.getJSON(mapcfg['json_points']['url'],args,
			function(data) {
				if (data){
					// Charger le json (data) et d�clarer les points
