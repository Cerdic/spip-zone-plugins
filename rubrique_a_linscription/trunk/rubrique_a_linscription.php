...
function rubrique_a_linscription_formulaire_charger($flux){
	...
	if (
	    ...
	    and (!$explicite) 
	    or ($explicite == 'on' and $flux['args']['form']=='inscription_avec_rubrique')
	   ){
...
function rubrique_a_linscription_formulaire_verifier($flux){
	...
	if ($flux['args']['form']=='editer_article' and $prive_creer=='on') {
...
function rubrique_a_linscription_formulaire_traiter($flux){
	...
	if (
		...
		if($groupe_mots){
			...
			if ($type){
				$id_mot = sql_insertq('spip_mots',array('id_groupe'=>$groupe_mots,'type'=>$type,'titre'=>_T('rubrique_a_linscription:mot_clef_de',array('nom'=>$nom_inscription))));
		...
		if ($mail_public or $mail_prive){
			...
			$envoyer_mail($mail,'['.extraire_multi(lire_config('nom_site')).']'. _T('rubrique_a_linscription:titre_mail_adresse_rubrique'),$corps);
		}
