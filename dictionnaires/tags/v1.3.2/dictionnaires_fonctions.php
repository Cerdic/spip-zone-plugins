...
function dictionnaires_replace_callback($captures){
	...
	if ((isset($definitions[$captures[2]]) AND $definition = $definitions[$captures[2]])
		or (isset($definitions[strtolower($captures[2])]) AND ($definition = $definitions[strtolower($captures[2])]))){
		...
		if ($remplacer_celui_la){
			if (function_exists("dictionnaires_remplacer_$type")){
				$remplacer = "dictionnaires_remplacer_$type";
			} elseif (function_exists("dictionnaires_remplacer_${type}_dist")) {
				$remplacer = "dictionnaires_remplacer_${type}_dist";
			} elseif (function_exists("dictionnaires_remplacer_defaut")) {
				$remplacer = "dictionnaires_remplacer_defaut";
			} else {
				$remplacer = "dictionnaires_remplacer_defaut_dist";
			}
