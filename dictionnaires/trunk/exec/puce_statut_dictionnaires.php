...
/**
 * Gestion des puces d'action rapide des dictionnaires
 *
...
/**
 * Gestion de l'affichage ajax des puces d'action rapide des dictionnaires
 *
...
**/
function exec_puce_statut_dictionnaires_dist()
{
	exec_puce_statut_dictionnaires_args(_request('id'),  _request('type'));
}
 ...
 * @return string Code HTML
**/
function exec_puce_statut_dictionnaires_args($id, $type)
{
		...
		AND $d['statut_textes_instituer'])
	{
		...
		$id = intval($id);
		$r = sql_fetsel("statut", $table_objet_sql, "$prim=$id");
		$statut = $r['statut'];
		$id_parent = 0;
	}
	else {
