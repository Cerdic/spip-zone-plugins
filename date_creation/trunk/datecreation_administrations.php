 ...
 * @return array
 */
function datecreation_declarer_tables_principales($tables_principales) {
 ...
 * @return void
 **/
function datecreation_upgrade($nom_meta_base_version, $version_cible) {
 ...
 */
function datecreation_creer_champs_date_creation() {
	...
	ecrire_config('datecreation/objets', serialize($tables));
 ...
 * @return void
 **/
function datecreation_vider_tables($nom_meta_base_version) {
	include_spip('base/abstract_sql');
	$tables = unserialize(lire_config('datecreation/objets'));
	if (is_array($tables)) {
		foreach ($tables as $table) {
