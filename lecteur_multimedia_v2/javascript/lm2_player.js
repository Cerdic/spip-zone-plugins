...
function video_play(i){
	track_index = i ;
	live_video = i ;
	jQuery('#sm2-container .logo_doc').fadeOut('slow');
	if (!isPaused && !isPlaying) {
		soundManager.createVideo({
			id: getFlashObject(), // required
			url: flvArray[i], // required
			// optional sound parameters here, see Sound Properties for full list
			volume: 50,
			//onmetadata: handleMetaData, // special video-only event handler (width/height info, etc.)
			//whileloading: videoIsLoading // remember to omit comma on the last item
			//autoPlay: true
			onplay:function(){
				jQuery("#loading").css("cursor","hand");
				var minutes = Math.floor(this.durationEstimate / 1000 / 60) ;
			...
			whileplaying : function(){
				...
				jQuery("#vposition").css({width:Math.round(timer2) +"%"});
				jQuery(".position").html(minutes + "'" + secondes +"''");
				
				isPlaying = (this.playState == "1");
				if(this.paused){
					isPlaying = false;
					isPaused = true;
				}else{
	...
	isPlaying = true;
	if(isPaused){
		soundManager.resume(getFlashObject());
	}else{
		soundManager.play(getFlashObject());
	}
...
function video_pause(){
	...
	soundManager.togglePause(getFlashObject());
	update_play_button();
}

function video_stop(){	
	if(soundManager.getSoundById(getFlashObject())){
		soundManager.destroyVideo(getFlashObject());
		isPlaying = false;
		isPaused = false;
		jQuery(".playliste li.play_on").removeClass("play_on");
		jQuery('#sm2-container .logo_doc').fadeIn('slow');
	}
	update_play_button();
}
