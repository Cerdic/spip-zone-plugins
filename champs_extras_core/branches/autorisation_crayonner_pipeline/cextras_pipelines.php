...
/**
 * Autoriser de crayonner un champ extra, on modifie les autorisations standards
 * @param array $flux
 * @return array $flux_modifié
 **/
function cextras_crayons_autoriser_crayonner($flux) {
	$type = $flux['args']['type'];
	$id = $flux['args']['id'];
	$qui = $flux['args']['qui'];
	$opt = $flux['args']['opt'];
	...
	$saisies = champs_extras_objet($table);
	$saisies_sql = champs_extras_saisies_lister_avec_sql($saisies);
	if (isset($saisies_sql[$opt['modele']])) {
		$opt['saisie'] = $saisies_sql[$opt['modele']];
		$opt['champ'] = $opt['modele'];
		$opt['type'] = $type;
		$flux['data'] = autoriser('modifierextra', $type, $id, $qui, $opt);
	}
	return $flux;
}
