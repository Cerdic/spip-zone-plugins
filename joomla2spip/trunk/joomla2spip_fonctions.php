...
function joomla2spip_auteur_import($mon_auteur){
	if($mon_auteur['statut'] == "Super Administrator" || $mon_auteur['statut'] == "Administrator") $statut = "0minirezo" ;
        ...
        else $statut = "nouveau";  //sera à confirmer
...
// pour générer le prefix
function balise_JOOMLA($p, $joomla_prefix = '') {
  if (interprete_argument_balise(2, $p))   $joomla_prefix  = interprete_argument_balise(2, $p);
  if ($joomla_prefix = '')   $joomla_prefix  = lire_config('joomla/prefix');
  $p->code = trim($joomla_prefix."_". interprete_argument_balise(1, $p) ); // $p->param[0];
  return $p;
 }
