...
/**
 * function qui permet d'afficher une image svg inline
 ...
 *
 **/
function inline_svg($url_relative, $id = '', $title = ''){
	...
	// on supprime l'id du svg
	$svg = preg_replace('/(<svg[^>]*?)( id=["\'].*?["\'])/s', '${1}', $svg);

	// on ajoute le role et aria-labelledby
	$replace .= 'role="img" aria-labelledby="'.$id.'"';
