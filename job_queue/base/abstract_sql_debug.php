...
function log_sql_update($table,$exp,$where){
	// loger tout ce que l'on fait sur les metas pour trouver qui desactive les plugins
	$log = date('Y-m-d H:i:s');
	$log .= ' (pid '.@getmypid().') ';
	$log .= "Update $table \n";
	$log .= 'exp='.var_export($exp,true)."\n";
	$log .= 'where='.var_export($where,true)."\n";
	$log .= 'count(_GLOBAL[meta][plugin]):'.count(unserialize($GLOBALS['meta']['plugin']))."\n";
	ob_start();
	debug_print_backtrace();
	$log .= ob_get_contents();
	ob_end_clean();
	$log .= "\n\n";
	ecrire_fichier(_DIR_TMP."sqlupdate.log", $log, true, false);
}
...
function sql_update($table, $exp, $where='', $desc=array(), $serveur='', $option=true)
{
	...
	if (!is_string($f) OR !$f) return false;
	if ($table=='spip_meta') log_sql_update($table,$exp,$where);
