...
/**
 * Retourne la valeur d'un champ pour une réponse donnée,
 * soit depuis la base, soit depuis _request()
 * @param str $champ
 * @param int|str $id_formulaires_reponse
 * @param string $source 'request'|'base'
 * @return valeur
 **/
function formidable_valeur_request_ou_base($champ, $id_formulaires_reponse = 0, $source = 'request') {
	$id_formulaires = sql_getfetsel('id_formulaire', 'spip_formulaires_reponses', "id_formulaires_reponse=$id_formulaires_reponse");
	if ($source == 'request' or !$id_formulaires_reponse or !$id_formulaire) {
		return _request($champ);
	} elseif ($source == 'base') {
		include_spip('formidable_fonctions');
		return calculer_voir_reponse($id_formulaires_reponse, $id_formulaire, $champ, '', 'brut', '');
	} else {
		return '';
	}
}
