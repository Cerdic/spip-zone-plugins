...
function formulaires_melusine_articles_charger(){
	$valeurs = array('d1'=>'menu','d2'=>'boutonsgauche','d3'=>'miseajour','d4'=>'mentionslegales','d5'=>'diaporama');
	melusine_liste_noisettes_dispo("articles");
	return $valeurs;
}

function formulaires_melusine_articles_verifier(){
	$erreurs = array();
...
function formulaires_melusine_articles_traiter(){
	$position=_request('position');
	if($position=='skels'){melusine_liste_noisettes_dispo("articles");return false;}
	$action=substr($position,0,1);
	$var=substr($position,1);
	
	if($action=="d" && $var<11 ){
		$varplus=$var+1;
		$chemin='melusine_articles/effectifs/'.$var;
		$chemin_bas='melusine_articles/effectifs/'.$varplus;
	...
	if($action=="m" && $var>1 ){
		$varmoins=$var-1;
		$chemin='melusine_articles/effectifs/'.$var;
		$chemin_haut='melusine_articles/effectifs/'.$varmoins;
		$pos=lire_config($chemin);
	...
	if($action=="s"){
		...
		melusine_rassembler($var,"articles");
	}
	if($action=="a"){
		$j=1;
		while($valeur!='aucun'){
		$chemin='melusine_articles/effectifs/'.$j;
	...
	if($action=="i"){
		...
		for ($i=1;$i<3;$i++){
			$j=$i-1;
			$chemin='melusine_articles/effectifs/'.$i;					
