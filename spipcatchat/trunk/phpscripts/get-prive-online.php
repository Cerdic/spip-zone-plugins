...
$url=$_SESSION['spipcatchatprivecode']; $nom=$_SESSION['catchatprivelogin']; //------Le nom de l'auteur de la session et le code du chat
if(false!=($tabfile=file_get_contents('../db_catchat/0471910101112141518996354/0471910101112141518996354line.js'))){$tableau=json_decode($tabfile,true);} $i=0; $timer= time();
onlineChat($_GET['auteur'].'_'.$nom,'0471910101112141518996354','record',$_SESSION['spipcatchatprivestatut'].$timer);/* si l'utilisateur n'est pas inscrit, on l'ajoute, et on modifie la date de sa derniere actualisation */
...
{	foreach($tableau as $key => $value)
	{	 if($timer-140<=chatdate($value))
		...
		$i++;	}	}	}
$count = onlineChat('','0471910101112141518996354','count','');// On compte le nombre d'entrées
