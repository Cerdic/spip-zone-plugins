...
function formulaires_clevermail_post_edit_charger_dist($pst_id = -1) {
	if ($pst_id == -1 || !$valeurs = sql_fetsel('*', 'spip_cm_posts', 'pst_id='.$pst_id)) {
		$valeurs = array(
			'pst_id' => -1,
			'pst_subject' => '',
			'pst_html' => '',
...
function formulaires_clevermail_post_edit_traiter_dist($pst_id = -1) {
  ...
  $lst_id = sql_getfetsel("lst_id", "spip_cm_posts", "pst_id="._request('pst_id'));
  $lst_name = sql_getfetsel("lst_name", "spip_cm_lists", "lst_id=".$lst_id);
  sql_updateq('spip_cm_posts', $champs, "pst_id = "._request('pst_id'));
