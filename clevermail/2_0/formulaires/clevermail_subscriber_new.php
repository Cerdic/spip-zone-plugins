...
function formulaires_clevermail_subscriber_new_traiter_dist() {
  ...
  $lsr_mode = intval(_request('cm_mode'));
    ...
    foreach($adresses as $adresse) {
    	if (!$sub_id = sql_getfetsel("sub_id", "spip_cm_subscribers", "sub_email='".$adresse."'")) {
    		$sub_id = sql_insertq("spip_cm_subscribers", array('sub_email' => $adresse));
    		sql_updateq("spip_cm_subscribers", array('sub_profile' => md5($sub_id.'#'.$adresse.'#'.time())), "sub_id=".$sub_id);
    	}
    	foreach(_request('cm_lists') as $lst_id) {
    		$lst_name = sql_getfetsel("lst_name", "spip_cm_lists", "lst_id=".$lst_id);
    		if (sql_countsel("spip_cm_lists_subscribers", "lst_id=".$lst_id." AND sub_id=".$sub_id) == 0) {
    			sql_insertq("spip_cm_lists_subscribers", array('lst_id' => $lst_id, 'sub_id' => $sub_id, 'lsr_mode' => $lsr_mode, 'lsr_id' => md5('subscribe#'.$lst_id.'#'.$sub_id.'#'.time())));
