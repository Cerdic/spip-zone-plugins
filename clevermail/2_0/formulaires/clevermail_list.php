<?php
function formulaires_clevermail_list_charger_dist($lst_id = -1) {
	if ($lst_id == -1 || !$valeurs = sql_fetsel('*', 'spip_cm_lists', 'lst_id='.$lst_id)) {
...
function formulaires_clevermail_list_verifier_dist($lst_id = -1) {
	$erreurs = array();
	...
	$nb = sql_countsel("spip_cm_lists", "lst_id != "._request('lst_id')." AND lst_name = '"._request('lst_name')."'");
  if ($nb > 0) {
...
function formulaires_clevermail_list_traiter_dist($lst_id = -1) {
  $champs = array(
  ...
  if (_request('lst_id') == -1) {
    ...
    spip_log('Nouvelle liste « '._request('lst_name').' »', 'clevermail');
  	...
  	sql_updateq('spip_cm_lists', $champs, "lst_id = "._request('lst_id'));
