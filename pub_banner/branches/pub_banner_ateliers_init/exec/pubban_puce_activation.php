...
function exec_pubban_puce_activation_emplacement_args($id) {
	...
	$div .= "<span class='puce_emplacement_fixe'>"
		...
		."<a href='".(generer_action_auteur("activer_emplacement", 'activer-'.$id, generer_url_ecrire('pubban_integer_tous','id_empl='.$id)))."&activer=oui' onclick='return confirm(confirm_changer_statut);' title='"._T('pubban:btn_active')."'>".$img_a.$end_img."</a>"
		."<a href='".(generer_action_auteur("activer_emplacement", 'desactiver-'.$id, generer_url_ecrire('pubban_integer_tous','id_empl='.$id)))."&activer=non' onclick='return confirm(confirm_changer_statut);' title='"._T('pubban:btn_desactive')."'>".$img_ina.$end_img."</a>"
		. "</span>";
