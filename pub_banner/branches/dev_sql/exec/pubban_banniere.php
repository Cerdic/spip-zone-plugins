...
function exec_pubban_banniere_dist() {
	...
	$list_pub = pubban_pubs_de_la_banniere($id_empl);
	...
	$boite = "<div class='cadre_padding'><div class='infos'>"
		.$titre_empl
		."<ul id='instituer_banniere-".$id_empl."' class='instituer_banniere instituer'><li>"._T('pubban:empl_is')."&nbsp;:"
		."<ul>"
		."<li class='publie ".(($s == '1') ? "selected" : "" )."'><a href='".generer_action_auteur("activer_banniere", "activer-$id_empl", self('&'))."' onclick='return confirm(confirm_changer_statut);'><img src='../prive/images/puce-verte.gif' alt=\""._T('pubban:actif')."\"  />"._T('pubban:actif')."</a></li>"
		."<li class='prop ".(($s == '2') ? "selected" : "" )."'><a href='".generer_action_auteur("activer_banniere", "desactiver-$id_empl", self('&'))."' onclick='return confirm(confirm_changer_statut);'><img src='../prive/images/puce-rouge.gif' alt=\""._T('pubban:inactif')."\"  />"._T('pubban:inactif')."</a></li>"
		."<li class='poubelle ".(($s == '3') ? "selected" : "" )."'><a href=\"javascript:delete_entry('".generer_action_auteur("activer_banniere", "trash-$id_empl", generer_url_ecrire('pubban_banniere_tous'))."', '"._T('pubban:confirm_delete_empl')."');\"><img src='../prive/images/puce-poubelle.gif' alt=\""._T('pubban:poubelle')."\"  />"._T('pubban:poubelle')."</a></li>"
		."</ul></li></ul>"
	...
	$res = icone_horizontale(_T('pubban:home'), generer_url_ecrire("pubban_admin"), find_in_path("img/stock_home.png"), "rien.gif", false)
		. icone_horizontale(_T('pubban:retour_liste_empl'), generer_url_ecrire("pubban_banniere_tous"), find_in_path("img/stock_left.png"), "rien.gif", false)
	...
	$contexte = array(
		'id_empl' => $id_empl,
		'icone_editer' => icone_inline(_T('bouton_modifier'), generer_url_ecrire("pubban_editer_banniere","id_empl=$id_empl"), find_in_path("img/stock_edit.png"), "rien.gif",$GLOBALS['spip_lang_right']),
		'icone_nouvelle_pub' => icone_inline(_T('pubban:nouveau_pub_dans_emplacement'), generer_url_ecrire("pubban_editer_publicite","id_pub=new&id_empl=$id_empl"), find_in_path("img/stock_insert-object.png"), "creer.gif",$GLOBALS['spip_lang_right']),
		'redirect'=> generer_url_ecrire("pubban_banniere"),
	...
	echo creer_colonne_droite('', true), debut_droite('', true), debut_cadre_trait_couleur(false, true),
		pipeline('affiche_milieu', array( 'args' => array( 'exec' => 'pubban_banniere' ), 'data' => $milieu ) ),
