...
function exec_pubban_pub_dist() {
	...
	$lien_popup = "javascript:popup(600,600,\"../?page=view_pub&id_pub=".$id_pub."\");";
	if($pub['statut'] == '3obsolete') {
		...
		$boite = "<div class='cadre_padding'><div class='infos'>"
			...
			."<ul>"
			."<li class='publie ".(($s == '1') ? "selected" : "" )."'><a href='".generer_action_auteur("activer_pub", "activer-$id_pub", generer_url_ecrire('pubban_pub','id_pub='.$id_pub))."&activer=oui' onclick='return confirm(confirm_changer_statut);'><img src='../prive/images/puce-verte.gif' alt=\""._T('pubban:active')."\"  />"._T('pubban:actif')."</a></li>"
			."<li class='prop ".(($s == '2') ? "selected" : "" )."'><a href='".generer_action_auteur("activer_pub", "desactiver-$id_pub", generer_url_ecrire('pubban_pub','id_pub='.$id_pub))."&activer=non' onclick='return confirm(confirm_changer_statut);'><img src='../prive/images/puce-rouge.gif' alt=\""._T('pubban:inactive')."\"  />"._T('pubban:inactif')."</a></li>"
			."<li class='poubelle ".(($s == '3') ? "selected" : "" )."'><a href=\"javascript:delete_entry('".generer_action_auteur("activer_pub", "trash-$id_pub", generer_url_ecrire('pubban_pub_tous'))."', '"._T('pubban:confirm_delete')."');\"><img src='../prive/images/puce-poubelle.gif' alt=\""._T('pubban:poubelle')."\"  />"._T('pubban:poubelle')."</a></li>"
			."</ul></li></ul>"
			.icone_horizontale(_T('pubban:voir_un_apercu'), $lien_popup, "racine-24.gif", "rien.gif", false)
			."</div><div class='nettoyeur'></div></div></div>";
	}
	...
	$res = icone_horizontale(_T('pubban:home'), generer_url_ecrire("pubban_admin"), find_in_path('img/stock_home.png'), "rien.gif", false)
		. icone_horizontale(_T('pubban:retour_liste_pub'), generer_url_ecrire("pubban_pub_tous"), find_in_path("img/stock_left.png"), "rien.gif", false)
		. icone_horizontale(_T('pubban:nouveau_pub'), generer_url_ecrire('pubban_pub_edit','id_pub=new'), find_in_path("img/stock_insert-object.png"), "creer.gif", false)
	...
	$contexte = array(
		'id_pub' => _request('id_pub'),
		'icone_editer' => icone_inline(_T('bouton_modifier'), generer_url_ecrire("pubban_pub_edit","id_pub=$id_pub"), find_in_path("img/stock_edit.png"), "rien.gif",$GLOBALS['spip_lang_right']),
	...
	echo creer_colonne_droite('', true), debut_droite('', true), debut_cadre_trait_couleur($icon, true),
		pipeline('affiche_milieu', array( 'args' => array( 'exec' => 'pubban_pub' ), 'data' => $milieu ) ),
