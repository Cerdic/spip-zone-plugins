...
echo "<ul style='margin:0;padding:10px;'><li><strong>Tabella non definita in SPIP</strong>: la tabella non &egrave; utilizzabile in un ciclo e quindi &egrave; inutile indicizzarla</li>";
...
echo "<li><strong>Non indicizza</strong>: Non indicizza la tabella e ne elimina i dati di indicizzazione</li></ul>";
...
if($tabelle_db) {
	while($tab=spip_fetch_array($tabelle_db)) {
			$stato = "";
			$idx = "";
			$azioni ="";
			if(array_key_exists($tab[0],$tabelle_standard)) continue;
			if(!array_key_exists($tab[0],$tables_principales)) {
				$stato = "nodef";
			} else {
		...
		switch($stato) {
			...
			case "ind":
				...
				//if (!array_key_exists($tab[0],$tabelle_standard)) $azioni ="<a href='#'>non indicizza</a>";
				break;
		}
