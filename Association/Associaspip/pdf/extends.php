...
class PDF extends PDF_Table
{
	...
	function PDF($orientation=$GLOBALS['association_metas']['fpdf_orientation'], $unite=$GLOBALS['association_metas']['fpdf_unit'], $format=$GLOBALS['association_metas']['fpdf_format'], $avec_titre=true) {
	...
	function Header(){
		if($this->ajoute_titre) { //Titre
			...
			$this->Cell(0,6,utf8_decode(html_entity_decode($this->titre)),0,1,'C');
			$this->Ln(10);
		}
		parent::Header(); //Imprime l'en-tete du tableau si necessaire
	}
