...
function action_editer_asso_activites_dist()
{
	...
	association_modifier_operation_comptable($date_paiement, $montant, 0, '['._T('evenement').$evenement."->activite$evenement] &mdash ". ($id_adherent?"[$participant"."->membre$id_adherent]":$participant)." +$inscrits", $GLOBALS['association_metas']['pc_activites'], $journal, $id_activite, $id_compte);
	...
	if (!$id_activite) { // la suite serait aleatoire sans cette cle...
	    ...
	    association_ajouter_operation_comptable($date_paiement, $montant, 0, '['._T('evenement').$id_evenement."->activite$evenement] &mdash ". ($id_adherent?"[$participant"."->membre$id_adherent]":$participant), $GLOBALS['association_metas']['pc_activites'], $journal, $id_activite);
	}
    }
