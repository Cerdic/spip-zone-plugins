...
//		  $ladresse=sql_fetsel("*",'spip_adresses', "id_adresse = (SELECT id_adresse FROM spip_adresses_liens WHERE objet='auteur' AND id_objet=$id_auteur AND type IN ('pref','dom','home','perso','domicile','main','principale','principal') LIMIT 0, 1)");
		  ...
		  if (!$mbr_ou)
//			$ladresse=sql_fetsel("*",'spip_adresses', "id_adresse = (SELECT id_adresse FROM spip_adresses_liens WHERE objet='auteur' AND id_objet=$id_auteur LIMIT 0, 1)");
...
$pdf->SetXY(20, 69);
#$pdf->Write(0, $GLOBALS['association_metas']['objet']);
$pdf->Write(0, $objet[0]);
$pdf->SetXY(20, 73);
$pdf->Write(0, $objet[1]);
$pdf->SetXY(20, 78);
...
switch ($GLOBALS['association_metas']['recufiscal'])
{
  case 1:
    ...
    $ladate=explode('/', $lesdates[0]); // date du decret
    $pdf->SetXY(142, 99);
    ...
    $ladate=explode('/', $lesdates[1]); // date du JO
    $pdf->SetXY(42, 104);
    $pdf->Write(0, $ladate[0]);
    $pdf->SetXY(50, 104);
    $pdf->Write(0, $ladate[1]);
  ...
  case 20:
    ...
    $pdf->Write(0, $GLOBALS['association_metas']['infofiscal']);
    break;
  default:
    $ycase=267;
}
...
#$pdf->SetXY(90, 70);
$pdf->Write(0, $montant);
// Donation : somme en toutes lettres... (ou chiffres precedes et suivis de *** )
...
switch (intval($ladate[1]))
{
  case 1:
    ...
    break;
  case 2:
    $mois='fevrier'; // probleme d'encodage
    break;
  case 3:
    $mois='mars';
    break;
  ...
  case 11:
    ...
    break;
  case 12:
    $mois='decembre'; // probleme d'encodage
    break;
  case 0:
  default:
    $mois='';
...
$pdf->Write(0, $mois);
$pdf->SetXY(73, 88);
if ($ladate[2]) $pdf->Write(0, $ladate[2]);
...
if (SIGNATURE_PRES) $pdf->Image(SIGNATURE_PRES,143,248);
/*
