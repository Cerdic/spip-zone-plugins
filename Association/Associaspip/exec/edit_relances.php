...
function exec_edit_relances(){
	...
	$res=association_icone(_T('asso:bouton_impression'),  generer_url_ecrire('edit_labels'), 'print-24.png');

	$res.=association_icone(_T('asso:bouton_retour'),  $_SERVER['HTTP_REFERER'], 'retour-24.png');
...
function relances_while($statut_interne)
{
	...
	while ($data = sql_fetch($query)) {
		...
		. $data['id_auteur']
		."</td>\n"
		.'<td class="$class">'. association_calculer_nom_membre($data['sexe'], $data['prenom'], $data['nom_famille']) ."</td>\n"
