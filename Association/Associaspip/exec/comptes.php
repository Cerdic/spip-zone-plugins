...
function comptes_totaux($where, $imputation)
{
	...
	return '<table width="100%">' .'<caption>'.
	 ...
	 ' :</caption><tbody>' .
	 '<tr class="impair">' .
	 '<th style="color:blue;">'. _T('asso:entrees') .'</th>' .
	 '<td class="nombre">' .association_nbrefr($somme_recettes). ' &euro; </td>' .
	 '</tr>'.'<tr class="pair">' .
	 '<th style="color:pink;">'. _T('asso:sorties') .'</th>' .
	 '<td class="nombre">'.association_nbrefr($somme_depenses) .' &euro;</td>' .
	 '</tr>'. '<tr>' .
	 '<th style="color: #9F1C30;">'. _T('asso:solde') .'</th>' .
	 '<td class="'.($solde>0?'impair':'pair').' nombre">'.association_nbrefr($solde).' &euro;</td>' .
	 '</tr>' .
	 '</tbody></table>';
}
