...
#		echo '<p>'._T('asso:adherent_liste_legende').'</p>';
		...
		echo '<table width="100%"><caption>'._T('asso:adherent_liste_nombre').'</caption><tbody>';
		...
		foreach ($membres as $statut) {
			...
			echo '<tr class="'. $GLOBALS['association_styles_des_statuts'][$statut] .'  border1">';
			echo '<td>'._T('asso:adherent_liste_nombre_'.$statut).'</td>';
			echo '<td class="nombre">'.$nombre.'</td>';
			$nombre_total += $nombre;
			echo '</tr>';
		}
		echo '<tr><th>'._T('asso:adherent_liste_nombre_total').'</th>';
		echo '<th class="nombre">'.$nombre_total.'</th></tr>';
		echo '</tbody></table>';
...
function adherents_liste($debut, $lettre, $critere, $statut_interne, $id_groupe)
{
	...
	while ($data = sql_fetch($query)) {
		...
		if ($GLOBALS['association_metas']['aff_id_auteur']=='on') {
			$auteurs .= '<td class="nombre entier">'
			. $id_auteur.'</td>';
		}
		if ($GLOBALS['association_metas']['aff_photo']=='on') {
			$auteurs .= '<td class="logo centre">'.$logo.'</td>';
		}
		if ($GLOBALS['association_metas']['aff_civilite']=='on' && $GLOBALS['association_metas']['civilite']=='on') $auteurs .= '<td class="honorific-prefix">'.$data['sexe'].'</td>';
		...
		if ($GLOBALS['association_metas']['aff_categorie']=='on') {
			$auteurs .= '<td class="'.$class.'">'. affiche_categorie($data['categorie']) .'</td>';
		}
		if ($GLOBALS['association_metas']['aff_validite']=='on') {
			$auteurs .= '<td class="'.$class.' horodatage">';
			if ($data['validite']==''){
				$auteurs .= '&nbsp;';
			}else{
				$auteurs .= '<abbr class="dtend date" title="'.$data['validite'].'">'. association_datefr($data['validite']) .'</td>';
			}
			$auteurs .= '</td>';
		}
		$auteurs .= '<td class="'.$class.'">'
		. '<a href="'. generer_url_ecrire('auteur_infos','id_auteur='.$id_auteur) .'">'.$icone.'</a></td>'
		. '<td class="'.$class.'">'. association_bouton(_T('asso:adherent_label_ajouter_cotisation'), 'cotis-12.gif', 'ajout_cotisation','id='.$id_auteur) .'</td>'
		. '<td class="'.$class.'">'. association_bouton(_T('asso:adherent_label_modifier_membre'), 'edit-12.gif', 'edit_adherent','id='.$id_auteur) .'</td>'
		. '<td class="'.$class.'">'. association_bouton(_T('asso:adherent_label_voir_membre'), 'voir-12.png', 'voir_adherent','id='.$id_auteur) .'</td>'
