...
if($idCentre>0) {
	...
	foreach(array_keys($tDelib['Refuse']) as $id_serie) {
		$tSeries[$id_serie]=$tab_referentiel['serie'][$id_serie];
	}
	asort($tSeries);
	$msg.="<table class='spip'>\n";
	foreach($tSeries as $iIdSerie=>$sSerie) {
		$nom_pdf=getRewriteString("Refus&eacute;s centre $centre-Serie $sSerie");
	   ...
	   $_SESSION['format'][$nom_pdf]=array('taille'=>'A4', 'orientation'=>'portrait');
		...
		$_SESSION['titre'][$nom_pdf]=html_entity_decode("Refus&eacute;(e)s centre $centre - S&eacute;rie $sSerie");
		...
		$tmp2="Refus&eacute;s au centre <b>$centre</b><br/>S&eacute;rie <b>$sSerie</b></A><br/><br/>";
		$lien=$tmp1.$tmp2;
		$msg.="<tr><td>".vignette('pdf',"Refus&eacute;s centre $centre - s&eacute;rie $sSerie")."</td><td colspan=2>$lien</td></tr>\n";
	}
	$msg.="</table>\n";
