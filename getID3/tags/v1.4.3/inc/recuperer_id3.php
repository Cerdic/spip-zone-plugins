	...
	if (sizeof($file_info) > 0) {
		...
		if (isset($file_info['id3v2']['APIC'])) {
			foreach ($file_info['id3v2']['APIC'] as $cle => $val) {
				if (isset($file_info['id3v2']['APIC'][$cle]['data'])
					&& isset($file_info['id3v2']['APIC'][$cle]['image_mime'])
					&& isset($file_info['id3v2']['APIC'][$cle]['dataoffset'])) {
					$imageinfo = array();
					...
					$dest = $dest.$tmp_file;
					if (ecrire_fichier($dest, $file_info['id3v2']['APIC'][$cle]['data'])) {
						$id3['cover'.$cle] = $dest;
					}
				}
			}
		...
		if (isset($file_info['flac']['APIC'])) {
			if (isset($file_info['flac']['APIC']['data']) && isset($file_info['flac']['APIC']['image_mime'])) {
				...
				$dest = sous_repertoire(_DIR_VAR, 'cache-getid3');
				$dest = $dest.$tmp_file;
				if (ecrire_fichier($dest, $file_info['flac']['APIC']['data'])) {
					$id3['cover'.$cle] = $dest;
				}
			}
		}
