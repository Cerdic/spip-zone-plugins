...
function config_mots_partout() {
  ...
  if ($connect_statut == '0minirezo' AND $connect_toutes_rubriques ) {
	...
	if($type != '') {
	  ...
	  spip_query("CREATE TABLE IF NOT EXISTS `".str_replace('spip_',$table_prefix,$table_principale)."` (`id_mot` bigint(20) NOT NULL default '0',`id_mot` bigint(1) NOT NULL default '0', KEY `id_document` (`id_mot`),KEY `id_mot` (`id_mot`)) TYPE=MyISAM;;");
	}
	
	/*Affichage*/

	debut_droite();
	echo "<form action=\"".generer_url_ecrire('config_mots_partout')."\">";
