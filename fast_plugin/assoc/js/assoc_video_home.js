...
class_assoc.prototype.onglet_actif = function(onglet,cet){
	if ($("#assoc_panel_resultat").find("#tr"+class_assoc.preselect_id).size()==0) return;
	
	
	$("#assoc_panel_onglet #les-onglets span").removeClass("selected");
	$(cet).addClass("selected");
	
	
	if (onglet=="resultat"){
		$("#assoc_panel_resultat").children(":eq(0)").css("display","block");
		if ($("#assoc_panel_resultat").children().size()>1) $("#assoc_panel_resultat").children(":eq(1)").remove();
	} else {
		$("#assoc_panel_resultat").children(":eq(0)").css("display","none");
		if ($("#assoc_panel_resultat").children().size()>1) $("#assoc_panel_resultat").children(":eq(1)").remove();
		if (onglet=="video"){
			video = $("#assoc_panel_resultat").find("#la_video" + class_assoc.preselect_id).html();
			$("#assoc_panel_resultat").append("<div id='voir_video'>" + video + "</div>");
		}
...
class_assoc.prototype.dynamique_video = function(){
	/*
	taille =  parseInt( $("#actu_mag li").size() + 1);
	mag.req = {type : "new", choix : "video_dynamique" , position : taille  };
	mag.ajax();*/
}

class_assoc.prototype.video_bloc = function(){
	titre = $("#preselect_assoc_titre").val();
	texte = $("#preselect_assoc_desc").val();
	taille =  parseInt( $("#actu_mag li").size() + 1);
	mag.req = {type : "new", choix : "video" ,id  : class_assoc.preselect_id, titre : titre , position : taille , texte : texte};
	mag.ajax();
}
