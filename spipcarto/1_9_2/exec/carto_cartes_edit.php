...
//	maj_documents($id_carte, 'carto_carte');
	...
	# affichage
	echo afficher_documents_colonne($id_carte, 'carto_carte');

	//TODO : gerer les droits
//	if (($connect_statut == "0minirezo")&&(in_array('mots_partout',liste_plugin_actifs()))) {
	include_spip("inc/plugin");
	if (array_key_exists('MOTSPARTOUT',liste_plugin_actifs())) {
...
//				$laMap.='<area shape="'.wkt2shape($geometrie,"HTML").'" coords="'.wkt2coords($geometrie,"HTML",$callage,$url_carte).'" href="#objet'.$id_objet.'" alt="'.$titre_objet.'"/>';				
				if ($flag_mots AND $options == 'avancees') {
					...
					if ((!$nouveau) && ($visible)) {
						echo $editer_mot('carto_objet', $tab_id, $cherche_mot, $select_group, $flag_editable);
					}
					else {
						echo $editer_mot('carto_objet', $tab_id, null, $select_group, $flag_editable);
					}
				}
echo fin_block();
