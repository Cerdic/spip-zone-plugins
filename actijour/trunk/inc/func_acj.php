...
function onglets_actijour($actif) {
	# script => icone
	$pages = array(
		'actijour_pg' => _DIR_IMG_ACJR . "activ_jour.gif",
	...
	foreach ($pages as $exec => $icone) {
		$res .= onglet(_T('actijour:onglet_' . $exec), generer_url_ecrire($exec), $exec, ($actif == $exec ? $exec : ''),
			$icone);
	}
...
//
// https://code.spip.net/@aff_referers
function aff_referers($result, $limit, $plus) {
	...
	if (count($nbvisites) > 0) {
		...
		for (reset($nbvisites); $numero = key($nbvisites); next($nbvisites)) {
			...
			if ($visites > 5) {
				$ret .= "<span style='color: red'>$visites " . _T('info_visites') . "</span> ";
			} else {
				if ($visites > 1) {
					$ret .= "$visites " . _T('info_visites') . " ";
				} else {
					$ret .= "<span style='color: #999999'>$visites " . _T('info_visite') . "</span> ";
				}
			}
			...
			if ($lesdomaines[$numero] == "(email)") {
				...
				if ((count($lesreferers[$numero]) > 1) || ((substr(supprimer_tags($lesreferers[$numero][0]), 0,
								1) != '/') && (count($lesreferers[$numero]) > 0))
				) {
					...
					if ($rac = $lesliensracine[$numero]) {
						$aff .= " <span class='spip_x-small'>($rac)</span>";
					}
