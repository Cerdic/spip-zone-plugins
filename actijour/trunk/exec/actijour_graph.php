...
#### fin add. actijour
			...
			if ($date_premier < $date_debut) ### modif actijour ... --> url
			{
			...
			if ((($date_today - $date_debut) / (24 * 3600)) > 30) ### modif actijour ... --> url
			{
...
####### actijour --- 'svg' pas exploite pour actijour !!!	
			if ($GLOBALS['accepte_svg']) {
				echo "\n<div>";
				echo "<object data='", generer_url_ecrire('statistiques_svg',
					"id_article=$id_article&aff_jours=$aff_jours"), "' width='450' height='310' type='image/svg+xml'>";
				echo "<embed src='", generer_url_ecrire('statistiques_svg',
					"id_article=$id_article&aff_jours=$aff_jours"), "' width='450' height='310' type='image/svg+xml' />";
				echo "</object>";
				echo "\n</div>";
				...
				$tagtitle = attribut_html(supprimer_tags(_T('info_aujourdhui') . " $visites_today &rarr; " . (round($prevision,
							0) + $visites_today)));
				...
				echo "</tr></table>";
				echo "</td>",
					"<td " . http_style_background("fond-stats.gif") . "  valign='bottom'>", http_img_rien(3, 1,
				...
				for ($jour = $date_debut; $jour <= $date_today; $jour = $jour + (24 * 3600)) {
					...
					if ($ce_jour == "1") {
						...
						if (date("m", $jour) == 1) {
							$afficher = "<b>" . annee(date("Y-m-d", $jour)) . "</b>";
						}
...
### actijour
			if ($id_article) {
				echo "<br />" . _T('info_popularite_5') . ' ' . $val_popularite;
			}
