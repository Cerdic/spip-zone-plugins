...
function action_corriger_lien_interne_dist(){
		...
		foreach($liens as $l){
			if(preg_match("/^(?:art)?([0-9]+)$/", $l[4],$m)){
				...
				if($id_dest = sql_getfetsel("id_article", "spip_articles", "id_source=$id_source and id_secteur=$id_secteur")){
					...
					spip_log("$id_article : $lien_actuel => $lien_corrige","correction_liens_internes.4");
					...
					spip_log("$id_article : dans " . $l[0] . " : lien vers " . $id_source . " non trouvé","correction_liens_internes_ko.4");
				}
			}
		}
	}
