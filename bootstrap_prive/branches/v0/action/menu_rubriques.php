...
function action_menu_rubriques_dist() {
	...
	if (!autoriser('ecrire')){
		$retour = "<ul class='dropdown-menu cols_1'><li class='dropdown-submenu toutsite'><a href='".generer_url_ecrire('accueil')."'>"._T('public:lien_connecter')."</a></li></ul>";
...
function menu_rubriques($complet = true){
	$ret = "<li><a href='".generer_url_ecrire('plan')."'>"._T('info_tout_site')."</a></li>";

	if (!$complet) {
		return "<ul class='dropdown-menu cols_1'>$ret\n</ul>\n";
	}
	...
	if ($i > 0) {
		...
		$ret = "<ul class='dropdown-menu cols_$nb_col'>"
		  . $ret
		  . "\n</ul>\n";
	}
	else
		$ret = "<ul class='dropdown-menu cols_1'>$ret\n</ul>\n";
...
function bandeau_rubrique($id_rubrique, $titre_rubrique, $zdecal) {
	...
	$ret = "<li class='dropdown-submenu haschild'>$nav<ul class='dropdown-menu cols_$nb_col'>";
