...
function exec_fiche_parent(){
	...
	while ($fiche = spip_fetch_array($result)) {
		...
		echo "<br /><fieldset><legend>"._T('genespip:derniere modification')." &ndash;&rsaquo;<i><b>".$fiche['date_update']."</b></i></legend>";
		...
		$result_pere = spip_query("SELECT * FROM spip_genespip_individu where id_individu=".$numi_pere." and poubelle <> '1'");
		while ($fiche_pere = spip_fetch_array($result_pere)) {
			$resultN = spip_query("SELECT date_evenement FROM spip_genespip_evenements where id_type_evenement='1' and id_individu = ".$fiche_pere['id_individu']);
			$naissance=NULL;
		...
		$result_pere = spip_query("SELECT * FROM spip_genespip_individu where sexe = '0' and poubelle <> '1' order by nom");
		echo "<option value=''>--"._T('genespip:pere inconnu')."--</option>";
		while ($fiche_pere = spip_fetch_array($result_pere)) {
				$resultN = spip_query("SELECT date_evenement FROM spip_genespip_evenements where id_type_evenement='1' and id_individu = ".$fiche_pere['id_individu']);
				$naissance=NULL;
				while (list ($date_evenement) = mysql_fetch_array($resultN)) {
		...
		$result_mere = spip_query("SELECT * FROM spip_genespip_individu where id_individu=".$numi_mere." and poubelle <> '1'");
		while ($fiche_mere = spip_fetch_array($result_mere)) {
			$resultN = spip_query("SELECT date_evenement FROM spip_genespip_evenements where id_type_evenement='1' and id_individu = ".$fiche_mere['id_individu']);
			$naissance=NULL;
			while (list ($date_evenement) = mysql_fetch_array($resultN)) {
				$naissance=genespip_datefr($date_evenement);
			}
		...
		$result_mere = spip_query("SELECT * FROM spip_genespip_individu where sexe = '1' and poubelle <> '1' order by nom");
		echo "<option value=''>--"._T('genespip:mere inconnu')."--</option>";
		while ($fiche_mere = spip_fetch_array($result_mere)) {
			$resultN = spip_query("SELECT date_evenement FROM spip_genespip_evenements where id_type_evenement='1' and id_individu = ".$fiche_mere['id_individu']);
			$naissance=NULL;
			while (list ($date_evenement) = mysql_fetch_array($resultN)) {
		...
		echo "<td colspan='2'><input name='submit' type='submit' value='Valider' class='fondo'></td></tr>";
		echo "</form>";
		echo "</table>";
		echo "</fieldset>";
	}
