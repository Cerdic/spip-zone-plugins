...
$url_action_document=generer_url_ecrire('fiche_document');
    ...
    pipeline('exec_init',array('args'=>array('exec'=>'articles','id_article'=>$id_article,'id_individu'=>$id_individu),'data'=>''));
...
;
    ...
    echo "</td><td style='color:red;font-size:15px'>";
if ($id_individu<>NULL){
        echo debut_boite_info();
        echo gros_titre(_T("Fiche n&ordm; ".$_GET['id_individu'].$_POST['id_individu']));
        genespip_tester_document($id_individu,$id_article,"articles");
        echo fin_boite_info();
}
    echo "</td></tr></table>";
...
$id_individu = $_GET['id_individu'].$_POST['id_individu'];
 ...
 // chargement prealable des fonctions produisant des formulaires
    ...
    .    pipeline('affiche_gauche',array('args'=>array('exec'=>'articles','id_article'=>$id_article,'id_individu'=>$id_individu),'data'=>''))

    .   creer_colonne_droite('', true)

    .   pipeline('affiche_droite',array('args'=>array('exec'=>'articles','id_article'=>$id_article,'id_individu'=>$id_individu),'data'=>''))
...
// affecter les globales dictant les regles de typographie de la langue
    ...
    . pipeline('affiche_milieu',array('args'=>array('exec'=>'articles','id_article'=>$id_article,'id_individu'=>$id_individu),'data'=>''))

    . (!$statut_rubrique ? ''
     : (debut_cadre_relief('', true)
        . $instituer_article($id_article, $statut_article, $id_individu)
...
function boite_info_articles($id_article, $statut_article, $visites, $id_version)
{
    ...
    AND autoriser('voirstats', 'article', $id_article)) {
        $res .= icone_horizontale(_T('icone_evolution_visites', array('visites' => $visites)), generer_url_ecrire("statistiques_visites","id_article=$id_article","id_individu=$id_individu"), "statistiques-24.gif","rien.gif", false);
    }
    ...
    AND autoriser('voirrevisions', 'article', $id_article))
        $res .= icone_horizontale(_T('info_historique_lien'), generer_url_ecrire("articles_versions","id_article=$id_article","id_individu=$id_individu"), "historique-24.gif", "rien.gif", false);
    ...
    if ($GLOBALS['meta']['articles_ortho'] == 'oui') {
        ...
        $res .= icone_horizontale(_T('ortho_verifier'), generer_url_ecrire("articles_ortho", "id_article=$id_article","id_individu=$id_individu"), "ortho-24.gif", "rien.gif", false, $js_ortho);
    }
...
function boites_de_config_articles($id_article)
{
    if (autoriser('modererforum', 'article', $id_article)) {
        ...
        $regler = $regler_moderation($id_article,"articles","id_article=$id_article","id_individu=$id_individu") . '<br />';
    }

    if (autoriser('modererpetition', 'article', $id_article)) {
        $petitionner = charger_fonction('petitionner', 'inc');
        $petition = $petitionner($id_article,"articles","id_article=$id_article","id_individu=$id_individu");
    }
...
function boite_article_virtuel($id_article, $virtuel, $flag)
{
    ...
    $masque = $virtualiser($id_article, $flag, $virtuel, "articles", "id_article=".$id_article."id_individu=".$id_individu);
...
$id_individu = $_GET['id_individu'].$_POST['id_individu'];
    if ($flag_modif) {
        return icone(_T('icone_modifier_article'), generer_url_ecrire("articles_edit","id_article=".$id_article."&id_individu=".$id_individu), $ip, $im, $align, false)
        . "<span class='arial1 spip_small'>$mode</span>"
        . aide("artmodif");
    }
    else return icone(_T('icone_modifier_article'), generer_url_ecrire("articles_edit","id_article=".$id_article."&id_individu=".$id_individu), "article-24.gif", "edit.gif", $align, false);
}
