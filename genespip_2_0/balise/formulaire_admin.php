...
function balise_FORMULAIRE_ADMIN_dyn($float='', $debug='') {
    ...
    global $id_individu, $id_article, $id_breve, $id_rubrique, $id_mot, $id_auteur, $id_syndic;
    ...
    if (!($id_individu || $id_article || $id_rubrique || $id_auteur || $id_breve || $id_mot || $id_syndic))
    ...
    foreach (array('individu', 'article','rubrique','auteur','breve','mot','syndic'=>'site')
    as $id => $obj) {
        if (is_int($id)) $id = $obj;
        if (${'id_'.$id}) {
            $env['id_'.$id] = ${'id_'.$id};
