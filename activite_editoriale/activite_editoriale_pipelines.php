	...
	if(autoriser("webmestre")){
		if (($id = intval($flux['args']['id'])) && ($type=='rubrique'))
		{
			if($config_stats_rubriques) {
				...
				$flux['data'] .= icone_horizontale(_T('activite_editoriale:icone_statistics'), generer_url_ecrire('activite_editoriale_statistics',"id_objet=$id&type=$type"), "", find_in_path("/images/statistics.png"), false);
			}
			if($config_mots_cles)
			{
				$flux['data'] .= recuperer_fond('fonds/mots_cles',array('id_rubrique' => $flux['args']['id']));
				$flux['data'] .= icone_horizontale(_T('activite_editoriale:icone_mots_cles'), generer_url_ecrire('activite_editoriale_mots_cles',"id_objet=$id&type=$type"), "", find_in_path("/images/mot-cle.gif"), false);
			}
		}
	}
	return $flux;
