...
function genie_activite_editoriale_alerte_dist() {
	...
	switch ($config_champ) {
		case 'maj_rubrique':
			activite_tester_maj_rubrique($alerter_auteur);
			break;
...
function activite_tester_date_modif_branche() {
	if ($rubLists = sql_select(array('id_rubrique','extras_delai','extras_identifiants','titre','extras_frequence'), "spip_rubriques", "`extras_delai` != ''")) {
		...
		while($list = sql_fetch($rubLists)) {
			...
			// ceci fonctionne pour la frequence
// 			if ($age % $frequence === 0) {
// 				$list['maj'] = $date_modif;
// 				activite_editoriale_envoyer_mail($list);
// 				spip_log("modulo ok", "activite_editoriale"._LOG_ERREUR);
// 			}
			// donc ceci devrait aussi ??
			if (age_rubrique($date_modif)>$list['extras_delai'] || $age % $frequence === 0) {
				$list['maj'] = $date_modif;
				activite_editoriale_envoyer_mail($list,$alerter_auteur);
				spip_log("modif_branche et age_rubrique + modulo ok", "activite_editoriale"._LOG_ERREUR);
			}
		}
	}
}
	...
	if ($rubLists = sql_select(array('id_rubrique','extras_delai','extras_identifiants','titre','extras_frequence'), "spip_rubriques", "`extras_delai` != ''")) {
		...
		while($list = sql_fetch($rubLists)) {
			...
			if (age_rubrique($date_modif)>$list['extras_delai'] || $age % $frequence === 0) {
				$list['maj'] = $date_modif;
				activite_editoriale_envoyer_mail($list,$alerter_auteur);
				spip_log("modif_rubrique et age_rubrique + modulo ok", "activite_editoriale"._LOG_ERREUR);
			}
		}
	}
}
	...
	if ($auteurLists = sql_select("*", "spip_auteurs", "id_auteur in (".$list['extras_identifiants'].")")) {
		while($auteurs = sql_fetch($auteurLists)) {
			...
			if ($envoyer_mail($to, $subject, $body)) {
				spip_log("Message envoyé à ".$to, "activite_editoriale"._LOG_ERREUR);
			} else {
