...
function table_liste_auteurs($id_groupe, $debut, $crit, $order, $id_tableau,$exc, $callback) {
	...
	for($i=0;$i<ceil($nb_aut/$pagination);$i++) {
		...
		$k = $i + 1;
		if($debut!=$j) 
			$flux .= '<a href="javascript:changer_page_'.$id_tableau.'('.$j.')" id="pagination_'.$id_tableau.'_'.$k.'">'.$k.'</a>&nbsp;';
		else
			$flux .= '<span id="pagination_'.$id_tableau.'_'.$k.'">'.$k.'</span>&nbsp;';
	}
	
	
	//LISTE
	$flux .= "<script type=\"text/javascript\">
	var currentPage_$id_tableau=1;
	function changer_page_$id_tableau(deb) {
	if(currentPage_$id_tableau-1!=(deb/$pagination)) {
		$('#pagination_".$id_tableau."_'+currentPage_$id_tableau).replaceWith('<a href=\"javascript:changer_page_$id_tableau('+((currentPage_$id_tableau-1)*$pagination)+')\" id=\"pagination_".$id_tableau."_'+currentPage_$id_tableau+'\">'+currentPage_$id_tableau+'</a>');
		currentPage_$id_tableau = (deb/$pagination)+1;
		
		$('#pagination_".$id_tableau."_'+currentPage_$id_tableau).replaceWith('<span id=\"pagination_".$id_tableau."_'+currentPage_$id_tableau+'\">'+currentPage_$id_tableau+'</span>');
	}
	...
	$.post('?exec=liste_auteurs', {id_groupe : ".$id_groupe.", debut:deb, crit:'".$crit."', order:'".$order."', id_tableau:'".$id_tableau."', where:'".$where[0]."', pagination:'".$pagination."'}, function(data) {
			...
			$('#div_$id_tableau').replaceWith('<div id=\"div_$id_tableau\">'+data+'</div>');
			$callback();
})};
...
function groupes_lister_champs_auteurs_elargis() {
...
function div_lien($i, $lien = array()) {
	...
	foreach(groupes_lister_champs_auteurs_elargis() as $k=>$v) {
		$liste_chp[] = $k;
	}
