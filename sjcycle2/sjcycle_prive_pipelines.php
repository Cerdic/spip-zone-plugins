...
function sjcycle_affiche_gauche(&$flux){
	...
	if ($flux['args']['exec'] == 'articles_edit') {
		...
		$flux['data'] .= debut_cadre_enfonce(url_absolue(find_in_path('images/sjcycle-24.png')), true, '', 'Diaporama <a href="../plugins/sjcycle/doc.php?art='.$flux["args"]["id_article"].'" target="_blank" onclick="javascript:window.open(\'../plugins/sjcycle/doc.php?art='.$flux["args"]["id_article"].'\', \'aide\', \'scrollbars=yes,resizable=yes,width=740,height=580\');; return false;" rel="#sjcycle_infobulle" id="sjcyle_aide" title="'._T('sjcycle:sjcycle_nouvelle_fenetre').'"><img src="../prive/images/aide.gif" alt="'._T('sjcycle:sjcycle_nouvelle_fenetre').'" class="aide" title="'._T('sjcycle:sjcycle_nouvelle_fenetre').'" /></a>', "", "");
		$flux['data'] .= '<div style="padding:2px;margin:10px 0px;" class="arial1 spip_xx-small">'
				...
				. "</div>\n";
		$flux['data'] .= debut_boite_info(true)._T('sjcycle:sjcycle_boite_info',array('art' => $flux["args"]["id_article"]));
