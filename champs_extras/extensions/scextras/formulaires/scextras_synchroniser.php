...
define(CONNECTION_REF, 'scextras_reference'); // nom du fichier contenant les paramètres de connexion à la base de référence
// Sécurité
if (!defined("_ECRIRE_INC_VERSION")) return;


function formulaires_scextras_synchroniser_charger()
{
	...
	if (!$serveur) $erreur = _T('scextras:erreur_conbase');
	else
	{
...
//echo "<br>$objet --- majr=$majr  ".strtotime($majr)."   majc=$majc".strtotime($majc) ;
			...
			{
				$txt_maj = _T('scextras:datemaj', array('date'=>$majr));;
				$ok = '';
				$warning = _T('scextras:majafaire');
				array_push($defaut, 1);
...
function formulaires_scextras_synchroniser_verifier()
{
...
function formulaires_scextras_synchroniser_traiter()
{
	$retours = array();
	...
	include_spip('base/connect_sql');
	$serveur = spip_connect(CONNECTION_REF);
	if (!$serveur) {$retours['message_erreur'] = _T('scextras:erreur_conbase'); return $retours;}
	else
	{
		...
		{
			echo '<br>'._T('scextras:objet_en_cours', array('objet'=>$objet));
			...
			if (!$result) {$retours['message_erreur'] = _T('scextras:erreur_lecture_ce', array('objet'=>$objet)); return $retours;}
			else
			{
				$row = sql_fetch($result);
				$valeur = $row['valeur'];

				if (!$valeur) {echo _T('scextras:erreur_lecture_defvide', array('objet'=>$objet));}
				else
				{
...
//echo "<br>139"; krumo($valeur);
	...
	//echo "<br>58"; var_dump($maj);
					...
					if ($r==false) {$retours['message_erreur'] = __LINE__._T('scextras:erreur_majce', array('objet'=>$objet)); return $retours;}
					else
					{
	...
	//echo '<br>maj[create]='; var_dump($maj['create']);
						...
						if (count($r)>0) {$retours['message_erreur'] = __LINE__._T('scextras:erreur_majce', array('objet'=>$objet)); return $retours;}
						else
						{
							// La MAJ de la meta a réussi. YOUPI.
							$retours['message_ok'] = _T('scextras:synchro_ok');
						}
					}
				}
			}
