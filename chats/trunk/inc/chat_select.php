 ...
 * @param string|int $id_objet, Identifiant de l'objet, ou "new" pour une création
 * @param int $id_rubrique, identifiant éventuel de la rubrique parente
 * @param int $lier_trad, identifiant éventuel de la traduction de référence
 * @param string $champ_titre, nom de la colonne SQL de l'objet donnant le titre :
 *                             pas idéal ? on devrait pouvoir le savoir
 *                             dans la déclaration de l'objet
 * @return array couples clés / valeurs des champs du formulaire à charger.
**/
function select_objet($type, $id_objet, $id_rubrique=0, $lier_trad=0, $champ_titre = 'titre') {
	...
	// si l'objet existe déjà, on retourne simplement ses valeurs
	if (is_numeric($id_objet))
		return sql_fetsel("*", $table, "$_id_objet=$id_objet");

	// ici, on demande une création.
	// on prerempli certains éléments : les champs si traduction,
	...
	// on récupère les valeurs de la traduction
	if ($lier_trad){
