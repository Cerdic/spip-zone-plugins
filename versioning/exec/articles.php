...
function articles_affiche($id_article, $row, $cherche_auteur, $ids, $cherche_mot,  $select_groupe, $trad_err)
{
 ...
 // chargement prealable des fonctions produisant des formulaires
	...
	$afficher_toutes_versions = charger_fonction('afficher_toutes_versions', 'inc'); 
...
// affecter les globales dictant les regles de typographie de la langue
	...
	// Debut infos liées à l'utilisation du plugin de versioning
	. afficher_version_of($id_article) // Affichage du titre et de l'id de l'article original
	
	. $afficher_toutes_versions('article',$id_article, $flag_editable) 
	// Fin infos liées à l'utilisation du plugin de versioning
...
// Debut Gestion des versions - Netapsys
function bouton_creer_versions($id_article,$align='')
{
	return icone(_T('versioning:bouton_creer_version'),generer_url_ecrire("articles_new_version","id_article=$id_article"), _DIR_VERSIONING_IMG . "article_new_version-24.png", "edit.gif", $align, false);
}
...
function titres_articles($titre, $statut_article,$surtitre, $soustitre, $descriptif, $url_site, $nom_site, $flag_editable, $id_article, $id_rubrique, $modif)
{
	...
	// Affichage de l'icone archivé dans le titre de l'article (on va chercher dans le dossier img du plugin)
	if ($statut_article == 'archi') {
		$res .= gros_titre($titre, _DIR_VERSIONING_IMG ."puce-".puce_statut($statut_article).".gif", false);
	}
	else {	// on récupère les images du dossier par défaut
	...
	$res .=  "<td style='width: 130px;' valign='top' align='$spip_lang_right'>";
	
	// Ajout du bouton (créer une nouvelle version) dans la zone privée, si l'article n'est pas deja une copie et si l'article à le statut publie
	if( (!isACopy($id_article)) && ($statut_article == 'publie'))
	{
		$res .= "\n"
			 . bouton_creer_versions($id_article);			
	}
...
function afficher_version_of($id_article_courant)
{
	// Récupère les informations de l'article courant
	$article_courant = article_select($id_article_courant);
	$id_article_courant_parent = $article_courant['version_of'];  
	
	if(!$id_article_courant_parent)
	{
		return '';
	}
	
	// Récupère les informations de l'article parent
	$article_orig = article_select($id_article_courant_parent);
	$id_article_orig = $article_orig['id_article'];
	$titre_article_orig = $article_orig['titre'];
	
	$res = '';
	
	$res .= "<div> "
			. _T('versioning:version_of_infos') . "$titre_article_orig ($id_article_orig)"
			. "</div>" ;
