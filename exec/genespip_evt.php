...
function genespip_evt($id_type_evenement,$id_individu){
	...
	while ($evt = spip_fetch_array($resultevt)) {
		...
		$resultlieu = spip_query("SELECT * FROM spip_genespip_lieux order by ville");
...
function genespip_new_evt($id_individu,$id_type_evenement){
	$resultevt = spip_query("SELECT * FROM spip_genespip_type_evenements where id_type_evenement=".$id_type_evenement);
	while ($evt = spip_fetch_array($resultevt)) {
		$clair_evenement=$evt['clair_evenement'];
	...
	if ($type_evenement=='MARR'){
		...
		echo '<form action="'.$url_action_fiche.'" method="post">';
		$sql = spip_query("SELECT nom FROM spip_genespip_individu group by nom") or die ("Requête invalide");
		...
		if ($_POST['actionnom']=='choixnom'){
			...
			echo "<table style='border:1px;border-color:black'>";
			echo "<form action='".$url_action_fiche."' method='post'>";
			$result_sexe = spip_query("SELECT sexe FROM spip_genespip_individu where id_individu=".$_POST['id_individu']);
			while ($sexe = spip_fetch_array($result_sexe)){
				$sexe_res = $sexe['sexe'];
				echo $_POST['id_individu']."/sexe=".$sexe_res;
			}
			$result_epoux = spip_query("SELECT id_individu, nom, prenom FROM spip_genespip_individu where sexe!=$sexe_res and nom='".$_POST['choix_nom']."' and poubelle <> '1' order by prenom");
			while ($liste = spip_fetch_array($result_epoux)) {
				$result_date = spip_query("SELECT id_type_evenement,date_evenement FROM spip_genespip_evenements where id_individu=".$liste['id_individu']." and id_type_evenement<>3");
