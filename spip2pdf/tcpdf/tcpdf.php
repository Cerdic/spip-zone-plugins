...
if(!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 2.2.001 (http://tcpdf.sf.net)');
	...
	*/
	class TCPDF {
		...
		*/
		 ...
		 * @since 1.53.0.TC017
		 */
		public $prevFillColor = array(255,255,255);
		 ...
		 * @since 1.53.0.TC017
		 */
		public $prevTextColor = array(0,0,0);
		...
		/**
		 * Indicates whether document is public
		 * @access public
    ...
    public $n_js;
		...
		*/
		public function SetRightMargin($margin) {
			//echo "SetRightMargin margin ".$margin." page ".$this->page." x ".$this->x ." w-margin ".($this->w - $margin)." \n<br/>" ;
			$this->rMargin=$margin;
		...
		*/
		public function GetStringWidth($s) {
			...
			if($this->isunicode) {
				...
				// 2008-02-25 Jun Matushita - Uses modified version of utf8StrRev - not sure this is really needed...
				$unicode = $this->utf8StrRevArr($s, false, $this->tmprtl);
		...
		*/
		public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0) {
			...
			if($txt != '') {
				...
				// 2008-02-25 Jun Matsushita - RTL (Farsi) justification
				if ($align == 'J') {
					// count number of spaces
					$ns = substr_count($txt2, chr(0).' ');
					//("ns:".$ns."<BR>");
					// get string width without spaces
					$width = $this->GetStringWidth($txt)-($ns*$this->GetStringWidth(' '));
		...
		* @access public
		*/
		public function toJPEG($file, $image) {
			if ($image) {
				// output
		...
		* @access public
		*/
		public function outputjpg($filename) {
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		*/
		 ...
		 * @return string
		 * @author Nicola Asuni
		 * @sinc$this->tmprtle 2.1.000 (2008-01-08)
		*/

		// 2008-02-25 Jun Matsushita - Splitted uft8StrRev in 2 to be able to get utf8 array - not sure this is needed...
		public function utf8StrRev($str, $setbom=false, $forcertl=false) {
			return $this->arrUTF8ToUTF16BE($this->utf8StrRevArr($str, $setbom, $forcertl), $setbom);
		}
		public function utf8StrRevArr($str, $setbom=false, $forcertl=false) {
			...
			if (!($forcertl OR $arabic OR preg_match(K_RE_PATTERN_RTL, $str))) {
				return $this->UTF8StringToArray($str);
			}
