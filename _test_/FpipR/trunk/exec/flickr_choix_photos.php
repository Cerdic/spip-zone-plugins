	...
	if($check) {
	  ...
	  if($photos->pages > 1) {
		...
		for($i=1;$i <= $photos->pages;$i++) {
		  if($i != $page) {
			echo '<a href="'.generer_url_ecrire('flickr_choix_photos',"page=$i&type="._request('type')."&id=".intval(_request('id'))."&sort=$sort".(_request('text_search')?"&text_search="._request('text_search'):'')).'">';
		  }
		  echo $i;
		  if($i != $page) {
			echo '</a>';
