...
function produceWharf($champ,$id, $title = '', $sb = '') {
  ...
  $res .= $title;
  $champ=str_replace("document.getElementById('","",$champ);
  $champ=str_replace("')","",$champ);
  $GLOBALS['numero_block'][$id] = $champ.$id.'-'.($GLOBALS['compteur_block']+1);
  if ($visible) {
    $res .= debut_block_visible($champ.$id);
  } else {
    $res .= debut_block_invisible($champ.$id);
  }
  $res .= $sb;
  $res .= fin_block();
  return $res;
...
';
  return produceWharf($champ,'tableau_lien','',$tableau_formulaire); 	
}
...
<td>'._T('bartypenr:barre_gestion_cr_changercasse').' :'. RaccourcisMajusculesMinuscules($champ, $champhelp).'
  ...
  return produceWharf($champ,'tableau_remplacer','',$tableau_formulaire); 
}
...
';
  ...
  return produceWharf($champ,'tableau_boutonsavances','',$tableau_formulaire); 	
}
...
// gestion du remplacement
	...
	$ret .=    bouton_barre_racc("swap_couche('".$GLOBALS['numero_block']['tableau_caracteres']."','');", _DIR_PLUGIN_BARRETYPOENRICHIE."/img_pack/icones_barre/clavier.png", _T('bartypenr:barre_caracteres'), $champhelp);
		...
		$ret .= "\n<td style='text-align: $spip_lang_left;' valign='middle'>";
		$col++;
		$ret .=    bouton_barre_racc("toggle_preview('".str_replace("document.getElementById('","",str_replace("')","",$champ))."');", _DIR_PLUGIN_BARRETYPOENRICHIE."/img_pack/icones_barre/eye.png", _T('bartypenr:barre_preview'), $champhelp);
