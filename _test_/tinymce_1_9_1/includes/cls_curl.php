...
//		automatically set CURLOPT_USERAGENT to the client's user-agent
...
class cls_curl
{
	...
	*/
	...
	function cls_curl($theURL)	{
		if (!function_exists('curl_init')) {
			//try to activate Curl library
			if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') {
				$dl_ok = @dl('php_curl.dll');
			} else {
				$dl_ok = @dl('curl.so');
			}
			if (!$dl_ok)
	...
	function &asPostString(&$theData, $theName = NULL) {
		...
		if (is_array($theData)) {
			foreach ($theData as $theKey => $theValue) {
				if ($thePrefix === NULL) {
					$thePostString .= '&' . cls_curl::asPostString($theValue, $theKey);
				} else {
					$thePostString .= '&' . cls_curl::asPostString($theValue, $thePrefix . '[' . $theKey . ']');
				}
			}
		} else {
	...
	*/
	...
	/**
	* Make CURL do a POST request with the fields specified (can be an array or a string)
	* Returns the result of the request
	* @param $fields : mixed : an array or a string to post
	* @returns string The contents of the page (or other interaction as defined by the
	*                 settings of the various curl options).
	* @access public	 	      
	**/
	function post($fields) {
		$this->setopt(CURLOPT_POST, true);
		$this->setopt(CURLOPT_POSTFIELDS, $this->asPostString($fields));
		return $this->exec();
	}
}
?>
