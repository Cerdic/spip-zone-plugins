...
function exec_honeypot_statistiques_dist() {
  ...
  if ($filtre){
	$result = spip_abstract_select("cnt", "spip_honeypot_stats", "filtre=$filtre");
	if ($row = spip_abstract_fetch($result)) {
	  $total_absolu = $row['cnt'];
	}
	
  } else {
	$result = spip_query("SELECT SUM(cnt) AS total_absolu FROM spip_honeypot_stats");
  ...
  if (count($log)>0) {
	...
	if ($date_premier < $date_debut)
	  echo http_href_img(generer_url_ecrire("statistiques_visites","aff_jours=$aff_jours_plus&filtre=$filtre"),
						 'loupe-moins.gif',
						 "style='border: 0px; vertical-align:center;'",
						 _T('info_zoom'). '-'), "&nbsp;";
	if ( (($date_today - $date_debut) / (24*3600)) > 30)
	  echo http_href_img(generer_url_ecrire("statistiques_visites","aff_jours=$aff_jours_moins&filtre=$filtre"), 
						 'loupe-plus.gif',
						 "style='border: 0px; vertical-align:center;'",
						 _T('info_zoom'). '+'), "&nbsp;";
	...
	if($GLOBALS['accepte_svg']) {
	  echo "\n<div>";
	  echo "<object data='", generer_url_ecrire('honeypot_statistiques_svg',"aff_jours=$aff_jours"), "' width='450' height='310' type='image/svg+xml'>";
	  echo "<embed src='", generer_url_ecrire('honeypot_statistiques_svg',"aff_jours=$aff_jours"), "' width='450' height='310' type='image/svg+xml' />";
	  echo "</object>";
	  echo "\n</div>";
	...
	$result_threat = spip_query("SELECT SUM(threat)/SUM(cnt) AS threat_total FROM $table WHERE $where");
	...
	if (count($log) > 60) {
	  ...
	  echo "<div align='left'>";
	  ///////// Affichage par mois
	  $result=spip_query("SELECT FROM_UNIXTIME(UNIX_TIMESTAMP(date),'%Y-%m') AS date_unix, SUM(cnt) AS total_visites  FROM $table WHERE $where AND date > DATE_SUB(NOW(),INTERVAL 2700 DAY) GROUP BY date_unix ORDER BY date");
