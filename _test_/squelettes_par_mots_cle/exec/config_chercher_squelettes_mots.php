...
//    Fichier cree pour SPIP avec un bout de code emprunte a celui ci.
...
function exec_config_chercher_squelettes_mots() {
  ...
  if ($connect_statut == '0minirezo' AND $connect_toutes_rubriques ) {
	...
	if(!is_array($groupes_mots)) {
		echo _T('squelettesmots:creergroupes',array('url'=>generer_url_ecrire('mots_tous')));	
	} else {
		...
		if (is_array($fonds))
			foreach($fonds as $fond => $a) {
				if(is_int($a[0])) {
				//on essaie de rester compatible avec la version précédente
				$a = array($a);
			}
			foreach($a as $list) {
	          list($id_groupe,$type,$id_table) = $list;
			  $index++;
