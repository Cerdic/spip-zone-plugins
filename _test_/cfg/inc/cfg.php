...
function inc_cfg_dist($class = null, $contexte = array(), $c=NULL, $page_complete = true)
{
	...
	$config = new $class(
		($nom = _request('cfg',$c))? $nom : 'cfg',
		($vue = _request('vue',$c))? $vue : $nom,
		($cfg_id = _request('cfg_id',$c))? $cfg_id : ''
		);

	if (!$config->autoriser()) {
		$res['erreur'] = _T('info_acces_refuse') . " (cfg {$config->nom} - {$config->vue} - {$config->cfg_id})";
		return $res;
	}

	$config->traiter();
	$res['html'] = $config->sortie($contexte,$page_complete);
	return $res;
}
...
//		$this->message .= print_r($this->champs, true);
	...
	*/
	...
	function sortie($contexte = array(), $page_complete = true)
	{
		...
		return
			($page_complete?$this->debut_page():"") .
			debut_cadre_trait_couleur('', true, '', $this->boite) .

			$formulaire .
			
			fin_cadre_trait_couleur(true) .	
			($page_complete?$this->fin_page():"");
