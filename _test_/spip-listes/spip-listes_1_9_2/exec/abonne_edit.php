...
global $suppr_auteur , $id_liste, $effacer_definitif,$nom,$email ; 
...
spip_query("DELETE FROM spip_abonnes_listes WHERE id_auteur='$id_auteur'");
...
if ($suppr_auteur AND $id_liste) {
        ...
        $query = spip_query("SELECT * FROM spip_listes WHERE id_liste='$id_liste'");
        $row = spip_fetch_array($query);
        $title = $row['titre'] ;
        echo "<h2> ".$nom." "._T('spiplistes:plus_abonne').$title." </h2>";
        $query="DELETE FROM spip_abonnes_listes WHERE id_auteur='$suppr_auteur' AND id_liste='$id_liste'";
	spip_query($query);
}
