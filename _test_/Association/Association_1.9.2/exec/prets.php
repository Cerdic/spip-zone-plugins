	...
	**/
	...
	function exec_prets(){
		...
		while ($data = spip_fetch_array($query)) {
			$statut=$data['statut'];
			echo '<div style="font-weight: bold; text-align: center" class="verdana1 spip_xx-small">RESSOURCE N&deg; <br />';
			echo '<span class="spip_xx-large">'.$data['id_ressource'].'</span></div>';
			echo '<p>Code: '.$data['code'].'<br />';
			echo $data['intitule'];
			echo '</p>';
		}
		fin_boite_info();
		...
		if ($connect_statut == '0minirezo') {
			debut_raccourcis();
			if ($statut=="ok") {
				icone_horizontale("Nouveau pret", generer_url_ecrire("edit_pret","action=ajoute&id=$id_ressource"), "fiche-perso-24.gif","cree.gif");
			}
			icone_horizontale("Retour", generer_url_ecrire("ressources","id=$id_ressource"), _DIR_PLUGIN_ASSOCIATION."/img_pack/livredor.png","rien.gif");	
			fin_raccourcis();
		}	

		debut_droite();
		debut_cadre_relief(  "", false, "", $titre = 'PRETS');
		
		echo "<table border=0 cellpadding=2 cellspacing=0 width='100%' class='arial2' style='border: 1px solid #aaaaaa;'>\n";
		echo "<tr bgcolor='#DBE1C5'>";
		echo '<td>&nbsp;</td>';
		echo '<td>ID</td>';
		echo '<td><strong>Sortie</strong></td>';
		echo '<td><strong>Nom</strong></td>';
		echo '<td><strong>Dur&eacute;e</strong></td>';
		echo '<td><strong>Retour</strong></td>';
		echo '<td colspan="2" style="text-align:center;"><strong>'._T('asso:adherent_entete_action').'</strong></td>';
		echo'  </tr>';
		...
		while ($data = spip_fetch_array($query)) {
			...
			echo '</td>';
			echo '<td class="arial11" style="border-top: 1px solid #CCCCCC;text-align:center;"><a href="'.$url_action_prets.'&action=supprime&id_pret='.$data['id_pret'].'"><img src="'._DIR_PLUGIN_ASSOCIATION.'/img_pack/poubelle-12.gif" title="Annuler le pret"></a></td>';
			echo '<td class="arial11" style="border-top: 1px solid #CCCCCC;text-align:center;"><a href="'.$url_edit_pret.'&action=modifie&id='.$data['id_pret'].'"><img src="'._DIR_PLUGIN_ASSOCIATION.'/img_pack/edit-12.gif" title="Editer le pret"></a></td>';
			echo'  </tr>';
		}     
		echo'</table>';
