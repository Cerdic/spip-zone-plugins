 ...
 * 
 * ATTENTION
 * VERSION PERSONNALISE POUR SPIP / INSCRIPTION2
 * (application des .error au éléments parent)
 *
 * 
...
jQuery.extend(jQuery.validator, {
	...
	prototype: {
		...
		check: function( element ) {
			element = this.clean( element );
			jQuery( element ).parent().removeClass( this.settings.errorClass );
			...
			for( var i = 0, rule; rule = rules[i++]; ) {
				try {
					...
					if( result === -1 )
						break;
					if( !result ) {
                                                // MODIFIÉ PAR DAMIEN (ajout de parent() )
						jQuery( element ).parent().addClass( this.settings.errorClass );
