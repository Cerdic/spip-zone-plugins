...
function formulaires_login_charger_dist($cible="",$login="",$prive=null){
	...
	$valeurs['_hidden'] = 
	'<input type="hidden" name="session_password_sha" value="" />'
	. '<input type="hidden" name="next_session_password_sha" value="" />';
...
function formulaires_login_verifier_dist($cible="",$login="",$prive=null){
	...
	$session_shapass = _request('session_password_sha');
	$session_shanext = _request('next_session_password_sha');
	...
	if ($session_login) {
		...
		$session_shapass, $session_shanext, $session_remember)){
			if (strlen($session_password) OR strlen($session_shapass))
