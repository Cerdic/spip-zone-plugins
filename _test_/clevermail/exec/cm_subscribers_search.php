...
function exec_cm_subscribers_search() {
	...
	echo '<form name="subscribers" action="'.generer_url_ecrire('cm_subscribers_search','').'" method="post">'."\n";
	...
	if(strlen(addslashes($_POST['email']))>2) {
		...
		if($total) {
			echo $total.' '._T('cm:resultats').'<br /><br />';
			...
			while ($row = spip_fetch_array($result)) {
				...
				echo '</td>'."\n";
				echo '<td class="arial1" style="border-top: 1px solid #CCC;">'."\n";
				echo '<a href="'.generer_url_ecrire("cm_subscribers_detail","sub_id=".$row['sub_id']).'">'._T('cm:modifier').'</a>';
				echo '</td>'."\n";
				echo '</tr>'."\n";
			}
			echo "</table>\n";
	...
	icone_horizontale(_T('cm:supprimer_abonnes'), 'javascript:if(confirm("'._T('cm:confirme_suppression_multiple_base').'")){checkbox2input(document.subscribers,"sub_id",document.subscribers.sub_ids);document.subscribers.submit();}', '../'._DIR_PLUGIN_CLEVERMAIL.'/img_pack/abonnes.png', 'supprimer.gif');
	echo '</form>'."\n";
	fin_page();
}
?>
