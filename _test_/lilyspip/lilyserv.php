<?php
//
// Serveur d'images Lilypond developpe pour SPIP par Richard Christophe
// a partir du serveur Tex de Philippe Riviere <fil@rezo.net> et Benjamin Sonntag <benjamin@sonntag.fr>
...
function lilypond_($texte, $code_format) {
	...
	$fichier = "$cache_dir/".md5(trim($texte));
	...
	if (!file_exists($fichier_image) OR (filemtime($fichier_image)<filemtime("lilyserv.php"))) {
		...
		$cmd = $lilypond_bin." --safe --png --output=$cache_dir/"." ".$fichier_source;
		exec($cmd);
		$cmd2 = $convert_bin." -trim ".$fichier_image." ".$fichier_image;
		exec($cmd2);
	}

	//efface fichiers ly et ps du CACHE
...
else if ($format=="ly") $type_header = "text/plain";
else if ($format=="midi") $type_header = "audio/mid";
