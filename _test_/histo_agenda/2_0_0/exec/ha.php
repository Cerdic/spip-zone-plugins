...
function ha_voir_modifications($dt1, $dt2)
{
 	...
 	if (($dt1 == "maintenant") || ($dt2 == "maintenant"))
	{
		...
		// de histo
		$fdi = fopen($GLOBALS["meta"]["adresse_site"]."/spip.php?page=histom&var_mode=recalcul&datelim=".$datelim, "r");
		$fdo = fopen($fn, "w");
		$deb = false;
...
function ha_voir_evenements($dt)
{
	...
	if ($dt == "maintenant")
	{
		...
		//$dt = date("Ymd");
		$fn = $ardir."/ar_".$dt.".txt";	
		$fdi = fopen($GLOBALS["meta"]["adresse_site"]."/spip.php?page=histo&var_mode=recalcul", "r");
		$fdo = fopen($fn, "w");
		$deb = false;
		while($line = fread($fdi, 1024))
		{
...
function exec_ha() 
{
	...
	$out = $commencer_page('&laquo; '._T('agenda:titre_page').' &raquo;', "documents", "mots", '', _DIR_PLUGIN_HA."/css/ha.css");
...
*/	
	$out .=  '<br><br><center>';
	$out .=  gros_titre(_T('ha:titre_page'), '', false);
	$out .=  '</center>';
	
	$fics = ha_get_fichiers();
	$out .= debut_gauche("", true);
	
	$out .=  '<form method="post" action="'.generer_url_ecrire('ha','haaction=evs').'">';
	$out .= debut_cadre_enfonce('',true,'',_T('ha:voir_evenements'));
	$out .=  "<div><span>"._T("ha:dates_sauvegardes")."</span>";
	$out .=  "<br /><select name='dateev' style='width:150px;'>";
	foreach($fics as $opt=>$fic)
	{
		$sel = ""; 
		if ($_POST["dateev"] == $opt)
			$sel = "selected";
		$out .=  "<option value='".$opt."' ".$sel.">".$fic."</option>";
	}
	$out .=  "</select>";
	$out .=  "<br /><input type='submit' value='"._T("ha:visualiser")."'>";
	$out .=  "</div>";
	$out .=  fin_cadre_enfonce(true);
	$out .=  "</form>";
	
	$out .= '<form method="post" action="'.generer_url_ecrire('ha','haaction=modifs').'">';
	$out .= debut_cadre_enfonce('',true,'',_T('ha:voir_modifications'));
	$out .= "<div><span>"._T("ha:modification_entre")."</span>";
	$out .= "<br /><select name='datem1' style='width:150px;'>";
	foreach($fics as $opt=>$fic)
	{
		$sel = "";
		if ($_POST["datem1"] == $opt)
	...
	$out .= "</select>";
	$out .= "<br /><span>"._T("ha:modification_et")."</span>";
	$out .= "<br /><select name='datem2' style='width:150px;'>";
	foreach($fics as $opt=>$fic)
	{
		$sel = "";
		if ($_POST["datem2"] == $opt)
	...
	$out .= debut_cadre_enfonce('',true,'',$titre);
	$out .= "<div>".$contenu."</div>";
	$out .= fin_cadre_enfonce(true);
