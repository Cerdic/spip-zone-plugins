...
// maxnev maximum number events visualized after the calendar, first parameter of balis
   ...
   if (isset($_GET['calendrier_mois'])) $calendrier_mois= (int) strip_tags($_GET['calendrier_mois']);
  
   if (isset($_GET['calendrier_annee']))  $calendrier_annee=(int) strip_tags($_GET['calendrier_annee']);
   ...
   GROUP BY articles.id_article ORDER BY articles.date_redac";
       ...
       while($article=mysql_fetch_assoc($result))
       {
         ...
         if ($date > date("Ymd", mkdate($M, $D - 31, $Y)) && $date < date("Ymd", mkdate($M, $D + 31, $Y))) {
            ...
            $events[$date][] = array('link' => generer_url_entite($article['id_article'], 'article'), 'title' => supprimer_numero($article['titre']), 'logo' => "");
         }
        }
   ...
   GROUP BY breves.id_breve ORDER BY breves.evento";
          ...
          while($article=mysql_fetch_assoc($result))
          {
            ...
            if ($date > date("Ymd", mkdate($M, $D - 31, $Y)) && $date < date("Ymd", mkdate($M, $D + 31, $Y))) {
               ...
               $events[$date][] = array(
               'link' => generer_url_entite($article['id_breve'],"breve"), 'title' => supprimer_numero($article['titre']), 'logo' => "");
            }
           }
return $events;
}
