 ...
 * @return array
 */
function formulaires_fusion_spip_supprimer_charger_dist() {
 ...
 * @return array
 */
function formulaires_fusion_spip_supprimer_verifier_dist() {
	$erreurs = array();
 ...
 * @return array
 */
function formulaires_fusion_spip_supprimer_traiter_dist() {
	$erreurs = array();
	...
	foreach ($principales as $nom_table => $val) {
		...
		spip_log('Suppression des objets '.$type_objet.' importés de la base '.$connect, 'fusion_spip_suppression_'.$connect.'.'._LOG_AVERTISSEMENT);

		$res = sql_select('id_final', 'spip_fusion_spip', 'objet='._q($type_objet).' and site_origine="'.$connect.'"');
		...
		sql_delete('spip_fusion_spip', 'objet='._q($type_objet).' and site_origine="'.$connect.'"');
	}

	$retour['message_ok'] = _T('fusion_spip:message_suppression_ok');
