...
\***************************************************************************/
 // Modif Frank SAURET pour le plugin FCKeditor
// Les lignes se terminant par //********** sont les lignes que j'ai ajouté pour fckeditor
$p=explode(basename(_DIR_PLUGINS)."/",str_replace('\\',/*'*/'/',realpath(dirname(dirname(__FILE__)))));//**********
define('_DIR_PLUGIN_FCKEDITOR',(_DIR_PLUGINS.end($p)));//**********		 

//inclure FCKeditor									
include_spip('FCKeditor/fckeditor');
//include_spip('plugins/fckeditor2/FCKeditor/fckeditor');// ceci marche aussi	 
/*include_spip('FCKeditor/fckeditor') tout court est la bonne syntaxe. 
include_spip se charge de trouver le fichier a inclure dans le path spip (donc y compris dans les repertoires des plugins). 
Il permet la surcharge des fichiers par d'autres plugins, ou la personalisation en en mettant une variante dans le dossier squelette.
les plugins peuvent etre installés dans n'importe quel sous repertoire, donc si tu met le chemin en dur dans le code, 
ca ne marchera que chez toi, pas chez quelqu'un qui classe ses plugins dans des sous repertoires.
si le nom de ton repertoire du plugin fkeditor est ici fckeditor2, il faut juste indiquer le chemin relatif :
include_spip('FCKeditor/fckeditor');
*/
...
echo "<P><HR><P>";
	...
	/*+++ Modif Frank SAURET pour FCKEditor */
...
/*+++ Suppression du découpage du texte long en plusieurs morceaux 
...
---*/	
//+++ remplacement par FCKeditor
		
// Modif Frank SAURET d'aprés Aurélien PIERARD => Inclure Fckeditor	
	//Ecriture Texte
	//echo "<B>"._T('info_texte')."</B>";
	//echo "<BR>";
	
	//Lecture de l'éditeur choisis
 ...
 //Action selon choix de l'éditeur	 
...
//echo "<a href=\"";
          if ($argc==0) {
                  $arguments="?exec=articles_edit&new=oui&editeur=wysiwyg";					
				  $argg1=$arguments;
          }
          else {
                  $arguments=ereg_replace('editeur=spip&','',$argv[0]);	 
				  $argg2=$arguments;
                  $arguments=ereg_replace('editeur=wysiwyg&','',$arguments);
				  $argg3=$arguments;
                  $arguments="?exec=articles_edit&new=oui&editeur=wysiwyg&".$arguments;		
				  $argg4=$arguments;
          }
		//Ecriture de la commande
		echo "<input type=\"button\" onClick=\"self.location.href='".$arguments."'\"\" value=\""._T('fckeditor:texte_editeur_avance')."\" class=\"fondo\" style=\"font-size:10px;\"/>";

		
		//Affichage de l'éditeur SPIP
		...
		echo "</TEXTAREA>\n";
				...
				if ($argc==0) {
					$arguments="?exec=articles_edit&new=oui&editeur=spip";
					$argg21=$arguments;
				}
				else {
					$arguments=ereg_replace('editeur=spip&','',$argv[0]);			 
					$arguments=ereg_replace('editeur=wysiwyg&','',$arguments);
					$arguments="?exec=articles_edit&new=oui&editeur=spip&".$arguments; 
				}	 
			
			//Ecriture de la commande
			echo "<input type=\"button\" onClick=\"self.location.href='".$arguments."'\" \" value=\""._T('fckeditor:texte_editeur_standard')."\" class=\"fondo\" style=\"font-size:10px;\"/>";
			//Affichage de l'éditeur FCKeditor
			$oFCKeditor = new FCKeditor('texte') ;
			$oFCKeditor->BasePath = _DIR_PLUGIN_FCKEDITOR."/FCKeditor/" ;	// pourquoi ça part du dossier spip ?	 
			$oFCKeditor->Height    = 500 ;		
			$oFCKeditor->Width =800 ;
			$oFCKeditor->ToolbarSet = 'Default'; // 'Default' pour la barre complète ou 'Basic' pour la barre réduite
			$oFCKeditor->Config['SkinPath'] ='../editor/skins/' . 'office2003' . '/' ;	// 'default' ou	'office2003' ou  'silver'
			$oFCKeditor->Value        = $texte;
			$oFCKeditor->Create() ;			
			
	}	
	
//--- fin de remplacement par FCKeditor	
