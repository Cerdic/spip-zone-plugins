...
if ($flag_editable) {
	...
	//Modification pour intégrer Fck Editor ( par Frank SAURET)
	echo "<div align='center'><font color=#0000FF><BR>";//
	echo _T('fckeditor:texte_modification_article_choix_editeur');//
	//echo "</font></div>";
	//Fin de Modification pour intégrer Fck Editor
	bouton_modifier_articles($id_article, $id_rubrique, $modif,_T('texte_travail_article', $modif), "warning-24.gif","warning-24.gif", "");
	echo "</font></div>";//**********
	echo "</div>";
}
...
function bouton_modifier_articles($id_article, $id_rubrique, $flag_modif, $mode, $ip, $imagefck, $im)
{		   


  	// Les lignes se terminant par //********** sont les lignes que j'ai ajouté pour fckeditor
	if ($flag_modif) {	 
		icone(_T('fckeditor:icone_modifier_article_spip'), generer_url_ecrire("articles_edit","id_article=$id_article"), $ip, $im);
		icone(_T('fckeditor:icone_modifier_article_fck'), generer_url_ecrire("articles_edit","id_article=$id_article&new=oui&editeur=wysiwyg"),$imagefck, $im);//**********
...
function titres_articles($titre, $statut_article,$surtitre, $soustitre, $descriptif, $url_site, $nom_site, $flag_editable, $id_article, $id_rubrique)
{
	...
	echo "<tr width='100%'><td width='100%' valign='top'>";//**********
		...
		echo "<tr width='100%'><td>";//**********
			 bouton_modifier_articles($id_article, $id_rubrique, $modif, _T('avis_article_modifie', $modif), "article-24.gif","../"._DIR_PLUGIN_FCKEDITOR.'/img_pack/article-fck-24.gif', "edit.gif");
		echo "</td>";//**********
		echo "</font></div>";//**********
		}
	echo "</tr>";//**********
