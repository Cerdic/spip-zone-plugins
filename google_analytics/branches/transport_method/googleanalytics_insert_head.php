...
function googleanalytics_snippet(){
	...
	$transport = "";
	if (!empty(lire_config('googleanalytics/ga_transport'))) {
		$transport = "ga('set', 'transport', '".lire_config('googleanalytics/ga_transport')."');";
		if (lire_config('googleanalytics/ga_transport') == "beacon") {
			$transport = "if (navigator.sendBeacon) {".$transport."}";
		}
	}
