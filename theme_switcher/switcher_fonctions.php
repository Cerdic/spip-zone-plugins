...
function switcher_affichage_final($texte){
	...
	if ($html) {
		...
		if (SWITCHER_AFFICHER) {
			...
			$code.='<option selected="selected" value="">Th&egrave;mes</option>';
			foreach( $squelettes_alternatifs as $key => $value)	$code.='<option value="'.parametre_url(self(),'var_theme',$key).'">&nbsp;-> '.$key.'</option>';
			$code.='</select>';
			$code.='</fieldset>';
			$code.='</form>';
			$code.='</div>';
