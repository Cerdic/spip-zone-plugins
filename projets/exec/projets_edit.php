 ...
 * Licence GPL
 *
 * Creation et edition d'un projet (exec=projets_edit)
 *
 */
...
function exec_projets_edit_args($id_projet, $id_parent, $new,$redirect)
{
	...
	if ($new == "oui") {
		$id_projet = 0;
		$titre = filtrer_entites(_T('projet:info_nouveau_projet'));
		$id_parent = 0;
	}
	else {
	...
	pipeline('exec_init',array('args'=>array('exec'=>'projets_edit','id_projet'=>$id_projet),'data'=>''));
	...
	echo pipeline('affiche_gauche',array('args'=>array('exec'=>'projets_edit','id_projet'=>$id_projet),'data'=>''));

	// Colonne droite
	echo creer_colonne_droite('', true);
	echo pipeline('affiche_droite',array('args'=>array('exec'=>'projets_edit','id_projet'=>$id_projet),'data'=>''));

	// Centre
	echo debut_droite('', true);

	$contexte = array(
		'icone_retour'=>icone_inline(_T('icone_retour'), $redirect ? $redirect:generer_url_ecrire("projets","id_projet=$id_projet"), find_in_path('prive/images/projet-24.gif'), "rien.gif",$GLOBALS['spip_lang_left']),
	...
	echo pipeline('affiche_milieu',array('args'=>array('exec'=>'projets_edit','id_rubrique'=>$id_rubrique),'data'=>''));
