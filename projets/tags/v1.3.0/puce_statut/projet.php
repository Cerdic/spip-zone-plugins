...
/**
 * Gestion des puces d'action rapide des projets
 *
 * @package SPIP\Projets\Puce_statut
 **/

/**
 * Gestion des puces statuts des projets
 *
...
function puce_statut_projet_dist($id, $statut, $id_projet_parent, $type = 'projet', $ajax = false, $menu_rapide = _ACTIVER_PUCE_RAPIDE) {
	...
	if (!$id or !_SPIP_AJAX or !$menu_rapide or !autoriser('instituer', $type, $id)) {
		$ajax_node = '';
	} else {
	...
	if ($ajax) {
		...
		$nom = "puce_statut_";
		$action = generer_url_ecrire('puce_statut_projets', "", true);
