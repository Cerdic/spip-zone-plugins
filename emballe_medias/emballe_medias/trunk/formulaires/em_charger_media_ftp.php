...
function formulaires_em_charger_media_ftp_verifier_dist($objet, $id_objet = 0, $extensions = array(), $type = null, $max = null, $redirect = '', $proposer_ftp = true) {
	...
	if (!_request('em_charger_forcer') && (count($erreurs) == 0)) {
		foreach ($files as $file) {
			...
			if (intval($document['id_document']) and (filesize($file['tmp_name']) == $document['taille'])) {
	...
	foreach ($files as $file) {
		...
		if (!is_numeric(reset($nouveaux_doc))) {
			...
			if (_request('em_charger_supprimer')) {
				...
				$del = supprimer_fichier($file['tmp_name']);
				if (!$del) {
					$res['message_erreur'] = _T('emballe_medias:message_suppression_fichier_nok', array('fichier' => $file['tmp_name']));
				}
			}
			if (!$ancre) {
...
function joindre_options_upload_ftp($dir, $mode = 'document', $extensions = null, $max = null) {
	...
	foreach ($fichiers as $f) {
		...
		if (preg_match(',\.([^.]+)$,', $f, $match)) {
			...
			if (!isset($exts[$ext])) {
				...
				} elseif (sql_getfetsel('extension', 'spip_types_documents', $a = "extension = '$ext'" . $inclus)) {
					$exts[$ext] = 'oui';
				} else {
					$exts[$ext] = 'non';
				}
			...
			if ($n === false) {
				$lefichier = $f;
			} elseif (!in_array($ledossier, $dirs) && is_array($newfiles = preg_files($dir.$ledossier))) {
	...
	if ((!$max or ($max >= count($texte_upload))) and (count($texte_upload)>1)) {
