		...
		if (($id_auteur= intval($GLOBALS['visiteur_session']['id_auteur'])) > 0) {
			...
			if (lire_config('emballe_medias/fichiers/chercher_article', 'off') == 'on') {
				$id_article = sql_getfetsel(
					...
					"A.statut IN ('prop','prepa') AND A.em_type = ".sql_quote($type).' AND L.id_auteur = '.intval($id_auteur),
					'',
					'A.maj'
				);
			}
			...
			if (!intval($id_article)) {
				 ...
				 * -* soit on récupère le secteur du premier spip_diogènes dont
				 * l'objet est emballe_medias et dans ce cas :
				...
				if (!$id_rubrique = intval($set['id_rubrique'])) {
					$rub_diogene = sql_getfetsel(
						'id_secteur',
						'spip_diogenes',
						'objet='.sql_quote('emballe_media')
					);
			...
			} elseif (is_numeric($id_document = sql_getfetsel('id_document', 'spip_documents_liens', 'objet="article" AND id_objet='.intval($id_article)))) {
				$e = _L("Document $id_document déja existant");
			} else {
				foreach (array(
					'surtitre', 'titre', 'soustitre', 'descriptif', 'nom_site', 'url_site', 'chapo', 'texte', 'ps'
				) as $champ) {
					if ($set[$champ]) {
						$c[$champ] = $set[$champ];
					}
			...
			if (!$e && intval($id_article)) {
				...
				if (is_numeric($res_document['result']['id'])) {
					...
					if (!$set['titre'] and !$set['texte']) {
						...
						$res_update_article = $crud('update', 'article', $id_article, $infos_doc);
					}

					if (defined('_DIR_PLUGIN_LICENCE') and isset($set['id_licence'])) {
						include_spip('action/editer_article');
					 ...
					 */
					if (defined('_DIR_PLUGIN_GIS') and is_array($set['gis'])) {
					 ...
					 */
					if (defined('_DIR_PLUGIN_SPIPICIOUS') and $set['tags']) {
...
function crud_medias_update_dist($id, $set = null) {
	...
	if (!$e) {
		$crud = charger_fonction('crud', 'action');
		foreach (array(
			'surtitre', 'titre', 'soustitre', 'descriptif', 'nom_site', 'url_site', 'chapo', 'texte', 'ps','id_licence'
		) as $champ) {
			if ($set[$champ]) {
				$c[$champ] = $set[$champ];
			}
		...
		if ($article['success']) {
			 ...
			 * Nécessite un array "gis" dans les arguments avec au moins une latitude et une longitude
			 */
			if (defined('_DIR_PLUGIN_GIS') and is_array($args['gis'])) {
				if (is_numeric($args['gis']['lat'])
					and is_numeric($args['gis']['lon'])) {
