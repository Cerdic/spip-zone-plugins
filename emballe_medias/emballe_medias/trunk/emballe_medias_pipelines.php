...
function emballe_medias_post_edition($flux){
	if(($flux['args']['table'] == 'spip_documents') && ($flux['args']['action'] == 'modifier')){
		...
		if(count($diogenes) > 0){
			...
			$articles_lies = sql_allfetsel('art.id_article,art.maj','spip_articles AS art left join spip_documents_liens AS doc_liens ON art.id_article = doc_liens.id_objet AND doc_liens.objet="article"','id_document='.intval($flux['args']['id_objet']).' AND '.sql_in('art.id_secteur',$secteurs));
	...
	else if(($flux['args']['table'] == 'spip_articles') 
			&& ($flux['args']['action'] == 'instituer')
			&& isset($flux['data']['statut']) && ($flux['data']['statut'] == 'poubelle')){
