...
function formulaires_em_charger_media_ftp_traiter_dist($objet,$id_objet=0,$extensions=array(),$type=null,$max=null,$redirect='',$proposer_ftp=true){
	...
	if (!$id_objet AND $objet) {
		$id_objet = sql_getfetsel("art.id_article","spip_articles AS art LEFT JOIN spip_auteurs_articles AS aut ON (art.id_article=aut.id_article)","art.statut IN ('prop','prepa') AND art.em_type = ".sql_quote($type)." AND aut.id_auteur = ".intval($id_auteur),"","art.maj");
		if(!intval($id_objet)){
			$nouvel_article = true;
			/**
	...
	$nb_docs = 0;
	foreach($files as $file){
		$nouveaux_doc = $ajouter_documents($file['tmp_name'],$file['name'],$objet,$id_objet,$mode,'',$actif);
		if (!is_numeric($nouveaux_doc))
