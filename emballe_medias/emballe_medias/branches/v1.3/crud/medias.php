	...
	if(!file_exists($tmp_name)){
		...
		if(($id_auteur= intval($GLOBALS['visiteur_session']['id_auteur']))>0){
			...
			if (lire_config('emballe_medias/fichiers/chercher_article','off') == 'on')
				$id_article = sql_getfetsel("A.id_article","spip_auteurs_liens AS L LEFT JOIN spip_articles AS A ON (L.objet='article' AND L.id_objet=A.id_article)","A.statut IN ('prop','prepa') AND A.em_type = ".sql_quote($type)." AND L.id_auteur = ".intval($id_auteur),"","A.maj");
