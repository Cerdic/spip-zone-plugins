<?php
// This is a SPIP language file  --  Ceci est un fichier langue de SPIP
// extrait automatiquement de https://trad.spip.net/tradlang_module/tickets?lang_cible=en
// ** ne pas modifier le fichier **

if (!defined('_ECRIRE_INC_VERSION')) {
	return;
}

$GLOBALS[$GLOBALS['idx_lang']] = array(

	// A
	'afficher_tickets' => 'Show bug reports',
	'ajout_deux_points' => '@texte@: ',
	'assignation_attribuee_a' => 'The bug report has been assigned to @nom@.',
	'assignation_attribuee_a_personne' => 'This bug has been assigned to nobody.',
	'assignation_mail_titre' => 'Change of bug report assignment',
	'assignation_modifiee' => 'Assignment Update',
	'assignation_non_modifiee' => 'Assignment not updated',
	'assignation_supprimee' => 'The assignment of this bug report has ben deleted.',
	'assigne_a' => 'Assigned to:',
	'assigne_a_th' => 'Assigned to',
	'assignes_a' => 'Assigned to',
	'autres_tickets_ouverts' => 'Other opened bug reports',

	// C
	'cfg_bouton_radio_aucune_colonne' => 'No column',
	'cfg_bouton_radio_desactiver_forum_public' => 'Disable the use of public forums.',
	'cfg_bouton_radio_une_colonne_par_groupe' => 'One column per group of keywords',
	'cfg_bouton_radio_une_colonne_par_groupe_important' => 'One column per group of important keywords',
	'cfg_bouton_radio_une_colonne_pour_tous_les_groupes' => 'One column for all groups of keywords',
	'cfg_bouton_tickets' => 'Bug reports',
	'cfg_descr_autorisations' => 'This setup page allows you to set permissions for writing, editing, assigning and commenting bug reports. Installing the <a href="@url@">Autorité</a> plugin allows for access to other configuration options, like allowing the author of a comment to modify it once published.',
	'cfg_descr_general' => 'This setup page lets you choose the values ​​of optional fields, comment moderation as well as various other parameters.',
	'cfg_descr_typologie' => 'This configuration page lets you build a classification system for your tickets using SPIP keywords.',
	'cfg_explication_cacher_moteurs' => 'Hide bug reports from search engines asking them not to index their content when they are displayed in the public space.',
	'cfg_explication_desactiver_public' => 'Show the tickets in the private space only', # MODIF
	'cfg_explication_formats_documents_ticket' => 'Separate formats with a comma.',
	'cfg_explication_readonly' => 'This part of the configuration is already set somewhere else.',
	'cfg_form_tickets_autorisations' => 'Permissions',
	'cfg_form_tickets_general' => 'General parameters',
	'cfg_form_tickets_typologie' => 'Classification',
	'cfg_groupe_versions' => 'Group of keywords of versions',
	'cfg_groupe_versions_aucun' => 'None',
	'cfg_groupe_versions_explication' => 'Select, if existing, the group of keywords that contains the versions for the roadmap',
	'cfg_inf_type_autorisation' => 'If you choose by status or by author, you will be prompted below your selection of status or authors.',
	'cfg_lbl_autorisation_auteurs' => '	Allow list of authors',
	'cfg_lbl_autorisation_statuts' => 'Allow by authors status',
	'cfg_lbl_autorisation_webmestre' => 'Allow webmasters only',
	'cfg_lbl_autoriser_modifier' => 'Those who can modify the ticket',
	'cfg_lbl_autoriser_modifier_case' => 'Allow those who have the ability to change the ticket (including author and assigned)',
	'cfg_lbl_cacher_moteurs' => 'Hide from search engines',
	'cfg_lbl_case_joindre_fichiers' => 'Allow to attach one or more files to bug reports',
	'cfg_lbl_case_notification_publique' => 'Be notified in public rather than private space',
	'cfg_lbl_colonnes_groupesmots' => 'Show keywords in the tables',
	'cfg_lbl_colonnes_groupesmots_explication' => 'Choose how to display the groups of keywords that can be associated with tickets (<strong>@groupes@</strong>) in the lists of tickets',
	'cfg_lbl_desactiver_public' => 'Disable public access',
	'cfg_lbl_formats_documents_ticket' => 'Accepted document formats',
	'cfg_lbl_forums_publics' => 'Comments on bug reports',
	'cfg_lbl_joindre_fichiers' => 'Attach one or more files',
	'cfg_lbl_lier_mots' => 'Link keywords',
	'cfg_lbl_lier_mots_aucun' => 'no group',
	'cfg_lbl_lier_mots_explication' => 'Visit the configuration page of each group of keywords to indicate if it should be associated with bug reports.<br/>The following groups can actually be associated: <strong>@groupes@</strong>',
	'cfg_lbl_liste_auteurs' => 'Authors of the website',
	'cfg_lbl_notif_destinataires' => 'Receipients of the notification of creation',
	'cfg_lbl_notification_publique' => 'Public notification',
	'cfg_lbl_selecteur_navigateur' => 'Browser selector',
	'cfg_lbl_statuts_auteurs' => 'Possible status',
	'cfg_lbl_type_autorisation' => 'Authorization method',
	'cfg_lgd_autorisation_assigner' => 'Assign bug reports',
	'cfg_lgd_autorisation_assigneretre' => 'Be assigned to a ticket',
	'cfg_lgd_autorisation_commenter' => 'Comment bug reports',
	'cfg_lgd_autorisation_ecrire' => 'Write bug reports',
	'cfg_lgd_autorisation_epingler' => 'Track the bug reports',
	'cfg_lgd_autorisation_modifier' => 'Edit the bug reports',
	'cfg_lgd_champs_options_autres' => 'Other options',
	'cfg_lgd_commentaires' => 'Comments',
	'cfg_lgd_fichiers' => 'File attached',
	'cfg_lgd_mots' => 'Associate keywords',
	'cfg_lgd_notifications' => 'Notifications',
	'cfg_lgd_notifs_forums' => 'Forums and notifications',
	'cfg_lgd_objets' => 'Association with other objects',
	'cfg_notif_admin' => 'All site administrators',
	'cfg_notif_assigne' => 'Only the person assigned to the ticket',
	'cfg_notif_auteur' => 'All authors of the site',
	'cfg_notif_liste' => 'A list of authors',
	'cfg_notif_liste_auteurs' => 'List of authors receiving notifications',
	'cfg_notif_webmestre' => 'All webmasters',
	'cfg_titre_tickets' => 'Bug reports - Setup of the plugin',
	'champ_assigne' => 'Assigned to:',
	'champ_assigner' => 'Assign to:',
	'champ_composant_th' => 'Component',
	'champ_createur' => 'Created by:',
	'champ_date' => 'Date:',
	'champ_date_debut' => 'From:',
	'champ_date_fin' => 'Until:',
	'champ_date_modif' => 'Modified on',
	'champ_date_th' => 'Date',
	'champ_description' => 'Description of the bug report',
	'champ_exemple' => 'Example:',
	'champ_fichier' => 'Attach a file',
	'champ_id' => 'Number',
	'champ_id_assigne' => 'Assigned to:',
	'champ_id_auteur' => 'Author:',
	'champ_jalon_th' => 'Milestone',
	'champ_maj' => 'Update:',
	'champ_maj_long' => 'Updated date',
	'champ_maj_th' => 'Update',
	'champ_mots_th' => 'Keywords',
	'champ_navigateur' => 'Browser:',
	'champ_nouveau_commentaire' => 'New comment',
	'champ_projet_th' => 'Project',
	'champ_recherche' => 'Search:',
	'champ_severite_th' => 'Severity',
	'champ_statut' => 'Status:',
	'champ_statut_th' => 'Status',
	'champ_sticked' => 'Sticked:',
	'champ_texte' => 'Text',
	'champ_titre' => 'Abstract',
	'champ_titre_th' => 'Summary',
	'champ_titre_ticket' => 'Title of the bug report',
	'champ_type_th' => 'Type',
	'champ_url_exemple' => 'Example URL',
	'champ_version_th' => 'Version',
	'changement_statut_mail' => 'The status of this bug report has been changed from "@ancien@" to "@nouveau@".',
	'classement_assigne' => 'Bug reports by assignment',
	'classement_asuivre' => 'The bug reports to follow',
	'classement_groupe' => 'Tickets by @groupe@',
	'classement_termine' => 'Closed bug reports',
	'classement_version' => 'Tickets by version',
	'commentaire' => 'comment',
	'commentaire_aucun' => 'No comment',
	'commentaires' => 'comments',
	'commenter_ticket' => 'Comment this bug report',
	'creer_et_associer_un_ticket' => 'Create and associate a bug report',
	'creer_ticket' => 'Create a bug report',

	// D
	'date_creation' => 'Created on @date@',
	'date_creation_auteur' => 'Bug report created on <strong>@date@</strong> by <strong>@nom@</strong>',

	// E
	'erreur_date_saisie' => 'This date is invalid',
	'erreur_date_saisie_superieure' => 'The maximum date must be greater than the minimum date',
	'erreur_texte_longueur_mini' => 'The minimum length of text is @nb@ characters.',
	'erreur_verifier_formulaire' => 'Verify your form',
	'explication_autoriser_modifier_base' => 'Automatically, the creator of the ticket and the user who is assigned may modify it. The following configuration allows you to define additional users who can modify the tickets.',
	'explication_champ_sticked' => 'The sticked bug reports are listed first regardless of their status.',
	'explication_description_ticket' => 'Describe as precisely as possible the need or problem.
	Indicate in particular if it occurs consistently or occasionally.
	If it is a display problem, specify with what browser you are experiencing it.',
	'explication_description_ticket_ss_nav' => 'Describe as precisely as possible the need or problem.
	Indicate in particular if it occurs consistently or occasionally.',
	'explication_fichier' => 'Add a file to the bug report.',
	'explication_notif_destinataire' => 'The author of the ticket and the assigned person will systematically receive the notification.',
	'explication_redaction' => 'When you’re finished writing your bug report, select the status «open and discussed».',
	'explication_url_exemple' => 'Enter here the URL of a page covered by this bug report.',

	// I
	'icone_modifier_ticket' => 'Edit this bug report',
	'icone_retour_ticket' => 'Back to the bug report',
	'info_commentaire' => 'Comment #@id@ :',
	'info_demande' => '@nb@ request',
	'info_demande_fermee' => '@nb@ closed',
	'info_demande_ouverte' => '@nb@ open',
	'info_demande_resolue' => '@nb@ solved',
	'info_demandes' => '@nb@ requests',
	'info_demandes_fermees' => '@nb@ closed',
	'info_demandes_ouvertes' => '@nb@ open',
	'info_demandes_resolues' => '@nb@ solved',
	'info_document_ajoute' => 'Added:',
	'info_liste_tickets' => 'Bug reports',
	'info_numero_ticket' => 'BUG REPORT NUMBER:',
	'info_page_configurer' => 'Tickets proposes several configuration pages in order to define the authorizations, the notifications, the comments, the system of classification of tickets as well as other options.',
	'info_retirer_ticket' => 'Withdraw this ticket',
	'info_retirer_tickets' => 'Withdraw all tickets',
	'info_sans' => 'Not defined',
	'info_sans_version' => 'No version',
	'info_ticket_1' => '1bug report',
	'info_ticket_aucun' => 'No bug report',
	'info_ticket_nb' => '@nb@ bug reports',
	'info_tickets' => 'Bug reports',
	'info_tickets_cles_association' => 'The tickets can be associated with:',
	'info_tickets_ouvert' => 'opened and discussed',
	'info_tickets_redac' => 'being edited',

	// L
	'label_mots' => 'Link keywords to the ticket',
	'label_paginer_par' => 'Paginate by:',
	'label_vue_liste_tickets' => 'Change the list display:',
	'lien_ajouter_ticket' => 'Add this ticket',
	'lien_filtrer' => 'Filter the bug reports',
	'lien_identification' => '<a href="@url@" class="spip_in">Log in.</a>',
	'lien_reponse_ticket' => 'Answer to the bug report',
	'lien_supprimer_filtres' => 'Remove all filters',
	'lien_vue_roadmap' => 'Roadmap',
	'lien_vue_tous' => 'Complete list',

	// M
	'mail_texte_message_auto' => 'This is an automatic message: please don’t answer.',
	'message_aucun_ticket_recherche' => 'No ticket matches your search',
	'message_automatique' => 'This is an automatic message: please don’t answer.',
	'message_page_publique_indisponible' => 'This page is unavailable. check that ZPIP is enabled and your plugin configuration allows public access to bug reports.',
	'message_zpip_inactif' => 'This option is disabled because it requires the plugin ZPIP.',
	'mots_aucun' => 'No keywords',

	// N
	'no_assignation' => 'No one',
	'non_assignes' => 'Not owned',
	'nouveau_commentaire_mail' => 'New comment on bug report',
	'nouveau_ticket' => 'New bug report',

	// O
	'option_intro' => '—',
	'option_navigateur_autre' => 'Other',
	'option_navigateur_tous' => 'All browsers',

	// P
	'page_titre' => 'Bug reports, bug tracking system',

	// R
	'revenir_gestion' => 'Back to the bug reports management',

	// S
	'severite_bloquant' => 'Critical',
	'severite_important' => 'Major',
	'severite_normal' => 'Normal',
	'severite_peu_important' => 'Trivial',
	'sinscrire' => 'Subscribe',
	'statut_ferme' => 'Closed',
	'statut_ferme_long' => 'All the closed bug reports',
	'statut_inchange' => 'The status has been modified.',
	'statut_mis_a_jour' => 'Status updated',
	'statut_ouvert' => 'Opened and discussed',
	'statut_poubelle' => 'In the dustbin',
	'statut_redac' => 'Editing in progress',
	'statut_resolu' => 'Resolved',
	'statut_resolu_long' => 'All the resolved bug reports',
	'suivre_tickets_assignes_a' => 'Bug reports assigned to @nom@',
	'suivre_tickets_comments' => 'Follow bug reports comments',
	'suivre_tickets_comments_rss' => 'Follow these comments via RSS',
	'suivre_tickets_de' => 'Bug reports from @nom@',
	'suivre_tickets_id' => 'Monitoring of the bug report #@id@ : @titre@',
	'suivre_tickets_rss' => 'Follow these bug reports via RSS',
	'suivre_tickets_rss_unique' => 'Follow this bug report by RSS',
	'suivre_tickets_statut' => 'Bug reports with the statutes :',
	'suivre_tickets_tous' => 'All bug reports',
	'syndiquer_ticket' => 'Syndicate the bug report:',
	'syndiquer_tickets' => 'Syndicate the bug reports of the website',

	// T
	'texte_ticket_statut' => 'Bug report status:',
	'ticket' => 'Bug report',
	'ticket_enregistre' => 'The bug report has been saved.',
	'tickets' => 'Bug reports',
	'tickets_autorisations' => 'Authorizations',
	'tickets_derniers_commentaires' => 'Last comments',
	'tickets_en_cours_auteur' => '@nom@ bug reports being processed',
	'tickets_general' => 'General parameters',
	'tickets_sticked' => 'Sticked bug reports',
	'tickets_sur_inscription' => 'Bug report or comment only available for registered users.
	',
	'tickets_sur_inscription_droits' => 'The rights you have are inadequate.',
	'tickets_traites' => 'All processed bug reports',
	'tickets_tries' => 'Bug reports matching your criteria',
	'tickets_typologie' => 'Classification',
	'titre' => 'Bug reports, bug tracking',
	'titre_ajouter_un_ticket' => 'Add a ticket',
	'titre_identification' => 'Authentication',
	'titre_liste' => 'List of the bug reports',
	'titre_objets_lies_ticket' => 'Linked to this ticket:',
	'tous_tickets_en_redaction' => 'All bug reports in progress',
	'tous_tickets_ouverts' => 'All opened bug reports',
	'tous_vos_tickets' => 'All your bug reports',
	'type_amelioration' => 'Improvements',
	'type_probleme' => 'Problem',
	'type_tache' => 'Task',

	// V
	'vos_tickets_assignes' => 'Bug reports assigned to you',
	'vos_tickets_assignes_auteur' => '@nom@ bug reports assigned to you',
	'vos_tickets_en_cours' => 'Your ongoing bug reports'
);
