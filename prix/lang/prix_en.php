<?php
// This is a SPIP language file  --  Ceci est un fichier langue de SPIP
// extrait automatiquement de https://trad.spip.net/tradlang_module/prix?lang_cible=en
// ** ne pas modifier le fichier **

if (!defined('_ECRIRE_INC_VERSION')) {
	return;
}

$GLOBALS[$GLOBALS['idx_lang']] = array(

	// C
	'cfg_choisir_option' => 'Please choose an option',
	'cfg_devise_label' => 'Default currency',
	'cfg_locale_explication' => 'Language used for formatting prices. It is the site language by default, but you can specify a regional one if necessary (Australian English, English of the United States, etc.)', # RELIRE
	'cfg_locale_label' => 'Default language',
	'cfg_titre_parametrages' => 'Configuration',

	// I
	'icone_voir_demo' => 'Demo page',
	'icone_voir_devises' => 'View currencies',
	'info_1_devises' => '1 currency',
	'info_nb_devises' => '@nb@ currencies',

	// L
	'label_devise_code' => 'Alphabetical code',
	'label_devise_code_num' => 'Numeric code',
	'label_devise_fraction' => 'Fraction',
	'label_devise_nom' => 'Name',
	'label_devise_symbole' => 'Symbol',
	'label_prix_ht' => 'ET price',
	'label_prix_ttc' => 'ATI price',
	'label_taxes' => 'Taxes',
	'label_total_ht' => 'Total ET',
	'label_total_ttc' => 'Total ATI',

	// M
	'message_extensions_php_ou' => 'One of the following PHP extensions is required: <em>@ext@</em>. <br> Install one or contact your host (preferably the first listed).', # RELIRE

	// P
	'prix_ht' => '@prix@ ET',
	'prix_ttc' => '@prix@ ATI',

	// T
	'titre_devises' => 'Currencies',
	'titre_page_configurer' => 'Configure Prices',
	'titre_prix' => 'Price'
);
