...
function notifications_reservation_vendeur_dist($quoi, $id_reservation, $options) {
	...
	$options['qui'] = 'vendeur';
	$dest = (isset($config['vendeur_' . $config['vendeur']]) and intval($config['vendeur_' . $config['vendeur']])) ? $config['vendeur_' . $config['vendeur']] : array(
		1
	);
	...
	$subject = _T('reservation:une_reservation_sur', array(
	...
	if (isset($GLOBALS[$var_reservation][$chaine_statut]))
		$subject = _T('reservation:' . $chaine_statut, array(
			'nom' => $GLOBALS['meta']['nom_site']
		));
