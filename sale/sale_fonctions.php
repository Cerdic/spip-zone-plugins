...
function extraire_listes($texte) {
	...
	if (count($textMatches) > 1) {
		for ($i = 1; $i < count($textMatches) - 1; ++$i ) {
			if (preg_match(',<ul[^>]*>,is', $tagMatches [$i - 1][0])) {
				++$niveau;
				$pref = $prefixe;
			} elseif (preg_match(',<ol[^>]*>,is', $tagMatches [$i - 1][0])) {
				++$niveau;
				$pref = $prefixenum;
			} elseif (strtolower($tagMatches [$i - 1][0]) == '</ul>') {
				$niveau--;
			} elseif (strtolower($tagMatches [$i - 1][0]) == '</ol>') {
				$niveau--;
			}
			$pre = '';
			if ($niveau) {
				$pre = substr($pref, 0, $niveau + 1);
			}
...
function retrouve_document($type, $fulltag, $innerTag, $shorttag) {
	...
	if (!$puce) {
		if (function_exists('definir_puce')) {
			$puce = definir_puce();
		}
	...
	if ($type == 'img' && isset($attributs['src'])) { // balise img
		$src = $attributs['src'];
	} elseif ($type == 'object' && isset($attributs['data'])) { // balise object
		$src = $attributs['data'];
	}
