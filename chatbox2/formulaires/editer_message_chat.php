...
function formulaires_editer_message_chat_charger_dist(
	$id_message = 'new',
	$type = 'message',
	$retour = '',
	$accepter_email = 'oui',
	...
	if (!intval($id_message)) {
		...
		$valeurs["rv"] = "";
		$valeurs["id_message_origine"] = $id_message_origine;
	}

	$id_message_origine = intval($id_message_origine);
...
function formulaires_editer_message_chat_verifier_dist(
	$id_message = 'new',
	$type = 'message',
	$retour = '',
	$accepter_email = 'oui',
	...
	if (_request('rv') == 'oui') {
		...
		if ($date_debut and $date_fin and $date_fin < $date_debut) {
			$erreurs['date_fin'] = _T('organiseur_public:erreur_date_avant_apres');
		}
	} else {
		set_request('rv', '');
	}
...
function formulaires_editer_message_chat_traiter_dist(
	$id_message = 'new',
	$type = 'message',
	$retour = '',
	$accepter_email = 'oui',
	...
	if ($id_message = $res['id_message']
		...
		// apres un message envoyes, retourner sur la boite d'envoi plutot que sur le message
		if (isset($res['redirect']) and ($res['redirect'] == generer_url_public('message', 'id_message=' . $id_message))) {
		//$res['redirect'] = generer_url_ecrire('messages', 'quoi=envoi');
		// troll			$res['redirect'] = generer_url_public('organiseur', 'quoi=envoi&id_message='.$id_message .'#'.$id_message,false,false);
		$res['redirect'] = generer_url_public('organiseur', 'quoi=envoi&id_message='.$id_message.'#msgs',false,false);
		}
