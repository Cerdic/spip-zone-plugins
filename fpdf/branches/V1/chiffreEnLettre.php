...
class ChiffreEnLettre
{
	...
	#Variables 
	private $leChiffreSaisi;
	private $enLettre='';
	private $chiffre=array(1=>"un",2=>"deux",3=>"trois",4=>"quatre",5=>"cinq",6=>"six",7=>"sept",8=>"huit",9=>"neuf",10=>"dix",11=>"onze",12=>"douze",13=>"treize",14=>"quatorze",15=>"quinze",16=>"seize",17=>"dix-sept",18=>"dix-huit",19=>"dix-neuf",20=>"vingt",30=>"trente",40=>"quarante",50=>"cinquante",60=>"soixante",70=>"soixante-dix",80=>"quatre-vingt",90=>"quatre-vingt-dix");
	...
	#echo $nombre;
		...
		case 2:
			...
			$this->Dizaine($nombre,$unite,$dizaine);
			break;
		...
		case 3:
			...
			$this->Centaine($nombre,$unite,$dizaine,$centaine);
			break;
	...
	return strtoupper($this->enLettre);
	}		
	...
	*/	
	...
	function Million($nombre,$unite,$dizaine,$centaine,$mille,$million)
	{
		...
		#le nombre de caract que comporte le nombre saisi de sa forme sans espace et sans 0 au début
			...
			elseif ($nb==2)
			{
			$this->Dizaine($mille,$unite,$dizaine);
			$this->enLettre.='mille ';
			}
			elseif ($nb==3)
			{
			$this->Centaine($mille,$unite,$dizaine,$centaine);	
			$this->enLettre.='mille ';
			}
			#Gestion des cents 
		...
		#le nombre de caract que comporte le nombre saisi de sa forme sans espace et sans 0 au début
			...
			$this->Unite($cent);
			elseif ($nb==2)
			$this->Dizaine($cent,$unite,$dizaine);
			elseif ($nb==3)
			$this->Centaine($cent,$unite,$dizaine,$centaine);
			}
	}
	...
	function Centaine($nombre,$unite,$dizaine,$centaine)
	{
	...
	function Dizaine($nombre,$unite,$dizaine)
	{
		$unite=substr($nombre,1);
		$dizaine=substr($nombre,0,1);
