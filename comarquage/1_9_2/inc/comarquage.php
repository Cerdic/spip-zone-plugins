...
/*if (!isset($GLOBALS['meta']['comarquage_xml_server'])){
	...
	ecrire_meta('comarquage_xml_server','http://lecomarquage.service-public.fr/xml2v2/');
	ecrire_metas();
}*/
...
if (!isset($GLOBALS['meta']['comarquage_default_xsl_file'])){
	include_spip('inc/meta');
	ecrire_meta('comarquage_default_xsl_file','spThemes.xsl');
	ecrire_metas();
}
if (!isset($GLOBALS['meta']['comarquage_code_insee'])){
	include_spip('inc/meta');
	ecrire_meta('comarquage_code_insee','34172');
	ecrire_metas();
}
...
function & comarquage_compile_page_xml($parametres,$url_base){
	...
	$parametres_xsl['CODE_INSEE'] = lire_meta("comarquage_code_insee" ); // nécessaire pour le tag des stats Xiti
	$parametres_xsl['URL_SCRIPT_XITI'] = $parametres_xsl['SITEURL'].'/'._DIR_PLUGIN_COMARQUAGE."xtcore2.js"; // nécessaire pour le tag des stats Xiti
	$parametres_xsl['TAG_ID'] = basename($parametres['xml_full_path'],'.xml'); // nécessaire pour le tag des stats Xiti
