...
function notation_upgrade($nom_meta_base_version,$version_cible){
	...
	if (   (!isset($GLOBALS['meta'][$nom_meta_base_version]) )
		...
		if ($current_version==0.0){
			...
			$config();
			// Ruse : sur une NOUVELLE installation, on a d�j� tout fait, donc $version_cible directement !!!!
		...
		if (version_compare($current_version,"0.6.3","<")){
			// Pour ceux qui ont install� une 0.6.2 directement avant la correction creant 'accepter_note'
			maj_tables(array('spip_articles'));
