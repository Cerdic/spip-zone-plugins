<?php
/**
 * Ce fichier contient l'ensemble des constantes et fonctions implémentant une API REST pour les données géographiques
 * selon le modèle imposé par le serveur HTTP abstrait.
 *
 * @package SPIP\ISOCODE\REST
 */
 ...
 * Celles-ci sont mises au format de l'API Isocode et fournies au client systématiquement en JSON.
 *
 * @api
 *
 * @param int                                       $code
 ...
 */
function http_geographie_erreur_dist($code, $requete, $reponse) {
	...
	include_spip('inc/isocode_repondre_http');
	$contenu = reponse_isocode_initialiser_contenu($requete);
	...
	$reponse = reponse_isocode_construire($reponse, $contenu);
...
/**
 * Fait un GET sur une collection géogeaohiques.
 * La requête est du type `/geographies/pays` ou `/svp/regions` et renvoie les objets associées contenus
 ...
 */
function http_geographie_get_collection_dist($requete, $reponse) {

	// Initialisation de l'api courante
	$api = 'geographie';
	...
	$declarer = charger_fonction('isocode_declarer_collections', 'inc');
	$collections = $declarer($api);

	// Vérification du nom de la collection.
	$collection = $contenu['requete']['collection'];
	include_spip('inc/isocode_verifier_requete_http');
	if (requete_isocode_verifier_collection($collection, $collections, $erreur)) {
		...
		if (requete_isocode_verifier_filtres($contenu['requete']['filtres'], $collection, $configuration, $erreur)) {
			...
			if ($contenu['donnees']) {
				...
				$contenu['donnees'] = pipeline('post_collection_isocode', $flux);
			}
		}
	}
	...
	if ($erreur) {
		...
		$contenu['erreur'] = reponse_isocode_expliquer_erreur($contenu['erreur'], $collection);
	}
