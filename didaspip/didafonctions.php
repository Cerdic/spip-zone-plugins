...
function dida_post_typo($texte)	{
	@$urldusite=$GLOBALS['meta']['didaspip_url'];
	if (!isset($urldusite)) $urldusite=dirname($_SERVER["PHP_SELF"]);
  
	@$width=$GLOBALS['meta']['didawidth'];
	if (!isset($width)) $width="800";
	
	@$height=$GLOBALS['meta']['didaheight'];
	if (!isset($height)) $width="600";
	
	@$type_affichage=$GLOBALS['meta']['didaspip_box'];
