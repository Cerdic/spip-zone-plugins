 ...
 */
    ...
    $.fn.opencrayon = function(evt, percent) {
        ...
        return this.each(function() {
            ...
            if ($(this).is(".crayon-has")) {
                ...
                $.post(configCrayons.url_crayons_html, params, function(c) {
                    try {
                        c = $.parseJSON(c);
                    } catch (e) {
                        c = {
                            $erreur:
                                "erreur de communication :" + "  " + e.message,
                            $html: ""
                        };
                    }
                    $(me)
                    ...
                    var pos = $(me).offset();
                    $(
                        '<div class="crayon-html" id="crayon_' +
                            id_crayon +
                            '"></div>'
                    )
                        .css({
                    ...
                    var diff =
                        $("#crayon_" + id_crayon).offset().left +
                        $("#crayon_" + id_crayon).width() -
                        $(window).width();
                    if (diff > 0) {
                        $("#crayon_" + id_crayon).css({
                            left: parseInt(pos["left"]) - diff
                        });
                    }
                });
            }
    ...
    $.fn.activatecrayon = function(percent) {
        ...
        this.each(function() {
            ...
            crayon
                .find("form")
                .append(
                    $('<input type="hidden" name="self" />').attr(
                        "value",
                        configCrayons.self
                    )
                )
                .ajaxForm({
                    ...
                    success: function(d) {
                        ...
                        me.find("em.crayon-searching").remove();

                        //Remise a zero des warnings invalides (unwrap)
                        crayon.find("span.crayon-invalide p").remove();
                        crayon.find("span.crayon-invalide").each(function() {
                        ...
                        if (d.$invalides) {
                            for (invalide in d.$invalides) {
                                ...
                                d.$invalides[invalide]["retour"]
                                    ? (retour =
                                          d.$invalides[invalide]["retour"])
                                    : (retour = "");
                                d.$invalides[invalide]["msg"]
                                    ? (msg = d.$invalides[invalide]["msg"])
                                    : (msg = "");
                                crayon
                                    .find("*[name='content_" + invalide + "']")
                                    .wrap(
                                        '<span class="crayon-invalide"></span>'
                                    )
                                    .parent()
                                    .append(
                                        "<p>" + retour + " " + msg + "</p>"
                        ...
                        var tous = $(
                            ...
                            .html(d[$("input.crayon-id", crayon).val()])
                            .iconecrayon();

                        // Invalider des préchargements ajax
                        if (
                            typeof jQuery.spip == "object" &&
                            typeof jQuery.spip.preloaded_urls == "object"
                        ) {
                ...
                .bind("form-submit-validate", function(
                    form,
                    a,
                    e,
                    options,
                ...
                .keyup(function(e) {
                    crayon.find(".crayon-boutons").show();
                    me.addClass("crayon-changed");
                ...
                .each(function(n) {
                    ...
                    } else if (!focus && !$(this).is(":disabled, [readonly]")) this.focus();
                })
                .end()
                .keydown(function(e) {
                    if (!e.charCode && e.keyCode == 119 /* F8, windows */) {
                        crayon.find("form.formulaire_crayon").submit();
                    }
                    if (e.keyCode == 27) {
                        /* esc */
                        me.cancelcrayon();
                ...
                .keypress(function(e) {
                    ...
                    if (
                        (e.ctrlKey &&
                            ...
                            ((e.charCode || e.keyCode) == 115 ||
                                (e.charCode || e.keyCode) == 83)) ||
                        /* ctrl-s, safari */
                    ...
                    if (maxh) {
                        ...
                        maxh =
                            this.scrollHeight < maxh ? this.scrollHeight : maxh;
                    ...
                    $("textarea", this)
                        .each(function() {
                            if (percent && this.scrollHeight > hauteur) {
                                this.scrollTop =
                                    this.scrollHeight * percent - hauteur;
                            }
                        })
                        .resizehandle()
    ...
    $.fn.iconecrayon = function() {
        return this.each(function() {
            ...
            $(this)
                .prepend(configCrayons.iconclick(this.className, type))
    ...
    $.fn.crayonsstart = function() {
        ...
        if (
            typeof crayons_init_dynamique == "undefined" ||
            crayons_init_dynamique == false
        ) {
            $(".crayon:not(.crayon-init)").on("mouseover touchstart", function(
                e
            ) {
                $(this)
