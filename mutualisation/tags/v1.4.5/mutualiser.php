	...
	if ($a_installer = (!is_dir(_DIR_SITE) or !(defined('_DIR_CONNECT') ?
		...
		if ($options['utiliser_panel']) {
			if (defined(_INSTALL_PANEL_HOST_DB)) {
				...
				$link = @mutu_connect_db(_INSTALL_PANEL_HOST_DB, 0, _INSTALL_PANEL_USER_DB, _INSTALL_PANEL_PASS_DB, '',
					_INSTALL_SERVER_DB);
				...
				$result = @sql_query('SELECT '
					...
					. ' WHERE ' . _INSTALL_PANEL_FIELD_SITE . " = '$site'", _INSTALL_SERVER_DB);
				...
				if (is_dir($options['repertoire'] . '/' . _SITES_ADMIN_MUTUALISATION)) {
					...
					define(_FILE_CONNECT,
						$options['repertoire'] . '/' . _SITES_ADMIN_MUTUALISATION . '/config/connect.php');
					...
					$options['login'] = sql_getfetsel('login', 'spip_auteurs',
						'id_auteur=' . intval($mutu['id_admin']));
					define(_FILE_CONNECT, '');
				}
			}
			/*
