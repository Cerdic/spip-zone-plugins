...
define('_DIRMUT',
	str_replace(str_replace('\\', '/', _ROOT_RACINE), '', str_replace('\\', '/', dirname(__FILE__))) . '/');
...
// https://code.spip.net/@mutualiser_creer
...
function mutu_etape_code_activation($e, $options) {
	...
	if ($options['utiliser_panel'] && !defined(_INSTALL_PANEL_HOST_DB)) {
		if (is_dir(_DIR_RACINE . $options['repertoire'] . '/' . $_SERVER['HTTP_HOST'] . '_disable')) {
			...
			exit;
		} elseif (is_dir(_DIR_RACINE . $options['repertoire'] . '/' . $_SERVER['HTTP_HOST'] . '_deleted')) {
			$lien = $options['url_contact_hebergeur'] ? $options['url_contact_hebergeur'] : ($options['url_hebergeur'] ? $options['url_hebergeur'] : _SITES_ADMIN_MUTUALISATION);
			echo mutu_minipres(
	...
	if ($options['creer_base']) {
		if (defined('_INSTALL_SERVER_DB')
			...
			if (defined('_INSTALL_USER_DB_ROOT')) {
				$link = mutu_connect_db(_INSTALL_HOST_DB, 0, _INSTALL_USER_DB_ROOT, _INSTALL_PASS_DB_ROOT, '',
					_INSTALL_SERVER_DB);
			} else {
				$link = mutu_connect_db(_INSTALL_HOST_DB, 0, _INSTALL_USER_DB, _INSTALL_PASS_DB, '',
					_INSTALL_SERVER_DB);
			}
			...
			if (!sql_selectdb(_INSTALL_NAME_DB, _INSTALL_SERVER_DB)) {
				if (_request('creerbase') == 'oui') {
					...
					if ($options['url_creer_base']
					...
					} elseif (sql_query('CREATE DATABASE ' . _INSTALL_NAME_DB, _INSTALL_SERVER_DB)
						...
						if ($options['creer_user_base']) {
							...
							mutu_close();
							$link = mutu_connect_db(_INSTALL_HOST_DB, '', _INSTALL_USER_DB, _INSTALL_PASS_DB, '',
								_INSTALL_SERVER_DB);
						}
						...
						if ($options['mail']) {
							...
							$mail($options['mail'],
								...
								_T('mutu:install_creation_bd_site_2', array(
									'base' => _INSTALL_NAME_DB . '@' . _INSTALL_HOST_DB . ' (' . _INSTALL_USER_DB . ')',
									'site' => $e
								)),
								$options['mail']
							);
	...
	if ($options['creer_site']) {
		...
		if (_request('creerrepertoire') && _request('creerrepertoire') == 'oui') {
			...
			echo mutu_minipres(
				...
				. ($ok
					? _T('mutu:install_creation_rep_ok_1') . '<a href="' . parametre_url(self(), 'creerrepertoire',
						'') . '">' . _T('mutu:install_creation_rep_ok_2') . '</a>.'
					: _T('mutu:install_err')
				) . '</h3>',
				" id='mutu'"
			);
			...
			echo mutu_minipres(
				...
				. '</a></h3>'
				. (!$ok_dir ? _T('mutu:install_repertoire_inaccessible',
					array('repertoire' => '<tt>' . $options['repertoire'] . '/</tt>')) : ''),
				" id='mutu'"
			);
			exit;
