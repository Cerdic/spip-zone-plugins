...
{
    global $table_prefix, $intPremierEnreg, $page;
...
{
    global $intPremierEnreg, $options, $spip_lang_right, $visiteurs, $connect_id_auteur, $connect_statut, $connect_toutes_rubriques, $page;
    ...
    if ($intPremierEnreg_suivant < $nombre_Enregistrements OR $intPremierEnreg > 0)
    {
        ...
        if ($intPremierEnreg > 0)
        {
            ...
            echo "<a href='"
                    ...
                    ,"' title='Voir la liste pr&#233;c&#233;dante des enregistrements' >"
					,"<<<"
					,"</a>";
        }
...
function requete_tableamap($table , $field, $key, $idWhere = false)
{
	...
	// Dans la suite : Le nombre de champs �fficher est dynamique.
	// Et : l'Identifiant (Id Ligne) doit �e le premier champ retourn�ar SELECT
	...
	$Rselect = sql_select( $leschamps,$table,$clauseWhere,$clauseGroupby,$clauseOrderby,$clauseLimit,$clauseHaving);

	$nbenreg = sql_count($Rselect) ;

	//-------- Voir le code de la requ�
...
function afficher_n_enregistrements_amap($tabLesEnregistrements, $table)
{
    ...
    foreach ($tabLesEnregistrements as $intNumLigne=>$tabUnEnregistrement)
    {
        ...
        foreach ($tabUnEnregistrement as $txtChamp=>$txtValeur)
        {
            ...
            echo "\t\t\t<a href='"
                       ,amap_generer_url_ecrire($page, "action=edit&id_ligne=".$idLigne."&table=".$table)  // limitation 1
                ,"'>\n";
            echo "\t\t\t\t",$txtValeur,"\n" ;
            echo "\t\t\t</a>\n";
            echo "\t\t</td>\n";
...
//=========================================================================
//
function table_amap_getmodif($table, $serveur, $field, $key , $idLigne, $modeFiche ="voir")
{
	...
	if ($nombre_Enregistrements>0)
	{
		...
		foreach ($field as $k => $v)
		{
	        ...
	        case "effacer" :
	                ...
	                $hiddens .= "<input type='hidden' name='action' value='ordresuplig'/>\n";
	                $txtbouton ="Effacer d&#233;finitivement";
	                break;
	        case "modif" :
	                $hiddens .= "<input type='hidden' name='id_ligne' value='".$idLigne."'/>\n";
	                $hiddens .= "<input type='hidden' name='action' value='maj'/>\n";
	                $txtbouton ="Modifier";
	                break;
	        case "voir" :
	                $hiddens .= "<input type='hidden' name='id_ligne' value='".$idLigne."'/>\n";
	                $hiddens .= "<input type='hidden' name='action' value='AUCUN'/>\n";
	                $txtbouton ="--";
	                break;
	        default:
...
//===========================================================
function table_amap_maj_post($table , $field, $key, $serveur, $idWhere)
{
  unset($_POST['table']);
  unset($_POST['serveur']);
  unset($_POST['mode']);
...
//===========================================================
function table_amap_effacer_post($table , $serveur, $field, $key, $idWhere)
{
  unset($_POST['table']);
  unset($_POST['serveur']);
  unset($_POST['mode']);
  unset($_POST['exec']);
	...
	return "" .
		...
		."</b> est effac&#233;e<br/>"));
} // function table_amap_effacer_post
...
function  amap_url_generer_post_ecrire ($fonction,$txt_var_post, $txtbtnsubmit="Enregistrer",$boolVoirSPIP=false)
{
    global $boolDebrid;
