...
function table_amap_lister_sortie_contrat($idSaison, $idProduit)
{
  ...
  { // on affiche l'ensemble des sorties disponible pour une saison et un produit donné
    ...
    while ($tabUnEnregistrement = sql_fetch($sqlResult))
    {
      ...
      $out .= "\t\t\t\t<a href='".generer_url_ecrire('amap_contrats', "action=modif&idSortie=".$tabUnEnregistrement['id_sortie']."&idSaison=".$idSaison."&idProduit=".$idProduit."&table=sorties")." '>\n";
      ...
      $out .= "\t\t\t<td class='arial1' style='border-top: 1px solid #cccccc;'>\n";
      $out .= "\t\t\t\t<a href='".generer_url_ecrire('amap_contrats', "action=suppr&idSortie=".$tabUnEnregistrement['id_sortie']."&idSaison=".$idSaison."&idProduit=".$idProduit."&table=sorties")." '>\n";
...
function table_amap_getmodif_sortie_contrat()
{
  ...
  if ($tabUnEnregistrement_1 = sql_fetch($sqlResult_1))
  {
    ...
    return generer_url_post_ecrire('amap_contrats', "table=sorties&action=maj")
                             ."<table>\n".$out
...
function table_amap_get_sortie_contrat($idSaison, $idProduit)
{
  ...
  if ( ($idSaison!=0) && ($idProduit!=0) )
  {
    ...
    return generer_url_post_ecrire('amap_contrats', "table=sorties&action=add")
                             ."<table>\n".$out
                             ."</table>$hiddens<input type='submit'/></form>";
