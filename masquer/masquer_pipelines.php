...
if (!defined('_MOT_MASQUER')) {
	define('_MOT_MASQUER', 'masquer');
}
if (!defined('_SPIP30000') && $GLOBALS['spip_version_code'] >= 17743) {
	define('_SPIP30000', 1);
}
...
function masquer_liste_objets_direct($objet) {
	...
	$tmp = defined('_SPIP30000') ? sql_allfetsel('id_objet', "spip_mots_liens AS ml INNER JOIN spip_mots AS m ON (ml.id_mot=m.id_mot AND ml.objet='$objet')", 'm.titre=' . sql_quote(_MOT_MASQUER)) : sql_allfetsel('id_' . $objet, "spip_mots_{$objet}s AS mo INNER JOIN spip_mots AS m ON mo.id_mot=m.id_mot", 'm.titre=' . sql_quote(_MOT_MASQUER));
