...
function formulaires_abonner_notifications_charger_dist($email, $key, $id_auteur=null){
	...
	if (!$rows)
		echo "<p>"._T('abonnernotifications:message_aucun_abonnement_a_modifier')."</p>";
...
function formulaires_abonner_notifications_traiter_dist($email, $key, $id_auteur=null){
	...
	elseif (_request('modifng')) {
		$ng = _request('notifications_groupees');
		if ($ng == '0') {
			sql_updateq("spip_forum",array('notifications_groupees'=>0),"email_auteur=".sql_quote($email));
			sql_updateq("spip_auteurs",array('notifications_groupees'=>0),"email=".sql_quote($email));
			$res = array("message_ok"=>_T('notifications_groupees:suivi_notifg_off',array('email'=>$email))); }
		if ($ng == '1') {
			sql_updateq("spip_forum",array('notifications_groupees'=>1),"email_auteur=".sql_quote($email));
			sql_updateq("spip_auteurs",array('notifications_groupees'=>1),"email=".sql_quote($email));
			$res = array("message_ok"=>_T('notifications_groupees:suivi_notifg_on',array('email'=>$email))); }
	}
