...
function action_charger_plugin_dist() {
	...
	if (!preg_match(',^(https?|ftp)://.*\.(zip|tgz|tar.gz|tar),', $archive = $arg, $type)
		AND !preg_match(',^(https?|ftp)://.*\.(zip|tgz|tar.gz|tar),', $archive = _request('url_zip_plugin'), $type)
	  AND !preg_match(',^(https?|ftp)://.*\.(zip|tgz|tar.gz|tar),', $archive = _request('url_zip_plugin2'), $type))
	{
		essaie_ajouter_liste_plugins($archive);
	...
	$status = null;
	$fichier = $tmp.basename($archive);
	if (!@file_exists($fichier)) {
		include_spip('inc/distant');
		$contenu = recuperer_page($archive, $fichier, false,_COPIE_LOCALE_MAX_SIZE);
		if (!$contenu) {
			spip_log('charger_decompresser impossible de charger '.$archive);
			$status = -1;
		}
	}
	...
	if (is_array($status)) {
		...
		if ($type == 'plugin') {
			...
			if (_request('extract')) {
				...
				$texte .= '<p>'._T('plugin_zip_installe_finie',array('zip'=>$archive)).'</p>';
				$texte .= "<h2 style='text-align:center;'>"._T('plugin_zip_active')."</h2>";
			} else {
                $texte = '<p>'._T('plugin_zip_telecharge',array('zip'=>$archive)).'</p>';
		...
		// C'est un paquet quelconque
		  ...
		  $retour = _T('plugin_charge_paquet',array('name' => basename($status['tmpname'])));
			...
			if (_request('extract')) {
			  $texte = '<p>'._T('plugin_zip_installe_rep_finie', array('zip'=>$archive, 'rep'=>$status['dirname'])).'</p>';
			} else {
                $texte = "<p>"._T('plugin_zip_telecharge',array('zip'=>$archive))."</p>\n";
				$texte .= liste_fichiers_pclzip($status);
				$suite = 'lib';
	...
	// fichier absent
		...
		$retour = _T('erreur');
		$texte = _T('plugin_erreur_charger', array('zip'=>$archive));
	}
	...
	echo minipres($retour." ",
		$suite
			? redirige_action_post(_request('action'),
				...
				$redirect_action,
				'',
					form_hidden('?url_zip_plugin='.urlencode($archive).'&extract=oui')
					.$texte
