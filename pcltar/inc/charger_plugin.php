...
// https://code.spip.net/@formulaire_charger_plugin
...
// https://code.spip.net/@interface_plugins_auto
function interface_plugins_auto($retour) {
  ...
  . '<br />'._T('plugin_info_automatique_exemples').'<ul class="spip">';
	...
	$les_urls = array('https://plugins.spip.net/rss-+-selection-2-1-+','https://contrib.spip.net/?page=rss-plugins-spip-2-1');
...
// https://code.spip.net/@chargeur_charger_zip
function chargeur_charger_zip($quoi = array())
{
	...
	if (isset($quoi['depot']) || isset($quoi['nom'])) {
		$quoi['archive'] = $quoi['depot'] . $quoi['nom'] . '.'.$quoi['type'];
	}
	...
	if($quoi['type'] === 'zip'){
		...
		if ($zip->error_code < 0) {
			spip_log('charger_decompresser erreur zip ' . $zip->error_code .
				' pour paquet: ' . $quoi['archive']);
			return //$zip->error_code
				$zip->errorName(true);
		}
	}else{
		$ok = PclTarExtract($fichier,
			$quoi['extract']
					? $dir_export
					: $tmpname,
			$quoi['remove']
		...
		if ($ok == 0) {
			spip_log('charger_decompresser erreur tar ' . PclErrorString() .
				' pour paquet: ' . $quoi['archive']);
...
// https://code.spip.net/@chargeur_montre_tout
...
// https://code.spip.net/@chargeur_edit
...
// renommer des morceaux
// https://code.spip.net/@chargeur_rename
...
// https://code.spip.net/@chargeur_activer_plugin
...
// https://code.spip.net/@liste_fichiers_pclzip
...
// https://code.spip.net/@essaie_ajouter_liste_plugins
...
// https://code.spip.net/@chercher_enclosures_zip
...
// https://code.spip.net/@liste_plugins_distants
...
// https://code.spip.net/@afficher_liste_listes_plugins
...
// https://code.spip.net/@bouton_telechargement_plugin
