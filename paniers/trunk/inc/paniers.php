...
function paniers_id_panier_encours() {
	...
	if (!$id_panier
		...
		if (time() < $st = strtotime("$date + " . 3600 * intval(lire_config('paniers/limite_enregistres',
					168)) . 'seconds')) {
	...
	if ($id_panier > 0 and $cookie) {
		...
		spip_setcookie($nom_cookie, $_COOKIE[$nom_cookie] = $cookie,
			time() + 3600 * lire_config('paniers/limite_ephemere', 24));
