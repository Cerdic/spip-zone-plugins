...
function contenu_naviguer($id_rubrique, $id_parent) {
	...
	//////////   Les articles en cours de redaction MODIFIE inclusion directe du fond si choisi ainsi dans cfg
	/////////////////////////
	
	if($liste_compacte=lire_config('taa/liste_compacte')){
	
	
		$sql=sql_select('id_article,id_trad','spip_articles',array('statut="prepa"', 'id_rubrique='.$id_rubrique));
		
		// on filtre les articles non traduits ou origines de traductions
		
		$id_article=array();
		while($row=sql_fetch($sql)){
			if($row['id_trad']==0 OR $row['id_trad'] == $row['id_article']){
		...
		if($id_article){
			...
			'puce'=>'images/puce-blanche.gif',	 		 	  			 	 
			);
		$contenu = recuperer_fond('prive/naviguer/afficher_articles', $contexte,array('ajax'=>true));
		$res .= cadre_depliable(chemin('/prive/images/article-24.gif'),_T('ecrire:info_tous_articles_en_redaction'),true,$contenu,'prix','titrem cadre-liste');   
		}
	
		//////////  Les articles publies  MODIFIE inclusion directe du fond
		/////////////////////////
		
		$sql=sql_select('id_article,id_trad','spip_articles',array('statut="publie"', 'id_rubrique='.$id_rubrique));
		...
		if($id_article){
			$contexte=array(
				...
				'statut'=>'publie',	
				'id_rubrique'=>$id_rubrique,
				'puce'=>'images/puce-verte.gif',	 	 		 	  			 	 
				);
			if($id_article=sql_fetsel('id_article','spip_articles',array('statut="publie"', 'id_rubrique='.$id_rubrique))){
				$contenu = recuperer_fond('prive/naviguer/afficher_articles', $contexte,array('ajax'=>true));
				$res .= cadre_depliable(chemin('/prive/images/article-24.gif'),_T('ecrire:info_tous_articles_presents'),true,$contenu,'prix','titrem cadre-liste'); 
			}
			}
		}
	else{
