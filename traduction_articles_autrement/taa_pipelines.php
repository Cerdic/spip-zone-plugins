		...
		foreach($langues_dispos as $key => $value){
			if($traductions[$value]!=$id_article){
				if(array_key_exists($value,$traductions)){
					...
					$onglets_traduction.= '<div class="non_traduit onglet"><a href="?exec=articles_edit&new=oui&lier_trad='.$id_trad.'&id_rubrique='.$id_rubrique.'&lang='.$value.'" title="'._T('ecrire:info_tout_site2').'">'.traduire_nom_langue($value).'</a></div>';
