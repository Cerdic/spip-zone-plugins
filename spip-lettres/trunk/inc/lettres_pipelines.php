	...
	if (autoriser('voir', 'lettres')) {
		...
		if ($id_rubrique && ( ($admin_abo_toutes_rubriques == 'oui')
			...
			if (autoriser('creerlettredans','rubrique',$id_rubrique)) {
				$flux['data'].= icone_inline(_T('lettresprive:creer_nouvelle_lettre'),
					generer_url_ecrire("lettres_edit", "id_rubrique=$id_rubrique"), 'lettre-24.png',"creer.gif", $spip_lang_right);
		...
		if ( ($admin_abo_toutes_rubriques == 'oui') || ($id_rubrique && lettres_rubrique_autorisee($id_rubrique)) ) {
			...
			$flux['data'].= icone_inline(_T('lettresprive:ajouter_abonne'), generer_url_ecrire("abonnes_edit", "id_rubrique=$id_rubrique"), 'abonne-24.png',"creer.gif", $spip_lang_right);
			$flux['data'].= icone_inline(_T('lettresprive:import_abonnes'), generer_url_ecrire("naviguer_import","id_rubrique=$id_rubrique"), 'import_abonnes.png', "rien.gif", $spip_lang_right);
			$flux['data'].= '<br class="nettoyeur" />';
		}
	}
	return $flux;
