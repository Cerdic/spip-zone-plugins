...
// http://doc.spip.org/@action_editer_motdoc_dist
...
function action_editer_motdoc_dist() {
	...
	if (!preg_match(',^(\d*)\D(-?\d*)\W(\w*)\W(\w*)\W(\w*)\W?(\d*)$,', $arg, $r)) 
		spip_log("action editer_motdoc: $arg pas compris");
	else action_editer_motdoc_post($r);
}

// http://doc.spip.org/@action_editer_motdoc_post
function action_editer_motdoc_post($r)
{
	...
	if ($id_mot) {
			...
			spip_query("DELETE FROM spip_mots_documents WHERE id_mot=$id_mot");
			}
	}
	...
	if (preg_match('/^(.*exec=)editer_motdoc(&.*)script=(grouper_mots)(.*)$/', $redirect, $r))
