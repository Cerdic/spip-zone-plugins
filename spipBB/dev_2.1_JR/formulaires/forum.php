...
if ($citer=='oui') {
	...
	$citer_texte = "<quote=".$citer_auteur.">".$citer_texte."</quote>";
}


//Est-ce qu'il s'agit d'une édition ?
$editer = _request('editer');

//Un autre post est editer, chargeons les variables nécessaires
if ($editer=='oui') {
	$id_editer_forum = _request('id_forum');
	$editer_texte= sql_getfetsel("texte", "spip_forum", "id_forum=" . intval($id_editer_forum));
}
	...
	// on charge la variable texte en fonction des cas de figure
	if($citer=='oui'){$texte = $citer_texte;}
	if($editer=='oui'){$texte = $editer_texte;}
	
	return array(
		'modere' => (($type != 'pri') ? '' : ' '),
		'nom_site' => '',
		'table' => $table,
		// [JR] AJOUT DE QUOTE OU EDITION ICI
		'texte' => $texte,
...
function formulaires_forum_traiter_dist() {
	...
	// si $editer vaut "oui" alors on fait une révision
	if ($editer == 'oui') {
		// modifier le contenu via l'API
		include_spip('inc/modifier');
		// recuperer les champs (y en a qu'un, c'est facile !)
		$c = array();
		foreach(array('texte') as $x)
		        $c[$x] = _request($x);
		// modifier
		modifier_contenu(
                'forum', // nom de la table
                $id_forum, // identifiant
                array('nonvide' => array('titre' => _L('info_sans_titre'))), // parametres
                $c); // champs a modifier
        list($redirect,$id_forum) = array('spip.php?page=voirsujet&id_forum='.intval($id_forum).'&var_mode=calcul','');
		}
	else {
