...
function formulaires_editer_liaison_objet_charger_dist($id_liaison_objet = 'new', $retour = '', $lier_trad = 0, $config_fonc = '', $row = array(), $hidden = '') {
	...
	if (!$valeurs['objet_dest'])
		$valeurs['objet_dest'] = _request('objet_dest');
	if (!$valeurs['id_objet_dest'])
		$valeurs['id_objet_dest'] = _request('id_objet_dest');
	if (!$valeurs['id_objet'])
	...
	if (!$types = lire_config('liaison_objet/type_liens_' . $valeurs['objet_dest'], array()))
