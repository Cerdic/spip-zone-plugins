...
function action_ranger_dist($arg=null){
    ...
    switch($action){
        ...
        case 'remonter_ordre':
            ...
            sql_updateq("spip_liaison_objets", array("ordre" =>$o),'id_liaison_objet='.$row['id_liaison_objet']); 
            }
                 $where = array(
                    'id_objet_dest='.$id_objet_dest,
                    'objet_dest='.sql_quote($objet_dest),
        ...
        case 'nouvel_ordre':
            ...
            foreach($nouvel_ordre AS $id_liaison_objet){
                $ordre++;
                sql_updateq("spip_liaison_objets", array("ordre" => $ordre),'id_liaison_objet='.$id_liaison_objet);
        ...
        case 'nouvel_ordre_objet':
            ...
            foreach($nouvel_ordre AS $id_liaison_objet){
                $ordre++;
                sql_updateq("spip_liaison_objets", array("ordre_objet" => $ordre),'id_liaison_objet='.$id_liaison_objet);
                include_spip('inc/invalideur');
                suivre_invalideur("id='liaison_objet/$id_liaison_objet'");    
            }
            break;
