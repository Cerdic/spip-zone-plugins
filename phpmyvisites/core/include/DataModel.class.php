...
// $Id: DataModel.class.php,v 1.80 2006/01/13 01:24:49 matthieu_ Exp $
...
class DataModel
{
	...
	function DataModel( & $o_archive, & $o_request)
	{
		...
		if(isset($this->infoSerialized['simple']) && $this->infoSerialized['simple'] == 1)
		{
			...
			{
				$GLOBALS['header_message_tpl'] .= "<br>This is a simple archiving. ONLY some statistics are available for this archive, because the period asked in the statistics is not finished yet. <br />You can turn 'Simple Archive' Off in the file /core/include/global.php by setting  CURRENT_XXXX_SIMPLE_ARCHIVE to false.<br/>";
	...
	function getSites()
	{
		if(!isset($this->siteInfo))
		{
			...
			//array();
			//require _PHPMV_DIR_CONFIG."/site_info.php";
...
//		printDebug($all); exit;
		...
		foreach($archives as $dateToDisplay => $archive)
		{
			...
			{
				$file = _PHPMV_DIR_DATA . "/archives/". $o_data->infoSerialized['idarchives'].".dat";
				if( !is_readable($file))
				{
			...
			else
			{
				...
				{
					$visPagGrp = $this->getContent('vis_pag_grp');
				}
			} 
