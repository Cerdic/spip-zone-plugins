...
formidable_ts = '';
$(function() {
	// mettre une haute fixe au wrapper, pour que le sticky header css puisse fonction avec un scroller
	formidable_ts = $(".tablesorter");
	wrapper_height = $('thead', formidable_ts).height() + $('tbody tr', formidable_ts).first().height()*10;
	$('head').append('<style id="formidable_ts" type="text/css">.formidable_tablesorter-wrapper {height:'+wrapper_height+'px;}</style>');
	formidable_ts.tablesorter({
		theme: 'blue',
		widgets: [
			"zebra",
...
function formidable_ts_add_reorder_arrows() {
	$('.move-arrows').remove();
	th = $('.tablesorter-ignoreRow th', formidable_ts).not('.filtered');
	...
	$('.move-arrows a').click(function() {
		...
		$('tr', formidable_ts).each(function(){
			cells = $(this).children();
