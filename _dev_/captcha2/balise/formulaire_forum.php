...
$ajouter_mot, $ajouter_groupe, $afficher_texte, $url_param_retour)
{
	...
	if (!$retour_forum = rawurldecode(_request('retour_forum'))) {
		...
		$captcha = _request('captcha2');   // surcharge plugin

		if ($afficher_texte != 'non') 
			$previsu = inclure_previsu($texte, $titre, $email_auteur, $auteur, $url_site, $nom_site, $ajouter_mot,$captcha); // surcharge plugin
...
function inclure_previsu($texte,$titre, $email_auteur, $auteur, $url_site, $nom_site, $ajouter_mot,$captcha)    // surcharge plugin
{
