...
function balise_FORMULAIRE_ABONNEMENT_COMPLET ($p) {
	return calculer_balise_dynamique($p, 'FORMULAIRE_ABONNEMENT_COMPLET', array('id_liste'));
}
...
function balise_FORMULAIRE_ABONNEMENT_COMPLET_stat($args, $filtres) {

	if(!$args[1]) $args[1]='formulaire_abonnement_complet';
	preg_match_all("/liste([0-9]+)/x",
               $args[1], $matches);

	if($id_liste=intval($matches[1][0])) {
	$args[1]='formulaire_abonnement_complet';
	$args[0]=$id_liste;
	}
...
function balise_FORMULAIRE_ABONNEMENT_COMPLET_dyn($id_liste, $formulaire) {
...
function formulaire_inscription_auteurs_complets($type,$acces_membres,$formulaire) {
	...
	//ajouter une liste de spip-listes a laquelle les abonnes sont abonnes d'office (alertes)
	$id_liste_interne_abonnes = "80";
	$id_zone_abonnes = "1";
	...
	if ($mail_valide && $nom_inscription_) {
		...
		if ($row = spip_fetch_array($result)) {
			...
			if ($continue) {
				...
				"._q($url_organisation).","._q($telephone).","._q($fax).","._q($skype).","._q($adresse).","._q($codepostal).","._q($ville).",
				"._q($pays).","._q($latitude).","._q($longitude).","._q($nom_site).","._q($url_site).","._q($login_).","._q($mdpass).","._q($statut).",
				"._q($htpass).","._q($extras).","._q($cookie).")");
				...
				if ($row = spip_fetch_array($result)) {
					...
					//on abonne l'auteur au zones et listes internes
					if($id_liste_interne_abonnes != "") spip_query("INSERT INTO spip_auteurs_listes (id_auteur,id_liste) VALUES ("._q($id_auteur).","._q($id_liste_interne_abonnes).")") ;
					if($id_zone_abonnes != "") spip_query("INSERT INTO spip_zones_auteurs (id_auteur,id_zone) VALUES ("._q($id_auteur).","._q($id_zone_abonnes).")") ;
