<?php
#----------------------------------------------------------#
#  Plugin  : spipbb - Licence : GPL                        #
#  File    : spipbb_da : language file                     #
#  Authors : !tom!@!mir!.!dk! 2008 et als                  #
#  http://www.spip-contrib.net/Plugin-SpipBB#contributeurs #
#  Contact : chryjs!@!free!.!fr                            #
#----------------------------------------------------------#

// This is a SPIP-forum module file  --  Ceci est un fichier module de SPIP-forum

$GLOBALS[$GLOBALS['idx_lang']] = array(

// A

// Admin menus
'admin_cat_01_general' => 'Administration',
'admin_cat_outils' => 'Værktøj',
'admin_cat_spam' => 'Spam Ord',
'admin_action_01_configuration' => 'Konfigurer',
'admin_action_02_etat' => 'Forums overblik',
'admin_action_fromphpbb' => 'Importer fra PhpBB',
'admin_action_gestion' => 'Management',
'admin_action_swconfig' => 'Konfigurer',
'admin_action_swforum' => 'Flagede posts',
'admin_action_swlog' => 'Spam Log',
'admin_action_swwords' => 'Styr Ord',
'admin_action_ZZ_debug' => 'Debug',

// Admin interface
'admin_afficher_bouton_alerte_abus' => 'Vis misbrugs knapper',
'admin_affichier_bouton_rss' => 'Vis RSS knapper',
'admin_age_forum' => 'Forum alder År/må',
'admin_avatar_affiche' => 'Accepter og vis avatarer (default er Ja efter installering)',
'admin_avatar_taille_sujet' => 'Avatars\s størrelse (i pixels) på emne siden',
'admin_avatar_taille_contact' => 'Avatars\s størrelse (i pixels) på kontakt siden',
'admin_avatar_taille_profil' => 'Avatars\s størrelse (i pixels) på profil siden',
'admin_average_posts'=> 'Daglige indlæg snit',
'admin_average_users' => 'Daglige påskrifter snit',
'admin_config_prerequis' => 'behov',
'admin_config_spam_words' => 'Anti-spam admin',
'admin_config_spipbb' => 'tænd SpipBB',
'admin_config_spipbb_info' => 'Vælg Ja til at tænde SpipBB',
'admin_config_tables_ok' => 'SpipBB database tabeller: OK',
'admin_date_ouverture' => 'Åbnings dato',
'admin_form_action' => 'Valg',
'admin_form_creer_categorie' => 'Lav en ketegori',
'admin_form_creer_forum' => 'Lav et forum',
'admin_form_deplacer' => 'Flyt',
'admin_form_descendre' => 'Ned',
'admin_form_editer' => 'Rediger',
'admin_form_messages' => '&nbsp;',
'admin_form_monter' => 'Op',
'admin_form_sujets' => '&nbsp;',
'admin_forums_configuration' => 'Konfigurer SpipBB',
'admin_forums_configuration_specifique' => 'Bestemte Specific emner',
'admin_forums_configuration_avatar' => 'General konfigurering af avatarer',
'admin_forums_titre' => 'Forums admin',
'admin_infos' => 'SpipBB - Admin - Opsummering',
'admin_interface' => 'Offentlig brugerflade muligheder',
'admin_nombre_lignes_messages' =>'Antal linnier i et indlæg',
'admin_plugin_requis_erreur' => 'mangler påkrævet Plugin',
'admin_plugin_requis_ok' => 'påkrævet Plugin installered',
'admin_sous_titre' => 'Gå til SpipBB forums admin panel',
'admin_spipbb_release' => 'SpipBB version',
'admin_spip_forums_erreur' => '<b>Fejl: Du skal tænde for SPIPs offentlige forums.</b>',
'admin_spip_forums_ok' => 'SPIPs offentlige forums er tændt.',
'admin_spip_mots_cles_erreur' => '<b>Fejl: Du skal tænde for SPIPs keywords</b>',
'admin_spip_mots_cles_ok' => 'SPIPs Keywords er tændt',
'admin_spip_mots_forums_erreur' => '<b>Fejl: Du skal tænde for SPIPs forum keywords </b>',
'admin_spip_mots_forums_ok' => 'SPIPs Keywords er tændt',
'admin_statistique' => 'Information',
'admin_surtitre' => 'Forums styring',
'admin_temps_deplacement' => 'Tid før en admin kan flytte',
'admin_titre' => 'SpipBB Admin',
'admin_total_posts' => 'Total antal indlæg',
'admin_total_users' => 'Antal medlemmer',
'admin_total_users_online' =>'Medlemmer Online',
'admin_valeur' => 'Værdi',

'aecrit' => 'wrote',
'alerter_abus' => 'Dette indlæg er fornærmende...', ## GAF 0.5
'alerter_sujet' => 'Fornærmende indlæg', ## GAF 0.5
'alerter_texte' => 'Vær venligst opmærksom på følgende indlæg:', ## GAF 0.5
'annonce' => 'Meddelelse',
'annonce_dpt' => 'Meddelelse: ',
'anonyme' => 'Anonym',
'auteur' => 'Forfatter',
'avatar_saisie_url' => 'Din avatar\'s URL (http://... ...)', ## GAF 0.6

// B

//C
'champs_obligatoires'=>'Felter markeret med en * skal udfyldes.',
'chercher' => 'Søg',
'choix_mots_annonce' => 'lav et meddelelse',
'choix_mots_ferme' => 'Luk tråden',
'choix_mots_postit' => 'Placer en sticky',
'choix_mots_selection' => 'Denne keyword gruppe skal mindst indeholde tre keywords. Normalt, vil dette plugin lave dem under installation. SpipBB har brug for - generalt - ordene {ferme} (lukket), {annonce} (kundgøre) and {postit} (postering) keywords, men du kan vælge andre.',
'choix_rubrique_selection' => 'Vælg det afsnit (rubrique) der skal være det afsnit der bliver dit forum. I denne , vil hver underafsnit være et "underforum" (en forum group), hver ny offentliggjort artikel åbner et nyt forum.',
'choix_squelettes' => 'Du må gerne bruge andre lay out (Squelette), husk blot at filerne der skal erstatte groupeforum.html og filforum.html skal ekistere når du skifter!',
'citer' => 'citer',
'config_choix_mots' => 'Vælg keyword gruppen',
'config_choix_rubrique' => 'Vælg det afsnitsom spipBB bruger',
'config_choix_squelettes' => 'Vælg lay out (Squelette)',
'config_spipbb' => 'Grundliggende spipBB konfigurering for at kunne etablere et forum med dette plugin.',
'contacter' => 'Kontakt',
'contacter_dpt' => 'Kontakt: ',

//D
'dans_forum' => 'i dette forum',
'deconnexion_' => 'Log af ',
'deplacer_confirmer' => 'Bekræft flytning',
'deplacer_sujet_dpt' => 'Flytning af:',
'deplacer_dans_dpt' => 'Flyt til forum:',
'deplacer_vide' => 'Der er ikke noget forum tilbage, det er ikke muligt at flytte denne tråd!',
'diviser_confirmer' => 'Del emne kontrol panel',
'diviser_expliquer' => 'Brug af denne form kan dele emnet i to dele, enten ved at vælge indlæg eller dele den valgte post i to.',
'diviser_dans_dpt' => 'Forum til nyt emne:',
'diviser_separer_choisis' => 'Del valgte indlæg',
'diviser_separer_suite' => 'Del fra valgte indlæg',
'diviser_selection_dpt' => 'Vælg:',
'diviser_vide' => 'Der er ikke andre forum tilbage: det er ikke muligt at dele det!',

//E
'email' => 'E-mail',
'emploi' => 'Job',
'en_ligne' => 'Hvem er online?',
'en_reponse_a' => 'Besvar indlæg',
'en_rep_sujet_' => '&nbsp;:::&nbsp;Emne : ',

//F

'fiche_contact' => 'Kontakt form',
'forum' => 'Forums',
'forum_dpt' => 'Forum: ',
'forum_ferme' => 'Dette forum er slukket',
'forum_ferme_texte' => 'Dette forum er slukket. Du kan ikke skrive til det mere.',
'forum_maintenance' => 'Dette forum er lukket for vedligeholdelse', ## GAF 0.6
'fromphpbb_base' => 'Database navn:',
'fromphpbb_choix_test' => 'Kør en "blank test import" (default valg):',
'fromphpbb_choix_test_titre' => 'Start rigtig eller test import',
'fromphpbb_erreur_db_phpbb_config' => 'umuligt at læse config værdier i phpBB databasen',
'fromphpbb_erreur_db_phpbb' => 'umuligt at få kontakt med phpBB databasen',
'fromphpbb_erreur_db_spip' => 'Umuligt at forbinde med SPIP databasen',
'fromphpbb_erreur_forums' => 'Umuligt at overføre fora',
'fromphpbb_fichier' => 'PhpBB config file fundet:',
'fromphpbb_host' => 'Servers navn/IP addresse',
'fromphpbb_login' => 'Login:',
'fromphpbb_parametres_base' => 'Vælg enten stien til phpBB config filen eller udfyld phpBB databasens access parameters:',
'fromphpbb_parametres_rubrique' => 'Vælg i hvilken sektor phpBB forums skal importeres til',
'fromphpbb_parametres_titre' => 'Konfigurer adgang til phpBB databasen',
'fromphpbb_password' => 'Password:',
'fromphpbb_prefix' => 'Tables prefix:',
'fromphpbb_racine' => 'Sti til phpBB (avatarer):',
'fromphpbb_table' => 'PhpBB config table fundet:',
'fromphpbb_titre' => 'Importer fra et PhpBB forum',
'fromphpbb_titre_etape' => 'Importer fra et PhpBB forum - skridt',

//G

//H

//I
'info' => 'Information',
'info_confirmer_passe' => 'bekræft nyt password:',
'info_inscription_invalide' => 'Umuligt at registrere',
'info_plus_cinq_car' => 'mere end fem bogstaver/tal',
//'infos_refus_suivi_sujet' => '',
//'infos_suivi_forum_par_inscription' => '',
'inscription' => 'Registrer',
'inscrit_s' => 'Registreret',

'ip_informations' => 'IP Informationer',
'ip_adresse_post' => 'IP addresse for dette indlægt',
'ip_adresse_membres' => 'Bruger skriver fra denne IP address',
'ip_adresse_autres' => 'Andre IP addresses denne bruger har brugt',


//L
'liste_inscrits' => 'Medlems liste',
'localisation' => 'Localisering',
'login' => 'Forbind',
'loisirs' => 'Fritid',

//M

'membres_inscrits' => 'registrerede medlemmer',
'membres_les_plus_actifs' => 'Mest aktive medlemmer',
'message' => 'indlæg',
'messages' => 'Indlæg',
'message_s' => 'indlæg',
'messages_derniers' => 'Seneste indlæg',
'message_s_dpt' => 'Indlæg: ',
'messages_voir_dernier' => 'Spring til seneste indlæg',
'messages_voir_dernier_s' => 'Læs seneste indlæg',
'moderateur_dpt' => 'Moderator: ', ## GAF 0.6
'moderateurs' => 'Moderatore',
'moderateurs_dpt' => 'Moderatore: ', ## 0.5
'modif_parametre' => 'Skift dine parametre',
'mot_' => 'Ord',
//'mot_groupe_moderation' => '',

//N
'nom_AIM'=>'chat (AIM)',
'no_message' => 'Ingen indlæg indeholder dette ord (søge kritierie)',
'nom_MSNM'=>'chat (MSN Messenger)',
'nom_util' => 'Medlems navn',
'nom_Yahoo'=>'chat (Yahoo)',
'non' => 'Nej',
'numero_icq'=>'chat (ICQ)',

//O
'oui' => 'ja',

//P
'pagine_page_' => '.. side ',
'pagine_post_s' => 'Indlæg',
'pagine_sujet_s' => 'emner',
'par_' => 'af',
'poster_date_' => 'Skrevet den: ',
'poster_message' => 'Skriv et indlæg',
'postit' => 'Postering',
'postit_dpt' => 'Postering: ',
'post_titre' => '&nbsp;:::&nbsp;Titel: ',
'previsualisation' => 'Preview',
'profil' => 'Profil',

//R
'raison_clic' => 'klik her',
'raison_texte' => 'At kende grunden',
'recherche' => 'Søg',
'recherche_elargie' => 'Advanceret søgning',
'refus_suivi_thread' => '(don\'t remind) Do no modify!', ## GAF 0.6
'reglement'=>'<p>Regler: Vi sletter alt vi finder ufedt.
Forum registrerer IP addresser, og vil blokere disse ved misbrug.
Vi bruger ikke din mail adresse til andet end at sende dig et password.</p>',
'reponse_s_' => 'Svar',
'resultat_s_pour_' => ' resultat',
'retour_forum' => 'Tilbage til forum forside',

//S

's_abonner_a'=> 'RSS: aboner på denne tråd',
'signature_saisie_texte' => 'Skriv din signatur', ## GAF 0.6+
'sign_tempo' => 'Bygget med <a href="http://www.spip-contrib.net/Plugin-SpipBB#contributeurs" class="copyright">SpipBB</a>',
'sinscrire'=> 'Registrer',
'site_propose' => 'Foreslået site',
'site_web' => 'site',
'squelette_filforum' => 'Layout for tråde:',
'squelette_groupeforum' => 'Layout for grupper:',
'statut' => 'Status',
'statut_admin' => 'Admin',
'statut_redac' => 'Moderator',
'statut_visit' => 'Medlem',
'sujet_auteur' => 'Forfatter',
'sujet_clos_texte' => 'Dette emne er lukket, du kan ikke kommentere.',
'sujet_clos_titre' => 'Emne lukket',
'sujet_dpt' => 'Emne: ',
'sujet_nouveau' => 'Nyt emne',
'sujet_repondre' => 'Svar',
'sujets' => 'Emner',
'sujet_s' => 'emner',
'sujets_aucun' => 'Der er ingen indlæg i dette forum endnu, skriv løs!',

'sw_admin_can_spam' => 'Tillad admin at skrive spam ord',
'sw_admin_no_spam' => 'Ingen spam',
'sw_ban_ip_titre' => 'Bloker IP også?',
'sw_config_exceptions_titre' => 'Undtagelser',
'sw_config_exceptions' => 'Du kan lave undtagelsr for priviligerede medlemmer her. Medlemmer der møder disse kriterier vil blive tilladt at skrive bestemte (spam) ord.',
'sw_config_generale' => 'Nuværende aktiverede spam ord:',
'sw_config_generale_titre' => 'Generelle spam filter indstillinger',
'sw_config_warning' => 'Her kan du definere den tekst som du vil "PM" (personlig besked) til dine brugere,k hvis du ønsker at advare dem via "PM" når de bruger spam ord (max. 255 bogstaver).',
'sw_config_warning_titre' => 'Privat besked advarsels indstillinger',
'sw_disable_sw_titre' => '<strong>Tænd Spam ord filter</strong><br />Hvis du har brug for at bypasse dette filter,<br />sæt det til "nej".',
'sw_modo_can_spam' => 'Tilad moderatorer at bruge spam ord',
'sw_nb_spam_ban_titre' => 'Atal gange reglerne overtrædes før brugeren automatisk er blokeret',
'sw_pm_spam_warning_message' => 'Dette er en advarsel. Du har brugt et eller flere ord vi definerer som spam. Hold op med det.',
'sw_pm_spam_warning_titre' => 'Advarsel.',
'sw_send_pm_warning' => 'Send en "PM" til brugeren og advar dem om at deres indlæg indeholder spam ord',
'sw_spam_forum_titre' => 'Styr flagede indlæg',
'sw_spam_titre' => 'Spam filtrering',
'sw_spam_words_action' => 'Fra dette kontrolpanel kan du tilføje, fjerne og redigere spam ord. Wildcards (*) accepteres i feltet, for eksempel *test* tager alle ord, hvor test indgår i.',
'sw_spam_words_mass_add' => 'Skriv alle de spam ord ind i spam ords listen i dette tekstområde. Seperer hvert ord md et komma, et semi komma eller et line-break',
'sw_spam_words_titre' => 'Spam ord filter',
'sw_spam_words_url_add' => 'skriv URL på filen på den rigtige måde. Som eksempel: http://spipbb.spip-zone.info/plugins/spipBB/base/spamwordlist.csv .',
'sw_warning_from_admin' => 'Vælg den administrator der er valgt som "PM" afsender',
'sw_warning_pm_message' => 'Privat besked tekst',
'sw_warning_pm_titre' => 'Privat besked emne',
'sw_word' => 'Ord',


//T
'titre_spipbb' => 'SpipBB',
'total_membres' => 'Vi har totalt ',
'total_messages_membres' => 'Medlemmerne har totalt skrevet',
'tous' => 'Alle',
'tous_forums' => 'Alle forums',

//V
'visible_annuaire' => 'Optræd på den (offentlige) medlemsliste', ## GAF 0.6
'visible_annuaire_forum' => 'Optræd på medlemslisten', ## GAF 0.6
'visites' => 'Besøg',
'votre_bio' => 'Kort om dig.',
'votre_email' => 'Din mail',
'votre_nouveau_passe' => 'Nyt password',
'votre_signature' => 'Din signatur:', ## GAF 0.6
'votre_site' => 'Dit site',
'votre_url_avatar' => 'Din avatars URL(http://...)', ## GAF 0.6
'votre_url_site' => 'Dit sites (URL)'


);

?>
