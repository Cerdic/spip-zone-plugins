...
(function($) {
	...
	$.Tooltip = {
		...
		hide:function(){
			...
			helper.parent.hide().removeClass( $.Tooltip.current.tSettings.extraClass );
			
			if( $.Tooltip.current.tSettings.fixPNG )
	...
	function save() {
		...
		if ( $.Tooltip.blocked || this == $.Tooltip.current || !this.tooltipText )
			return;
	...
	function update(event)	{
		...
		if(event) {
			...
			left = event.pageX + $.Tooltip.current.tSettings.left;
			top = event.pageY + $.Tooltip.current.tSettings.top;
			helper.parent.css({
				left: left + 'px',
				top: top + 'px'
			});
		...
		if(v.x + v.cx < h.offsetLeft + h.offsetWidth) {
			left -= Math.min(h.offsetWidth + 20 + $.Tooltip.current.tSettings.left,h.offsetLeft);
			helper.parent.css({left: left + 'px'});
		}
		// check vertical position
		var css = {}, delta = {};
		if(v.y + v.cy < h.offsetTop + h.offsetHeight) {
			delta.y = - (h.offsetHeight + 20 + $.Tooltip.current.tSettings.top)
			css.top =  top + delta.y;
			//helper.parent.css({top: top + 'px'});
		} 
		if(h.offsetTop+delta.y<v.y){
			delete(css.top); 
			css.left = left - Math.min(h.offsetWidth + 20 + $.Tooltip.current.tSettings.left,h.offsetLeft);
			//helper.parent.css({left: left + 'px'});
		}
		helper.parent.css($.each(css,function(i,n){css[i]=n+"px"}));
	...
	function hide(event) {
		if($.Tooltip.blocked || (helper.parent.is(":visible") && $.Tooltip.persistent))
			return;
		// clear timeout if possible
		if(tID)
			clearTimeout(tID);
