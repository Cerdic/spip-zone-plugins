<?php
	...
	function afficher_abonne_boucle($row, $own) {
		...
		if ($id_lettre) {
			$clics = sql_countsel('AC.id_clic', 'spip_abonnes_clics AS AC INNER JOIN spip_clics AS C ON C.id_clic=AC.id_clic', 'C.id_lettre='.intval($id_lettre).' AND AC.id_abonne='.intval($abonne->id_abonne));
			if ($clics == 1)
