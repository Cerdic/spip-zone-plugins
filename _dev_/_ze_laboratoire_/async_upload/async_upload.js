...
jQuery.fn.async_upload = function(add_function) {
  ...
  function do_async_upload(form) {
    ...
    if (!form.jFrame) {
      form.jFrame = $("<iframe id='upload_frame"+num+"' name='upload_frame"+num+"' frameborder='0' marginwidth='0' marginheight='0' scrolling='yes' style='position:absolute' onload='this.iframeload()'></iframe>")
      .appendTo("body");
    }
    
    //IE apparently do not write anything in an iframe onload event handler 
    form.jFrame[0].iframeload = function() {
