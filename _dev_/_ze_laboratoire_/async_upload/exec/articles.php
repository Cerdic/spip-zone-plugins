...
function articles_documents($flag_editable, $type, $id)
{
	...
	if  ($GLOBALS['meta']["documents_$type"]!='non' AND $flag_editable) {
		...
		$res .= $f('articles', "id_article=$id", $id, _T('info_telecharger_ordinateur'), 'document', 'article','',0,generer_url_ecrire("documenter","id_article=$id&type=$type",true));
      ...
      iframe[0].iframeload = function() {
          ...
          if(res.is(".upload_document_added")) {
            res.find(">div").each(function(){
              var cont = $("#"+this.id);
              if(!cont.size()) {
                cont = $(this.id.search(/--/)!=-1 ? "portfolio":"documents").parent();
              }
