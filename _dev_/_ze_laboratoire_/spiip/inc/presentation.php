...
function afficher_tranches_requete($num_rows, $colspan, $tmp_var, $url='', $nb_aff = 10) {
	...
	for ($i = 0; $i < $num_rows; $i += $nb_aff){
			...
			if ($deb_aff + 1 >= $deb AND $deb_aff + 1 <= $fin) {
				...
				if ($url) {
					...
					. '\');"';
					*/
					$ajax="class=\"ajax\" id=\"$url:$tmp_var:".($deb-1)."--$tmp_var\" ";
				}
				$texte .= "<a {$ajax}href=\"$script#a$ancre\">$deb</a>";
			}
	}
	...
	if ($deb_aff == -1) {
			...
			if ($url) {
				...
				. '\');"';
				*/			
				$ajax="class=\"ajax\" id=\"$url:$tmp_var:-1--$tmp_var\" ";
			}
			$texte .= "<a {$ajax}href=\"$script#a$ancre\"><img src='". _DIR_IMG_PACK . "plus.gif' title='"._T('lien_tout_afficher')."' /></a>";
	}
...
function afficher_articles($titre_table, $requete, $afficher_visites = false, $afficher_auteurs = true, $obligatoire = false, $afficher_cadre = true, $afficher_descriptif = true) {
	...
	if ($cpt > $nb_aff) {
		$nb_aff = (_TRANCHES); 
		$tranches = afficher_tranches_requete($cpt, $afficher_auteurs ? 4 + $ajout_col : 3 + $ajout_col, $tmp_var, generer_url_ecrire_ajax('memoriser',"id_ajax_fonc=$id_ajax_fonc",true), $nb_aff);
...
function afficher_articles_trad($titre_table, $requete, $afficher_visites = false, $afficher_auteurs = true,
	...
	if ($cpt > $nb_aff) {
		$nb_aff = (_TRANCHES); 
		$tranches = afficher_tranches_requete($cpt,  4, $tmp_var, generer_url_ecrire_ajax('memoriser', "id_ajax_fonc=$id_ajax_fonc"), $nb_aff);
	} else $tranches = '';
	...
	if (!$deb_aff) {
			...
			$texte_img .= "<div style='float: $spip_lang_right;'><a  class='ajax' id='".generer_url_ecrire_ajax('memoriser',"id_ajax_fonc=$id_ajax_trad")."--$div_trad' style='visibility:hidden'><img src='". _DIR_IMG_PACK . "langues-off-12.gif' /></a></div>";
...
function afficher_breves($titre_table, $requete, $affrub=false) {
	...
	return affiche_tranche_bandeau($requete, "breve-24.gif", 3, $couleur_foncee, "white", $tmp_var, $titre_table, false, $largeurs, $styles, 'afficher_breves_boucle', array( $afficher_langue, $affrub, $langue_defaut));
...
function afficher_auteurs ($titre_table, $requete) {
	...
	$tmp_var = 't_' . substr(md5(join('', $requete)), 0, 4);
	$nb_aff = floor(1.5 * _TRANCHES);
	$deb_aff = intval(_request($tmp_var));	$tranches = '';
	if ($cpt > $nb_aff) {
		$nb_aff = (_TRANCHES); 
...
function debut_page($titre = "", $rubrique = "accueil", $sous_rubrique = "accueil", $onLoad = "", $css="", $id_rubrique = "") {
	...
	echo recuperer_fond('dist_back/debut_page',array(
