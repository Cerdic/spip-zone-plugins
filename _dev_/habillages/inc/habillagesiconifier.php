...
include_spip('inc/habillages_actions');

function inc_habillagesiconifier_dist($id_objet, $id,  $script) {
	...
	if (!$logo = $logo_f($id, $id_objet, 'on')) {
		$masque = habillages_indiquer_logo($texteon, $id_objet, 'on', $id, $script);
		$res = block_parfois_visible('on', "<b>$texteon</b>", $masque, '', $visible=true);
	} else {
		list($img, $clic) = habillages_decrire_logo($id_objet,'on',$id, 170, 170, $logo, $texteon, $script);
		...
		if ($logo = $logo_f($id, $id_objet, 'off')) {
			...
			list($img, $clic) = habillages_decrire_logo($id_objet, 'off', $id, 170, 170, $logo, $texteoff, $script);

			$masque = block_parfois_visible('off', "<b>$texteoff</b><p>$img</p>", $clic, 'margin-bottom: -2px');

			$res .= "<center>$masque</center>";
		} else {
		  $masque = habillages_indiquer_logo($texteoff, $id_objet, 'off', $id, $script);
		  $res .= block_parfois_visible('off', "<b>$texteoff</b>", $masque, '', $visible=true);
		}
	}
...
function habillages_indiquer_logo($titre, $id_objet, $mode, $id, $script) {

	global $formats_logos;
	$dir_ftp = habillages_determine_upload();
	$afficher = "";
	...
	if (!$afficher) {
		...
		$afficher = "\n" .
			"\n<input name='image' type='File' class='forml' style='font-size:9px;' size='15' value='../plugins/habillages/habillages-data/logos/logoon.png'>" .
...
function habillages_decrire_logo($id_objet, $mode, $id, $width, $height, $img, $titre="", $script="") {
