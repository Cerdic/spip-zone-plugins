...
function creer_chaine_url($x) {
  ...
  if (_urls_en_minuscules) {
    // Sans doute des effets de bord sur les caracteres non ISO-8859, attention
    $url = strtolower($url);
  }
  $x['data'] = $url;
...
function _generer_url_propre($type, $id_objet) {
  ...
  // Gestion des URL arborescentes sur les objets qui le permettent
  if (_urls_propres_arborescentes) {
    if (in_array($type, array('rubrique', 'article', 'breve', 'site', 'mot'))) {
      //  Recuperer le parent de l'objet.
      switch($type) {
        case 'rubrique':
          $parent_type = 'rubrique';
          $parent_col_id = 'id_parent';
          break;
        case 'article':
        case 'breve':
        case 'site':
          $parent_type = 'rubrique';
          $parent_col_id = 'id_rubrique';
          break;
        case 'mot':
          $parent_type = 'groupe_mots';
          $parent_col_id = 'id_groupe';
          break;
      }
      $parent_row = sql_fetsel($parent_col_id, $table, $col_id.'='.$id_objet, '', '', 1);
      // Creer l'URL du parent si ce n'est pas la racine
      if ($parent_type == 'rubrique' && $parent_row[$parent_col_id] == 0) {
        $parent_url = '';
      } else {
        $parent_url = _generer_url_propre($parent_type, $parent_row[$parent_col_id]);
        $url = $parent_url.$url;
      }
    } elseif ($type == 'auteur') {
      $url = 'auteurs/'.$url;
    }
    if (in_array($type, array('rubrique', 'groupe_mots'))) {
      $url = $url.'/';
    }
  }
...
function _generer_url_complete($type, $id, $args='', $ancre='') {
  ...
  if ($propre = _generer_url_propre($type, $id)) {
    $url = _debut_urls_propres
      ...
      . $propre
      . $marqueur2;
    if (!(_url_propres_arborescentes && in_array($type, array('rubrique', 'groupe_mots', 'mot')))) {
      $url .= _terminaison_urls_propres;
    }
...
// http://doc.spip.org/@generer_url_groupe_mots
function generer_url_groupe_mots($id_groupe, $args='', $ancre='') {
  return _generer_url_complete('groupe_mots', $id_groupe, $args, $ancre);
}
...
function generer_url_document($id_document, $args='', $ancre='') {
  ...
  $url = generer_url_document_dist($id_document, $args, $ancre);
  // Repositionne l'URL par rapport a la racine du site (#GLOBALS)
  $url = str_repeat('../', $GLOBALS['profondeur_url']).$url;
  return $url;
}
