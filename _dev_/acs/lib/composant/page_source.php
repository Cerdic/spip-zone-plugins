...
function widgy($composant, $param, &$vars, $label='', $indentation='', $nic = '') {
  ...
  $r = '<table><tr valign="top"><td>'.$indentation.'</td><td><b><a class="'.get_widget_class($composant, 'widgy').'" href="?exec=acs&onglet=composants&composant='.$composant.($nic ? '&nic='.$nic : '').'" title="'._T('acs:composant').' '.$label.($nic ? ' '.$nic : '').'">'.$label.'</a></b></td><td>';
