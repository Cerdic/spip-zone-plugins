...
function pi_INCLURE($args) {
  ...
  if (substr($include, 0, 11) == 'composants/') {
    $r = affiche_widgy($include, $param, $args[1]['vars'], indent($args[1]['indentation']));
  }
  else {
...
function affiche_widgy($include, $param, &$vars, $indentation) {
  ...
  return widgy($composant, $param, $vars, $label, $indentation);
}

function widgy($composant, $param, &$vars, $label='', $indentation='') {
  $label = ucfirst(str_replace('_', ' ', $label));
  $r = '<table><tr valign="top"><td>'.$indentation.'</td><td><b><a class="'.get_widget_class($composant, 'widgy').'" href="?exec=acs&onglet=composants&composant='.$composant.'" title="'._T('acs:composant').' '.$label.($param ? ' '.$param : '').'">'.$label.'</a></b></td><td>';
    ...
    foreach($vars as $varname=>$v) {
      if (($v['composant'] == $composant) && ($v['type'] == 'widget')) {
        ...
        if (isset($GLOBALS['meta'][$var]) && $GLOBALS['meta'][$var]) {
          $r .= '<table><tr><td><a class="nompage" href="?exec=acs&onglet=composants&composant='.$composant.'" title="'._T('acs:variable').'">'.$varname.'</a> : </td><td>'. widgy($GLOBALS['meta'][$var], '', $vars, $GLOBALS['meta'][$var]).'</td></tr></table>';
        }
      }
    }
  }
...
function liste_balises() {
  $balises_acs = 'INTRO, ACS_CHEMIN, ACS_DERNIERE_MODIF';
