...
function inc_geomap_config(){
		...
		$out = "";
		$out .= debut_cadre('r', _DIR_PLUGIN_GIS."img_pack/correxir.png");
		if(_request('ok')){
		...
		$out .= '<br/>';
		$out .= '<a href="http://www.google.com/apis/maps" target="_blank" ><img src="'._DIR_PLUGIN_GIS.'img_pack/logo_google.gif" border="0" align="left" hspace="10" ></a>';
		$out .= '<form name="googlemapkey" method="post" action="'.generer_url_ecrire('gis').'">';
		$out .= '<br/>';
		$out .= '<label>Google Map API Key <a href="http://www.google.com/apis/maps/signup.html" target="_blank" >'._T('gis:conseguir').'</a></label> <input type="text" name="key" value="'.$apikey.'" size="30" />';
		$out .= '<input type="submit" name="ok" value="ok" />';
		
		if(_request('ok')){
			$out .= '<div align="center" style="margin:20px auto">';
			$out .= ''._T('gis:clave_engadida').'<code>'._request('key').'</code>';
			$out .= '</div>';
		}
		if (strlen($apikey)){
			if(_request('choisir')){
			...
			. "<p>"._T('gis:default_geoloc')."</p>"
			. "<div id='formMap' name='formMap' style='width: 470px; height: 350px;margin:10px auto;'></div>"
			. $gis_append_clicable_map('formMap','form_lat','form_lonx',$glat,$glonx,'form_zoom',$gzoom,false);
