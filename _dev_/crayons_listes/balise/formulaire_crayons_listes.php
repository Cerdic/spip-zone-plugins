...
function balise_FORMULAIRE_CRAYONS_LISTES ($p) {

	return calculer_balise_dynamique($p, 'FORMULAIRE_CRAYONS_LISTES', array());
}

function balise_FORMULAIRE_CRAYONS_LISTES_stat ($args, $filtres) {
	
	if(!$args[0]) $args[0]='formulaire_crayons_listes';
	return array($args[0]);
}



function balise_FORMULAIRE_CRAYONS_LISTES_dyn($formulaire) {
...
$formulaire = "formulaires/formulaire_crayons_listes";	
	...
	if (($GLOBALS['auteur_session'] ? ($id_auteur = $GLOBALS['auteur_session']['id_auteur'] + 0) : '')
		|| ($d  AND ($confirm != 'oui'))) {
	
		if ($id_auteur || (
			($res = spip_query(
				"SELECT * FROM spip_auteurs WHERE cookie_oubli=" . _q($d) .
					" AND statut<>'5poubelle' AND pass<>''"))
