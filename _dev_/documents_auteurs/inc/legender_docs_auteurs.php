...
function inc_legender_docs_auteurs_dist($id_document, $document, $script, $type, $id, $ancre, $deplier=false) {
	...
	if (preg_match('/_edit$/', $script)){
		if ($id==0)
			$action = redirige_action_auteur('supprimer_docs_auteurs', "document-$id_document", $script, "id_$type=$id#$ancre");
		else
...
function vignette_formulaire_legender($id_document, $document, $script, $type, $id, $ancre)
{
	...
	if (preg_match('/_edit$/', $script)) {
		...
		$action = redirige_action_auteur('supprimer_docs_auteurs', "document-$id_vignette", $script, "id_$type=$id&show_docs=$id_document#$ancre");
	} else {
