...
//////////////////////////
	...
	foreach ($choses as $chose){
		$chaine1=$chose;
		$chaine2='_lies_mot';
		if (($chose=='rubriques')||($chose=='breves'))
			$chaine2='_liees_mot';
		elseif ($chose=='syndic')
			$chaine1='sites';
		elseif ($chose=='forum')
//Attention conflit avec afficher_forum !
			$chaine1='forums';
		$f='afficher_'.$chaine1;

		$from='spip_'.$chose.' AS '.$chose.', spip_mots_'.$chose.' AS lien';
		$where="lien.id_mot='$id_mot' AND lien.".$choses_possibles[$chose]['id_chose']."=".$chose.".".$choses_possibles[$chose]['id_chose'];

		 if ($choses_possibles[$chose]['statut_'.$GLOBALS['connect_statut']])
		 	$where.=" AND chose.statut IN (".$choses_possibles[$chose]['statut_'.$GLOBALS['connect_statut']].")";
