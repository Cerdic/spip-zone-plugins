...
function exec_echoppe_edit_produit(){
	...
	if (spip_num_rows($res_le_produit) == 0 && $contexte['new'] != "oui"){
	...
	$sql_description_produit = "SELECT * FROM spip_echoppe_produits_descriptions WHERE id_produit = '".$id_produit."' AND lang = '".$lang_produit."';";
	...
	(is_array($description_produit))?array_merge($contexte,$description_produit):$contexte = $contexte;
