...
// modif pour accesgroupes
// inclure le fichier de redéfinitions des BOUCLES pour utiliser ses fcts de filtrage des requètes pour l'espace privé
...
// modif pour accesgroupes : filtrer les rubriques restreintes directement dans la requete SQL
	$sql_rub_restreintes = accesgroupes_rubriques_accessibles_where('rub1.id_rubrique ', 'prive');
//echo '$sql_rub_restreintes = '.$sql_rub_restreintes;	
 
	$sql = "SELECT rub1.id_rubrique, rub1.titre, rub1.id_parent, rub1.lang, rub1.langue_choisie 
			 	 						 FROM spip_rubriques AS rub1, spip_rubriques AS rub2 
										 WHERE ((rub1.id_parent = $id_rubrique) 
										 OR (rub2.id_parent = $id_rubrique 
										 AND rub1.id_parent=rub2.id_rubrique)) 
										 AND rub1.id_rubrique != $exclu
										 AND $sql_rub_restreintes 
