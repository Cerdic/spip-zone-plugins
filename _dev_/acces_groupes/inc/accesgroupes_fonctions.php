...
// 		 pour remplir le tableau des rubriques restreintes en utilisant le principe d'héritage des restrictions 
	function accesgroupes_liste_rubriques_restreintes($id_parent = 0, $prive_public = 'public'){
//echo '<br>debut accesgroupes_liste_rubriques_restreintes $prive_public = '.$prive_public;
...
//echo '<br>mysql_error $sql1 = '.mysql_error();					 
        			while ($row1 = spip_fetch_array($result1)) {
        					...
        					// si le parent ou le secteur est déja dans le tableau : vu le principe d'héritage pas la peine d'aller plus loin :)
    									if (accesgroupes_verif_acces($rub_ec, $prive_public) == 1 OR accesgroupes_verif_acces($rub_ec, $prive_public) == 2) {
...
//fct pour renvoyer le tableau des articles appartenant aux rubriques à accès restreint
  function accesgroupes_liste_articles_restreints($prive_public = 'public') {
  				 ...
  				 if (!is_array($Tart_restreints)) {
    				  ...
    				  $Trub_rest = accesgroupes_liste_rubriques_restreintes('', $prive_public);
...
// fct pour renvoyer le tableau des brèves appartenant à des rubriques à accès restreint
  function accesgroupes_liste_breves_restreintes($prive_public = 'public') {
...
// 		 (puissant le Cedric pour maximiser ce qui est fait par la requete mySQL !)
	function accesgroupes_liste_forums_restreints($prive_public = 'public') {
        	 static $Tforum_restreints;
        	 if (!is_array($Tforum_restreints)) {
        			$Tforum_restreints = array();
        			// rattaches aux rubriques
        			$liste_rub = accesgroupes_liste_rubriques_restreintes('', $prive_public);
        			$where = calcul_mysql_in('id_rubrique', join(",",$liste_rub));
        			// rattaches aux articles
        			$liste_art = accesgroupes_liste_articles_restreints($prive_public);
        			$where .= " OR " . calcul_mysql_in('id_article', join(",",$liste_art));
        			// rattaches aux breves
        			$liste_breves = accesgroupes_liste_breves_restreintes($prive_public);
...
// fct pour renvoyer le tableau des signatures des articles appartenant à une rubrique restreinte
  function accesgroupes_liste_signatures_restreintes($prive_public = 'public') {
...
//		 avec le petit array_keys() qui va bien pour récupérer le tableau de valeurs à la fin !
  function accesgroupes_liste_documents_restreints($prive_public = 'public') {
        	 ...
        	 if (!is_array($Tdocuments_restreints)){
        			...
        			$liste_syn = accesgroupes_liste_syndics_restreints($prive_public);
...
// fct pour renvoyer le tableau des rubriques syndiquées appartenants à une rubrique restreinte
  function accesgroupes_liste_syndics_restreints($prive_public = 'public') {
...
// fct pour renvoyer le tableau des articles syndiqués appartenant à une rubrique restreinte
  function accesgroupes_liste_syndic_articles_restreints($prive_public = 'public'){
...
// filtrage RUBRIQUES
	function accesgroupes_rubriques_accessibles_where($primary, $prive_public = 'public'){
//echo '<br>accesgroupes_rubriques_accessibles_where => $prive_public = '.$prive_public;			 
					 $liste = accesgroupes_liste_rubriques_restreintes('', $prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}

// filtrage ARTICLES
  function accesgroupes_articles_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_articles_restreints($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}

// filtrage BREVES
  function accesgroupes_breves_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_breves_restreintes($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}
	
// filtrage FORUMS
	function accesgroupes_forums_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_forums_restreints($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}

// filtrage SIGNATURES
	function accesgroupes_signatures_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_signatures_restreintes($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}

// filtrage DOCUMENTS
  function accesgroupes_documents_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_documents_restreints($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}

// filtrage SYNDICS
  function accesgroupes_syndics_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_syndics_restreints($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}

// filtrage ARTICLES SYNDIQUES
	function accesgroupes_syndic_articles_accessibles_where($primary, $prive_public = 'public'){
					 $liste = accesgroupes_liste_syndic_articles_restreints($prive_public);
					 return calcul_mysql_in($primary, join(",",$liste),"NOT");
	}
...
// filtre de test pour savoir si l'acces à un article est restreint dans la partie PUBLIQUE
