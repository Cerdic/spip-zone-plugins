...
function exec_mots_type_dist()
{
	...
	if (($id_groupe AND !$row) OR
		...
		$res = debut_cadre_relief($technique==''?"groupe-mot-24.gif":"mot-technique-24.png", true)
		...
		$res .= "<div style='padding: 5px; border: 1px dashed #aaaaaa; background-color: #dddddd;'>";
		$checked =  ($technique == "oui") ? "checked='checked'" : ''; 
		$res .= "<input type='checkbox' name='technique' value='oui' $checked id='technique' /> <label for='technique'>"
		. "<b>"._T('motstechniques:info_mots_cles_techniques')."</b>"
		._T('motstechniques:bouton_mots_cles_techniques')."</label><br />";
		// Par defaut, tous les groupes de mots cles techniques geres par l'interface de gestion des groupes de mots cles sont visibles dans les formulaires d'ajouts de mots cles
		$res .= "<input type='hidden' name='affiche_formulaire' value='oui' />";
