...
function action_a2a_lier_article(){
	...
	$sql_test_liaison = "SELECT * FROM spip_article_articles WHERE id_artile='".$contexte['id_article_source']."' AND id_article_cible='".$contexte['id_article_cible']."';";
	$res_test_liaison = spip_query($sql_test_liaison);
	if (spip_num_rows($res_test_liaison) == 0){
		$sql_liaison = "INSERT INTO spip_article_articles VALUES ('".$contexte['id_article_source']."', '".$contexte['id_article_cible']."');";
