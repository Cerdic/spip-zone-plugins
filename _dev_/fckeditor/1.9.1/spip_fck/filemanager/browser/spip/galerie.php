...
function sous_arborescence($id_rubrique) {
	...
	if (($nbSousRubriques + $nbDocumentsRubrique + $nbDocumentsArticles + $nbSites) > 0) {
		...
		while ($row = mysql_fetch_array($sousRubriques)) {
			...
			if ($content != '') {
				$retour .= '<a href="#" onclick="show(\'rub'.$row['id_rubrique'].'\');"><img src="'.$cheminEcrire.'img_pack/triangle-droite.gif" width="8" height="8" class="puce"></a>';
			} else {
				$retour .= '<img src="'.$cheminEcrire.'img_pack/rien.gif" width="16" height="14" />';
			}
			$retour .= '<img src="'.$cheminEcrire.'img_pack/rubrique-24.gif" align="absbottom" /> ';
			$retour .= '<a href="javascript:addDoc(\'spip.php?rubrique'.$row['id_rubrique'].'\');" title="Lier la rubrique">' . $row['titre'].' <img src="'.$cheminEcrire.'img_pack/plus.gif" align="bottom" border="0" /> <img src="'.$cheminEcrire.'img_pack/rubrique-12.gif" align="absbottom" border="0" /></a>';
			if ($content != '') {
		...
		if ($nbArticles > 0) {
			...
			while (list(, $article) = each($listeArticles)) {
				...
				$retour .= '<div class="indent">';
				$retour .= '<img src="'.$cheminEcrire.'img_pack/article-24.gif" align="absbottom" /> ';
				$retour .= '<a href="javascript:addDoc(\'spip.php?article'.$article['id'].'\');" title="Lier l\'article">' . $article['titre'] . ' <img src="'.$cheminEcrire.'img_pack/plus.gif" align="bottom" border="0" /> <img src="'.$cheminEcrire.'img_pack/puce-'.$image_article[$article['statut']].'.gif" align="bottom" border="0" /></a>';
				$retour .= '</div>';
			}
		}
		if($nbSites > 0) {
			...
			while (list(, $site) = each($listeSites)) {
				$retour .= '<div class="indent">';
				$retour .= '<img src="'.$cheminEcrire.'img_pack/site-24.gif" align="absbottom" /><a href="javascript:addDoc(\'spip.php?site'.$site['id'].'\');" title="Lier l\'outil">';
				$retour .= $site['titre'].' ';
				$retour .= '<img src="'.$cheminEcrire.'img_pack/plus.gif" align="bottom" border="0" /> <img src="'.$cheminEcrire.'img_pack/puce-'.$image_article[$site['statut']].'.gif" align="bottom" border="0" /></a>';
				$retour .= '</div>';
			}
		}
		$retour .= '</div>';
