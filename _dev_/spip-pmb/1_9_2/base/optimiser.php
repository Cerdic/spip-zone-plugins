...
//APA meme traitement des articles syndiqués en provenance de PMB

	# les articles syndiques appartenant a des sites effaces
	$res = spip_query("SELECT syndic_articles_pmb.id_syndic_article,syndic_articles_pmb.id_syndic
		      FROM spip_syndic_articles_pmb AS syndic_articles_pmb
		        LEFT JOIN spip_syndic AS syndic
		          ON syndic_articles_pmb.id_syndic=syndic.id_syndic
		       WHERE syndic.id_syndic IS NULL");

	while ($row = spip_fetch_array($res,SPIP_ASSOC))
		spip_query("DELETE FROM spip_syndic_articles_pmb
		WHERE id_syndic_article=".$row['id_syndic_article']
		." AND id_syndic=".$row['id_syndic']);
