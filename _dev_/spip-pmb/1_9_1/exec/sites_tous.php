...
debut_droite();
 ...
 afficher_sites(_T('titre_sites_syndiques'), array('FROM' => 'spip_syndic', 'WHERE' => "(syndication='oui' OR syndication='sus') AND statut='publie' AND id_syndic IN (select id_syndic from spip_syndic_articles)", 'ORDER BY' => "nom_site"));

 afficher_sites(_T('titre_sites_syndiques')." PMB", array('FROM' => 'spip_syndic', 'WHERE' => "(syndication='oui' OR syndication='sus') AND statut='publie' AND id_syndic IN (select id_syndic from spip_syndic_articles_pmb)", 'ORDER BY' => "nom_site"));
...
if ($options == 'avancees' AND $connect_statut == '0minirezo') {
 ...
 afficher_syndic_articles(_T('titre_dernier_article_syndique')." PMB", array('FROM' => 'spip_syndic_articles_pmb', 'ORDER BY' => "date DESC",  'LIMIT' => "50"));
