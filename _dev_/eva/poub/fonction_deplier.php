...
document.write('<div id=\"Layer".$numero_block["$nom_block"]."\" style=\"display:none\">');",
			      '',
...
function verif_butineur() {
	...
	if (eregi("opera", $browser_description)) {
		...
		$browser_layer = (($browser_version < 7) ? '' :  http_script('', find_in_path('fonction_deplier.js'),'')); // menu EVA
	}
	else if (eregi("msie", $browser_description)) {
		eregi("MSIE ([^;]*)", $browser_description, $match);
		$browser_name = "MSIE";
		$browser_version = $match[1];
		$browser_layer = (($browser_version < 5) ? '' :  http_script('', find_in_path('fonction_deplier.js'),'')); // menu EVA
		...
		$browser_name = "Safari";
		$browser_version = $match[1];
		$browser_layer = http_script('', find_in_path('fonction_deplier.js'),'');  // menu EVA
	}
