...
function debut_block_visible($nom_block){
	...
	return '<div id="Layer'.$numero_block["$nom_block"].'">';

}

function debut_block_invisible($nom_block, $rub2){ // OK
// La variable $rub2 est correctement récupérée depuis la page menu.html avec l'id_rubrique : OK
	global $numero_block, $compteur_block, $browser_layer;
	if (!$browser_layer) return '';
	if (!$numero_block["$nom_block"] > 0){
		$compteur_block++;
...
//echo '$rub2:'.($rub2 ? $rub2 : '#');
	return http_script("vis['".$numero_block["$nom_block"]."'] = 'hide'; document.write('<div id=\"Layer".$numero_block["$nom_block"]."\" style=\"display:none\">');",
			      '',
			   "<div id=\"Layer".$numero_block["$nom_block"]."\">");

}

function fin_block() {
  return (!$GLOBALS['browser_layer'] ? '' : "</div>");
}

function bouton_block_invisible($nom_block, $rub, $icone='') { // OK
// La variable est toujours passée
//echo '$bouton_block_invisible:'.$rub;
	global $numero_block, $compteur_block, $browser_layer, $spip_lang_rtl, $rub;
	...
	return produire_acceder_couche($javasc, $numero_block[$nom_block], $icone, $rub);
}

/////////////////////////////////////////////////////////////////////:
// Ne pas encore s'occuper de cette fonction
function bouton_block_visible($nom_block, $rub_invisible){ // NA
//echo '$rub_visible:'.($rub_visible ? $rub_visible : '#');
	...
	return produire_acceder_couche($javasc, $numero_block[$nom_block], "deplierbas.gif", $rub_invisible);
}
/////////////////////////////////////////////////////////////////////////

function produire_acceder_couche($couches, $nom, $icone, $rub) {
// Ne récupère la variable $rub qu'au chargement de la page. Les clics sur les triangles ne fonctionnent pas.
	global $spip_lang_rtl;
echo '$rub_acceder_couche:'.($rub ? $rub : '#');
	return http_script("acceder_couche([" . join(',',$couches) . '], ' .
			   $nom .
			   ", '" .
			   "plugins/eva/images/" . // menu EVA
			   "', '" .
			   $icone .
			   "', '" .
...
function verif_butineur() {
	...
	if (eregi("opera", $browser_description)) {
		...
		$browser_layer = (($browser_version < 7) ? '' :  http_script('', find_in_path('fonction_deplier2.js'),'')); // menu EVA
	}
	else if (eregi("msie", $browser_description)) {
		eregi("MSIE ([^;]*)", $browser_description, $match);
		$browser_name = "MSIE";
		$browser_version = $match[1];
		$browser_layer = (($browser_version < 5) ? '' :  http_script('', find_in_path('fonction_deplier2.js'),'')); // menu EVA
		...
		$browser_name = "Safari";
		$browser_version = $match[1];
		$browser_layer = http_script('', find_in_path('fonction_deplier2.js'),'');  // menu EVA
	}
