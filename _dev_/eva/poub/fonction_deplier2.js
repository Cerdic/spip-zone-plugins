// acceder_couche() du code HTML est la fonction qui appelle swap_couche() de la page rendue. La variable rub doit être passées à swap_couche à la fonction quand on clique sur les triangles et c'est php:function produire_acceder_couche qui appelle acceder_couche().
...
function swap_couche(couche, rtl, dir, no_swap, rub) {
	...
	/* alert("On va afficher la rubrique " + rub); */
	afficher_liste_articles(rub);
}
...
function acceder_couche(couches, n, dir, icone, texte, sens, rub) {
// 		if(rub == ""){alert("acceder_couche = " + rub);}
	  javasc = ''
	  for (j=0; j<couches.length; j+=1)
	  	javasc += 'swap_couche(' + couches[j][0] + ", '" + sens + "','" + dir + "', " + couches[j][1] + ", '" + rub +  "');";
	  /* alert(javasc); */
