...
if (preg_match('/\{\{\{/', $texte) || preg_match('/\{2\{/', $texte)){
	...
	$sommaire_complet.= "<h4 class='center'><a name='haut' class='sommaire_article'>Sommaire</a></h4>";
	$sommaire_complet.= "<ul>";
	...
	for($i=0; $i<count($texte_sommaire[0]); $i++){
		$sommaire_complet.= preg_replace(
			'/\{\{\{(.*?)\}\}\}/',
			'<li><strong><a href="#sommaire_'.$j.'">$1</a></strong></li>',
			preg_replace(
				'/(\{2\{)(.*?)(\}2\})/',
				'<ul><li><a href="#sommaire_'.$j.'">$2</a></li></ul>',
				$texte_sommaire[0][$i])
		);
	$j++;
		}
	$sommaire_complet.= "</ul></div>";
