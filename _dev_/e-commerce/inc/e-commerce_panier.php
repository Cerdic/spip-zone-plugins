...
function afficher_paniers($titre_table, $requete, $icone = '') 
	...
	$from = $requete['FROM'] ? $requete['FROM'] : 'spip_paniers AS panier';
	...
	if ($cpt) 
		...
		while ($row_paniers = spip_fetch_array($result_paniers)) 
			...
			$id_boutique = $row_paniers['id_boutique'];
			$tous_id[] = $id_panier;
			$somme_chapo=0;
			$somme_quantite=0;

			$result_chapo=spip_query("
				SELECT spip_articles.chapo, spip_paniers.quantite
				FROM spip_articles, spip_paniers 
				WHERE spip_articles.id_article=spip_paniers.id_article 
				AND spip_paniers.id_boutique=".$id_boutique." LIMIT 0, 30");
