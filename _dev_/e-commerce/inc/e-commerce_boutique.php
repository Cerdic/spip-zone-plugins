...
// DEBUGGING MODE
//
//	echo "<p><strong>"._L("affiche boutique [id_boutique -> $id_boutique]")."</strong> ";
//	exit;
//
// FIN
//	
...
function boutique_nom_cookie_boutique($id_boutique) 
	{
	return $GLOBALS['cookie_prefix'].'cookie_boutique_'.$id_boutique;
	}

function boutique_verif_cookie_sondage_utilise($id_boutique) 
	{
	//var_dump($_COOKIE);
	$cookie_utilise=true;
	$nom_cookie = boutique_nom_cookie_boutique($id_boutique);
	...
	if (!$cookie = addslashes($GLOBALS['cookie_boutique']))
		...
		$query = "SELECT id_reponse FROM spip_reponses ".
			"WHERE id_boutique=$id_boutique AND cookie='".addslashes($cookie)."'";
