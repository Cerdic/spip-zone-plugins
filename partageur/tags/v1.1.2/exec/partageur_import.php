...
function exec_partageur_import() {
	...
	$id_partageur = intval(_request("id_partageur"));
...
function partageur_syndiquer($id_partageur,$id_article,$id_rubrique=0,$cle="") {
	...
	//-------------------------------
	// Recupere la config ou charger les valeurs par defaut
	//-------------------------------

	if (lire_config('partageur/import_statut')=="publie") $import_statut = "publie";  else $import_statut = "prop";
	if (lire_config('partageur/import_mot_article'))  $import_mot_article=true; else  $import_mot_article=false;
	if (lire_config('partageur/import_mot_evnt'))  $import_mot_evt=true; else  $import_mot_evt=false;
	if (lire_config('partageur/import_mot_groupe_creer')=="oui")  $import_mot_groupe_creer=true; else  $import_mot_groupe_creer=false;
	if (lire_config('partageur/import_mot_groupe')) {
		$id_import_mot_groupe = intval(lire_config('partageur/import_mot_groupe'));
	} else {
	...
	if ($id_nouvel_article) {
		return $id_nouvel_article;
	} else {
		return $log_html;
	}
}
