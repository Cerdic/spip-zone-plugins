...
function vu_afficher_titre_publication($type,$row){
	$titre = $row['titre'];
	return array(typo(supprime_img($titre,'')),'');
}

// affichage de la puce
function vu_puce_statut_publication($id, $statut, $id_rubrique, $type, $ajax='') {
...
/*	if ($ajax){
	...
	if ((! _SPIP_AJAX) AND $type != 'publication') 	
		// Si Ajax n'est pas active ET que l'objet n'est pas une publication
		$over ='';
	else {
...
function afficher_publications_boucle($row, $own)
{
	...
	$primary = "id_publication";		// On force $primary. Semble poser probleme sinon : elle n'est pas definie.
	...
	if (autoriser('voir',$type,$id_objet)){
		...
		$vals[] = vu_puce_statut_publication($id_objet, $statut, $id_rubrique, $type);

		// Affichage du titre (fonction definie au-dessus)
		list($titre,$suite) = vu_afficher_titre_publication($type,$row);
		$flogo = '';
