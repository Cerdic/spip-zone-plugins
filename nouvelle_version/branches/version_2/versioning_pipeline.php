...
function versioning_boite_infos($flux)
{
    ...
    $row = sql_fetsel(
        ...
        "version_of="
        . $flux['args']['id']
        . " AND statut!='archi' AND  statut!='poubelle'"
    );
    ...
    if ($idversion = intval($row['id_article'])) {
        $flux['data'] .= icone_horizontale(
            _T('versioning:se_rendre_sur_la_version'),
            generer_url_ecrire('articles', "id_article=$idversion"),
            "",
            _DIR_PLUGIN_VERSIONING . "images/voir_new_version-24.png",
            false
        );
    } else {
    ///// Sinon si il n'existe pas d'autre version on propose de la créer uniquement si l'article est publié
        if (($id = intval($flux['args']['id'])) && ($type=='article')) {
            if (!($idorig = $flux['args']['row']['version_of']) && ($statut=='publie')) {
            ...
            } elseif (($idorig = $flux['args']['row']['version_of']) && ($statut!='poubelle') && ($statut!='archi')) {
                ...
                if (autoriser('modifier', 'article', $flux['args']['row']['version_of'])) {
                    ...
                    $flux['data'] .= icone_horizontale(
                        ...
                        generer_url_ecrire('articles', "id_article=$idorig"),
                        "",
                        _DIR_PLUGIN_VERSIONING . "images/voir_old_version-24.png",
                        false
                    );
                    $flux['data'] .= "<script> $('.publie').hide(); </script>";
                }
            }
        }
    }
