...
class menuItem {
	...
	//
	// Contructeur :
	function menuItem($titre, $url, $nom, $icone="", $condition="true",$nom_sousmenu="",$url_sansjs="") {
	  $this->titre = $titre;
	  $this->url = $url;
	  $this->nom = $nom;
	  $this->icone = $icone;
	  $this->condition = $condition;
	  $this->nom_sousmenu = $nom_sousmenu;
	  $this->url_sansjs = $url_sansjs;
	}

	function onglet($onglet_ref){
		$test = true;
		if ($this->condition!="")
		  if (!preg_match("{return}",$this->condition))
			  $test = eval("return (".$this->condition.");");
			else
			  $test = eval($this->condition.";");
	  if ($test)
			onglet($this->titre, $this->url, $onglet_ref, $this->nom, $this->icone);
	}
	function menu_niveau1($rubrique,$sous_rubrique){
		$test = true;
		if ($this->condition!="")
		  if (!preg_match("{return}",$this->condition))
			  $test = eval("return (".$this->condition.");");
			  ...
			  $test = eval($this->condition.";");
	  if ($test)
			icone_bandeau_principal ($this->titre,$this->url,$this->icone,$this->nom, $rubrique,$this->url_sansjs,$this->nom_sousmenu, $sous_rubrique);
	}
	function menu_niveau2($sous_rubrique){
		$test = true;
		if ($this->condition!="")
		  if (!preg_match("{return}",$this->condition))
			  $test = eval("return (".$this->condition.");");
			  ...
			  $test = eval($this->condition.";");
	  if ($test)
			icone_bandeau_secondaire ($this->titre,$this->url,$this->icone,$this->nom, $sous_rubrique);
	}
}
