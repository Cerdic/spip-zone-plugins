...
function action_taxonomie_get_wikipedia_dist() {
	...
	if ($arguments) {
		...
		if (intval($id_taxon)) {
			...
			$recherche = array('name' => $nom_scientifique, 'tsn' => $taxon['tsn']);
			$options = array('section' => $section);
			$information = wikipedia_get_page($recherche, $langue, $options);
			if (!empty($information['text'])) {
				...
				$texte_converti = '<multi>'
								  . '[' . $spip_langue . ']'
								  . ($convertir ? $convertir($information['text'], 'MediaWiki_SPIP') : $information['text'])
								  . '</multi>';
