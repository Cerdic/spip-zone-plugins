...
function eval_upgrade($nom_meta_base_version,$version_cible){
	...
	if (   (!isset($GLOBALS['meta'][$nom_meta_base_version]) )
		...
		if ($current_version==0.0){
			...
			// pour la gestion des évolutions, prendre modèle sur le fichier "notation_upgrade.php" 
			
			// ajout des champs commentaire et id_mot à la table spip_notations
			...
			// ajout du champ id_mot à la table spip_notations_objets
			sql_alter("TABLE spip_notations_objets ADD COLUMN id_mot BIGINT(21) NOT NULL DEFAULT '0' AFTER id_objet");
			// ajout de l'index de clé primaire multiple avec objet et id_objet
...
function eval_vider_tables($nom_meta_base_version) {
	...
	// suppression de la clé d'index sur spip_notations_objets
