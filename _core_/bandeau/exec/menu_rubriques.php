...
function bandeau_rubrique($id_rubrique, $titre_rubrique, $zdecal, $largeur, $image='') {
	...
	. "'\nclass='bando_rub$image' style='width: "
	. $largeur
	. "px;'>\n&nbsp;"
...
// http://doc.spip.org/@bandeauHover
	function bandeauHover(r) {
		if (!$(r).is('.hovered'))
			...
			.end()
			.hover(
				function(){\$(this).children('.bando_rub').show();},
				function(){\$(this).children('.bando_rub').hide();}
			);
		};");
	...
	. '<div class="brt">'
	. $nav
	. "</div>\n<div class='bando_rub' style='bottom: 14px; $spip_lang_left: "
	. $pxdecal
	. "; z-index: "
	. ($zdecal+1)
