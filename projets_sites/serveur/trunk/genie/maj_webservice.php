...
function genie_maj_webservice_dist ($t)
{
    ...
    $webservices = sql_allfetsel('webservice,id_projets_site,fo_login,fo_password', 'spip_projets_sites', "webservice!=''");

    if (count($webservices) > 0) {
        foreach ($webservices as $key_webservice => $value_webservice) {
            $champs = $analyser_webservice($value_webservice['webservice'], $value_webservice['fo_login'], $value_webservice['fo_password']);
//            var_dump($champs);
