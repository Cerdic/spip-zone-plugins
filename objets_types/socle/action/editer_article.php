...
function insert_article($id_rubrique) {
	...
	if (!$id_rubrique = intval($id_rubrique)) {
		$row = sql_fetsel("id_rubrique, id_secteur, lang, "._TYPE, "spip_rubriques", "id_parent=0",'', '0+titre,titre', "1");
		$id_rubrique = $row['id_rubrique'];
	} else $row = sql_fetsel("lang, id_secteur, "._TYPE, "spip_rubriques", "id_rubrique=$id_rubrique");
	...
	$type = $row[_TYPE];
	if(isset($GLOBALS['types']['article'][$type]))
		$type = $GLOBALS['types']['article'][$type];
