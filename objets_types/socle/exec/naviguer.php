...
function exec_naviguer_args($id_rubrique, $cherche_mot, $select_groupe)
{
	if (!$id_rubrique) {
		...
		//selection du champ type
		$row = sql_fetsel('id_parent, id_secteur, titre, statut, extra,  lang, descriptif, texte, '._TYPE, 'spip_rubriques', "id_rubrique=$id_rubrique");
	...
	if ($id_rubrique > 0) {
		...
		$typer_rubrique = $typer($id_rubrique, $flag_editable, $type, 'rubrique', 'naviguer', $id_parent);
	} else $typer_rubrique = '';
	...
	echo naviguer_droite($row, $id_rubrique, $id_parent, $id_secteur, $haut, $n_forums, $editer_mot, $typer_rubrique, $flag_editable, $boucles, $extra),
	  fin_gauche(),
	  fin_page();
	}
}

// https://code.spip.net/@naviguer_droite + typer_rubrique
function naviguer_droite($row, $id_rubrique, $id_parent, $id_secteur, $haut, $n_forums, $editer_mot, $typer_rubrique, $flag_editable, $boucles, $extra)
{
...
// https://code.spip.net/@afficher_rubrique_rubrique
