...
function fabriquer_liste_types() {
	global $types;
	static $liste = '';
	static $correspondance = array();
	if($liste == '') {
		foreach($types as $objet => $type)
			if(is_array($type))
				foreach($type as $_type) {
					$liste .= '|'.$_type;
					$correspondance[$_type] = ($objet == 'secteur') ? 'rubrique' : $objet;
				}
			else {
				$liste .= '|'.$type;
				$correspondance[$type] = ($objet == 'secteur') ? 'rubrique' : $objet;
			}
	}
	return array($liste, $correspondance);
}

function recuperer_type($objet, $id) {
	include_spip('base/abstract_sql');
	$id_objet = id_table_objet($objet);
	$table_objet = table_objet_sql($objet);
	$type = sql_getfetsel(_TYPE, $table_objet, $id_objet.'='.intval($id));
...
function urls_type_dist(&$fond, $url) {
	...
	list($liste, $correspondance) = fabriquer_liste_types();

	// Ce bloc gere les urls page et la compatibilite avec les "urls standard"
	if ($fond=='sommaire'){
		if (preg_match(
		',^[^?]*[?/](article|rubrique|breve|mot|site|auteur'.$liste.')(?:\.php3?)?.*?([0-9]+),',
		$url, $regs)) {
			$fond = $regs[1];
			$id_objet = 'id_'.$correspondance[$fond];
			if ($regs[1] == 'site') {
				if (!isset($contexte['id_syndic']))
					$contexte['id_syndic'] = $regs[2];
			} else {
				if (!isset($contexte[$id_objet]))
					$contexte[$id_objet] = $regs[2];
			}
		...
		/* Compatibilite urls-page avec formulaire en get !!! */
			...
			',[?/&](article|breve|rubrique|mot|auteur|site'.$liste.')[=]?([0-9]+),',
			$url, $regs)) {
			$fond = $regs[1];
			if ($regs[1] == 'site') {
				if (!isset($contexte['id_syndic']))
