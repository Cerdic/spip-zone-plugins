<?php
/**
 * URLs arborescentes multidomaines
 *
 * Identique aux URLs arbo de base, mais supprime le segment correspondant au secteur des domaines
 *
 * Les différences avec urls/arbo.php sont précédées de commentaires [Multidomaines]
 *
 */
...
define('URLS_ARBO_MULTIDOMAINES_EXEMPLE', '/article/titre');
# specifier le form de config utilise pour ces urls
define('URLS_ARBO_MULTIDOMAINES_CONFIG', 'arbo_mutidomaines');
 ...
 * @return string
 */
function url_arbo_multidomaines_parent($type) {
	static $parents = null;
	if (is_null($parents)) {
		$parents = array(
			'article' => array('id_rubrique', 'rubrique'),
 ...
 * @return string
 */
function url_arbo_multidomaines_terminaison($type) {
 ...
 * @return array|string
 */
function url_arbo_multidomaines_type($type) {
 ...
 * @return array
 */
function urls_arbo_multidomaines_creer_chaine_url($x) {
 ...
 * https://code.spip.net/@declarer_url_arbo_multidomaines_rec
 *
 * @param string $url
 * @param string $type
 * @param string $parent
 ...
 * @return string
 */
function declarer_url_arbo_multidomaines_rec($url, $type, $parent, $type_parent, $contexte = array()) {
	if (is_null($parent)) {
		return $url;
	}
 ...
 */
function renseigner_url_arbo_multidomaines($type, $id_objet, $contexte = array()) {
	$urls = array();
	...
	// parent
	$champ_parent = url_arbo_multidomaines_parent($type);
 ...
 *
 * https://code.spip.net/@declarer_url_arbo_multidomaines
 *
 * @param string $type
 * @param int $id_objet
 * @param array $contexte
 ...
 * @return string
 */
function declarer_url_arbo_multidomaines($type, $id_objet, $contexte = array()) {
	static $urls = array();
	...
	if (!isset($urls[$type][$id_objet][$hash]) or $modifier_url) {
		$r = renseigner_url_arbo_multidomaines($type, $id_objet, $contexte);
		// Quand $type ne reference pas une table
		if ($r === false) {
			return false;
		}
	...
	return declarer_url_arbo_multidomaines_rec($u['url'], $type, $u['parent'], $u['type_parent'], $contexte);
}
 ...
 * @return string
 */
function _generer_url_arbo_multidomaines($type, $id, $args = '', $ancre = '') {
	...
	$propre = declarer_url_arbo_multidomaines($type, $id, $c);
	...
	if ($champ_parent
	  ...
	  and isset($contexte[$champ_parent]) and $contexte[$champ_parent]) {
		...
		$propre_contexte = declarer_url_arbo_multidomaines($type, $id, $c);
	...
	if ($propre) {
		$url = _debut_urls_arbo
			. $debut_langue
			. rtrim($propre, '/')
			. url_arbo_multidomaines_terminaison($type);
	} else {
...
function urls_arbo_multidomaines_dist($i, $entite, $args = '', $ancre = '') {
	...
	$t = array_diff(array_unique(array_merge(array('.html', '.htm', '/'), url_arbo_multidomaines_terminaison(''))), array(''));
	if (count($t)) {
	...
	if (strlen($url_propre) and !preg_match(',^[^/]*[.]php,', $url_propre)) {
		...
		if (is_array($cfg_multi = lire_config('multidomaines', array()))) {
			foreach($cfg_multi as $id_rubrique_multi => $v) {
				...
				if (
					...
					and $url_domaine = sql_getfetsel(
						...
						'spip_urls',
						array(
							'type=' . sql_quote('rubrique'),
							'id_objet=' . intval($id_rubrique_multi),
						),
						'', // groupby
						'date DESC',
						'0,1'
			...
			$cfg_multi = array(); // éviter des notices après
		}
		...
		while (count($url_arbo) > 0) {
			...
			if ($row) {
				...
				$type_parent = '';
				if ($p = url_arbo_multidomaines_parent($type)) {
					$type_parent = end($p);
				}
		...
		if (count($url_arbo_new)) {
			...
			if (strncmp($caller, 'urls_', 5) == 0 and $caller !== 'urls_decoder_url') {
				...
				foreach ($url_arbo_new as $k => $o) {
					...
					if ($s = declarer_url_arbo_multidomaines($o['objet'], $o['id_objet'], $c)) {
						$url_arbo_new[$k] = $s;
					} else {
				...
				if ($url_arbo_new !== $url_propre_originale) {
					$url_redirect = _debut_urls_arbo
						. $url_arbo_new
						. url_arbo_multidomaines_terminaison($entite)
						. ($args?"?$args":'');

					// [Multidomaines] Pour mettre la redirection en absolu, ne pas utiliser url_absolue().
					// Il faut tenir compte du domaine dans lequel se trouve l'objet.
					// (Ne fonctionne que si l'objet a un champ id_secteur. Sinon on pourrait remonter récursivement, mais bon...).
					$cle_objet_redirect = id_table_objet($entite);
					if (isset($contexte[$cle_objet_redirect])) {
						// Retrouver le secteur
						$table_redirect = table_objet_sql($entite);
						$id_secteur_redirect = @sql_getfetsel(
							'id_secteur',
							$table_redirect,
							$cle_objet_redirect . '=' . $contexte[$cle_objet_redirect]
						);
						// Soit c'est dans un domaine
						if (in_array($id_secteur_redirect, $cfg_multi)) {
							$url_redirect = rtrim($cfg_multi[$id_secteur_redirect]['url'], '/') . '/' . $url_redirect;
						// Soit dans le site de base
						} else {
							$url_redirect = rtrim($GLOBALS['meta']['adresse_site'], '/') . '/' . $url_redirect;
						}
					} else {
						// en absolue, car assembler ne gere pas ce cas particulier
						include_spip('inc/filtres_mini');
		...
		if (($entite == '' or $entite == 'type_urls')
			...
			return $urls_anciennes($url_propre_originale, $entite, $contexte);
		}
	}
