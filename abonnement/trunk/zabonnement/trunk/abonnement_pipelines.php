...
function abonnement_post_insertion($flux){
	...
	$id_auteur=$flux['args']['id_auteur'];
		...
		foreach($commande_abos as $abo){
			$id_abo = $abo['id_objet'];
			spip_log('abonnement_post_insertion > id_abonnement='.$id_abo.'et auteur= '.$id_auteur,'abonnement');
			
			$abonnement = sql_fetsel('*', 'spip_abonnements', 'id_abonnement = '. $id_abo);
			...
			if ((!$id = sql_getfetsel('id_auteur','spip_contacts_abonnements','objet="'.$objet.'" and id_auteur='.$id_auteur.' and id_objet='.sql_quote($id_abo).' and validite > NOW()')))
...
function abonnement_post_edition($flux){
	...
	if ($flux['args']['table']=='spip_auteurs') {
		spip_log('abonnement_post_edition table=spip_auteurs','abonnement');
		...
		if ($abonnements && is_array($abonnements)) {
			foreach($abonnements as $key => $id_abonnement)	{
			  if($echeances[$key])	
				if (($id = sql_getfetsel('id_auteur','spip_contacts_abonnements','id_auteur='.$id_auteur.' and id_objet='.sql_quote($id_abonnement).'and objet=abonnement and validite > NOW()'))){
				
				sql_updateq("spip_contacts_abonnements",
					array('validite'=>$echeances[$key]),
					'id_auteur='.$id_auteur.' and id_objet='.sql_quote($id_abonnement).'and objet=abonnement and validite > NOW()');
