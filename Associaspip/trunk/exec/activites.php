...
function exec_activites() {
	sinon_interdire_acces(autoriser('voir_activites', 'association') AND (test_plugin_actif('AGENDA') OR test_plugin_actif('SIMPLECAL')) );
...
/// AFFICHAGES_LATERAUX : TOTAUX : nombre d'activites de la periode en cours selon iscriptions
...
/// AFFICHAGES_CENTRAUX : TABLEAU
	...
	if ($id_mot) {
		...
		if ( $desc_table('spip_mots_evenements') ) // SPIP2
			$q_from .= 'INNER JOIN spip_mots_evenements AS k ON  k.id_evenement=e.id_evenement';
		elseif ( $desc_table('spip_mots_liens') ) // SPIP3
			$q_from .= "INNER JOIN spip_mots_liens AS k ON (k.id_objet=e.id_evenement AND k.objet='evenement')";
		else // ??
			$q_from .= ' INNER JOIN spip_mots AS k ON k.id_mot=e.id_mot ';
		$q_from .= ' LEFT JOIN spip_mots AS m ON k.id_mot=m.id_mot';
	...
	echo association_bloc_listehtml2('evenements',
		sql_select("e.id_evenement, e.date_debut, e.date_fin, e.titre  AS intitule, e.lieu,  COUNT(a.id_activite) AS inscriptions, SUM(a.quantite) AS quantites, SUM(a.prix_unitaire) AS montants, CASE COUNT(a.id_activite) WHEN 0 THEN 0 ELSE 1 END AS participations $mc_sel", $q_from, $q_where, 'e.id_evenement', 'date_debut DESC, date_fin DESC', $limit, $q_having), // requete
		array(
