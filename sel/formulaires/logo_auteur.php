 ...
 */
function formulaires_logo_auteur_charger_dist($objet, $id_auteur, $retour='', $options=array()){
	$objet = 'auteur';
	...
	if (!isset ($options['titre'])) {
		...
		$libelle = (($id_auteur OR $objet != 'rubrique') ? $objet : 'racine');
	...
	if (!isset ($options['editable']))
		...
		$options['editable'] = autoriser('iconifier',$objet,$id_auteur);
	$res = array(
	...
	foreach($etats as $etat) {
		$logo = $chercher_logo($id_auteur, $_id_objet, $etat);
		if ($logo){
 ...
 */
function formulaires_editer_logo_identifier_dist($objet, $id_auteur, $retour='', $options=array()){
	return serialize(array($objet, $id_auteur));
}
 ...
 */
function formulaires_logo_auteur_verifier_dist($objet, $id_auteur, $retour=''){
	$erreurs = array();
 ...
 * @return Array
 */
function formulaires_logo_auteur_traiter_dist($objet, $id_auteur, $retour=''){
	$res = array('editable'=>' ');
	...
	if ($on OR _request('supprimer_logo_off')){
		$logo = $chercher_logo($id_auteur, $_id_objet, $on ? 'on' : 'off');
		if ($logo)
			spip_unlink($logo[0]);
	...
	// sinon supprimer ancien logo puis copier le nouveau
		...
		foreach($sources as $etat=>$file) {
			if ($file and $file['error']==0) {
				...
				if ($logo)
					spip_unlink($logo[0]);
				$ajouter_image($type.$etat.$id_auteur," ",$file);
				$res['message_ok'] = ''; // pas besoin de message : la validation est visuelle
			}
		}
	}
