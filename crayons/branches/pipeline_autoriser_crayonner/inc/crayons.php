...
// Possibilité pour un plugin de se branche dessus après coup via un pipeline
// pour modifier l'autorisation par défaut
// Exemple d'usage : champ extra qui restreint par type de champ
function autoriser_crayonner_dist($faire, $type, $id, $qui, $opt) {
	// Pour le futur passage au pipeline
	$args = array(
		'faire' => $faire,
		'type' => $type,
		'id' => $id,
	...
	if (!_CRAYONS_TABLES_EXTERNES) {
		...
		if (!isset($GLOBALS['tables_principales']['spip_'.table_objet($type)])
			...
			return
				pipeline('crayons_autoriser_crayonner', array(
					'data' => false,
					'args' => $args,
				)
			);
	...
	return pipeline('crayons_autoriser_crayonner', array(
		'data' => autoriser('modifier', $type, $id, $qui, $opt),
		'args' => $args,
	));
}
