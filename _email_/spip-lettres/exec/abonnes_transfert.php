<?php
	...
	function exec_abonnes_transfert() {
		...
		list($titre) = spip_fetch_array(spip_query('SELECT titre FROM spip_lettres WHERE id_lettre="'.$id_lettre.'"'));
		...
		switch ($etape) {
			...
			case 2:
				foreach ($tableau_lettres as $tableau_id_lettre) {
					list($titre) = spip_fetch_array(spip_query('SELECT titre FROM spip_lettres WHERE id_lettre="'.$tableau_id_lettre.'" LIMIT 1'));
