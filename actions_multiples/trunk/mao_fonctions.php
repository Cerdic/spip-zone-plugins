...
function mao_actionner($texte, $fond, $objet, $ids) {
	// N'insérer les modifications qu'une seule fois
	if (strpos($texte, '<tr class="action_row">') === false) {
		...
		if (preg_match_all(_MAO_PATTERN_TR, $tbody, $balises_tr)) {
			foreach($balises_tr[2] as $_balise) {
				if (preg_match($regexp_id, $_balise, $td_id)){
					...
					$contexte = array('nom' => "ids_$objet", 'valeur' => $id, 'active' => in_array($id, $ids));
