...
function formulaire_metas() {
	...
	if (!_request('GestionMetasSubmit')) {
		...
		$query .= " SET
					...
					titre = ".sql_quote($metas['titre']).",
					description = ".sql_quote($metas['description']).",
					keywords = ".sql_quote($metas['keywords']);
		if ($metas['id_meta'] != '') $query .= " WHERE id_meta = ".$metas['id_meta'];
		//echo $query;
		$res = spip_query($query);
		
		// on met a jour l'url
		$query = "UPDATE spip_urls SET url = " . sql_quote($metas['url']) . " WHERE type = '" . $ElementGestionMetas . "' AND id_objet = " . $IdElementGestionMetas;
		$res = spip_query($query);
	}
	
	// on recherche l'url pour l'element :
	$query = "SELECT url FROM spip_urls WHERE type = '" . $ElementGestionMetas . "' AND id_objet = " . $IdElementGestionMetas;
	$res = spip_query($query);
	$result = spip_fetch_array($res);
	$metas['url'] = $result['url'];
	...
	<form id="metas" method="post">
		<fieldset>
			...
			<p><label for="GestionMetas_url">URL</label><br />
			<input id="GestionMetas_url" type="text" name="GestionMetasUrl" value="'.htmlspecialchars($metas['url'], ENT_QUOTES).'" style="width: 98%"/></p>
