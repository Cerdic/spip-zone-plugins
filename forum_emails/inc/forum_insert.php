...
// https://code.spip.net/@prevenir_auteurs
...
prevenir_auteurs_forums($id_article, $id_forum, $sujet, $corps, $parauteur);// ajouté par francois.vachon@aigo.ca 
...
// https://code.spip.net/@forum_insert_secure
...
//***************** ajouté par francois.vachon@aigo.ca**************************
// Fonction permettant prévenir par courriel tous les participants à d'un tread de forum 
// Les auteurs de l'article étant déjà traité par la fonction  prevenir_auteurs(), ne pas traiter ici ces derniers.

function prevenir_auteurs_forums($id_article, $id_forum, $sujet, $corps, $parauteur){
	include_ecrire('inc_mail.php3');
	$id_parent = $id_forum;
	$mails = array();

// Début de la modification du corps du message pour supprimer la mention "a la suite de votre article."
$forum_poste_par_avant = _T('forum_poste_par', array('parauteur' => $parauteur));
$forum_poste_par_apres = split( $parauteur,$forum_poste_par_avant);
$forum_poste_par_apres = $forum_poste_par_apres[0].$parauteur;
$corps = ereg_replace($forum_poste_par_avant,$forum_poste_par_apres,$corps);
// fin de la modification du corps***********************************************************************
   ...
   while($r = spip_fetch_array(spip_query("select email_auteur, id_parent from spip_forum where id_forum=$id_parent and statut='publie'"))) {
		...
		$id_parent=$r['id_parent'];
		 ...
		 if ($email_auteur!='' && !in_array($email_auteur, $mails)) {
			if ($passe){// Ne pas envoyer de courriel à l'auteur du nouveau message
