...
function inc_forum_insert_dist() {
	...
	if ($afficher_texte <> 'non') {
		...
		$antispam=unserialize($GLOBALS['meta']["antispam"]);
		if (count($antispam)) {
			$separateur=trim($antispam['separateur'])?trim($antispam['separateur']):';';
			foreach($antispam as $key => $value){
				if ($key!='separateur'){
					...
					foreach ($val as $v){
						$c=0;
						$v=trim($v);
						$va=explode($separateur,$v);
						if (count($va)) {$v=trim($va[0]);$c=intval($va[1]);}
						if ($v && substr_count(strtolower(_request($key)),$v)>$c){
							tracer_erreur_forum('antispam : plus de '.$c.' occurrences ('.substr_count(strtolower($test[$key]),$v).') de '.$v.' dans '.$key);
							return $retour_forum;
						}
					}
				}
	...
	if (strlen(_request(_request('hash')))) {
