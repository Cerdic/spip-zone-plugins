...
function balise_UPLOADIFY ($p) {
	$p = calculer_balise_dynamique($p,'UPLOADIFY', array());
	return $p;
}

// stat
function balise_UPLOADIFY_stat($args, $filtres) {
		return $args;
}

// dyn
function balise_UPLOADIFY_dyn($str="")
{	  
  
  if (isset($GLOBALS['auteur_session']['statut']) &&
		 ($GLOBALS['auteur_session']['statut']!="0minirezo")) {
      spip_log("uploadify: user inconnu"); 
  } else {
