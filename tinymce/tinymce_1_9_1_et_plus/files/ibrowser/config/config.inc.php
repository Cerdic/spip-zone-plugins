<?php
	...
	//ajoute le "/" final
	if (strlen($_GET['library'])>0 && $_GET['library']{strlen($_GET['library'])-1}!='/')
		$_GET['library'] = $_GET['library'].'/';
	//ajoute un "/" initial
	if ($_GET['library']{0}!='/')
		$_GET['library'] = '/'.$_GET['library'];
	//langue
	$lang_file = 'lang/'.$_GET['lang'].'.php';
	if (!is_file($lang_file))
		$_GET['lang']='en';
	//echo 'LANG:'.$_GET['lang'].':LANG'.$lang_file.' - '.getcwd().' - '.$_SERVER['QUERY_STRING'];
	//exit;
	
	
	//$cfg['lang']   	= 'fr';														// default language; e.g. 'en'		
	$cfg['lang']   		= $_GET['lang'];											// default language; e.g. 'en'		
	...
	$cfg['ilibs'] = array (																// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions	
		array (																		
			'value'   	=> $_GET['library'], // '/images/librairie/'
			'text'    	=> 'Images du site',
		)/*,
		array (
	...
	//$cfg['ilibs_dir'] 	   = array('/images/librairie/'); // Répertoire à modifier par ACTi	suivant le site			   	// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions
	$cfg['ilibs_dir'] 	   = array($_GET['library']); // Répertoire à modifier par ACTi	suivant le site			   	// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions
	...
	//modification ACTi du 28/02/2006 pour que ça marche si le port est différent de 80
	$cfg['base_url'] 	= 'http://' . $_SERVER['SERVER_NAME'].(($_SERVER['SERVER_PORT']!=80)?':'.$_SERVER['SERVER_PORT']:''); 							// base url; e.g. 'http://localhost/'	
