...
function articles_edit($id_article, $id_rubrique, $lier_trad, $id_version, $new, $config_fonc, $row)
{
	...
	$rers_new =  $_GET['new'];
//RERS       Texte par défaut pour aider à la rédaction des fiches de savoirs
if ($rers_new == 'oui')
{
 if ($id_rubrique == $rers_rub_offres)  // rers
 { $rerstextedefaut = array('texte' => _T('rers_texte_defaut_offre')) ;
 } 
 else
 {
   if ($id_rubrique == $rers_rub_demandes)
   { $rerstextedefaut = array('texte' => _T('rers_texte_defaut_demande')) ; 
   }
   else
   { $rerstextedefaut = null  ;
   }
 }
}
	$contexte = array(
