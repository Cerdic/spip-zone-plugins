...
function afficher_sites_boucle($row, &$tous_id, $voir_logo, $bof)
{
  global $spip_lang_right;
	...
	if ($syndication == "oui" OR $syndication == "off" OR $syndication == "sus") {
		...
		$total_art_pmb = spip_fetch_array(spip_query("SELECT COUNT(*) AS n FROM spip_syndic_articles_pmb WHERE id_syndic='$id_syndic'"));
		
		$s = " " . $total_art['n'] + $total_art_pmb['n']. " " . _T('info_syndication_articles');
	} else {
			$s = "&nbsp;";
	}
	$vals[] = $s;
