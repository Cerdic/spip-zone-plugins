...
if (strval($nom_site)!='' AND $modifier_site == 'oui' AND $flag_editable) {
	...
	if ($syndication_old != $syndication AND $syndication == "non")
	{
		...
		spip_query("DELETE FROM spip_syndic_articles_pmb WHERE id_syndic=$id_syndic");
	}
	calculer_rubriques();
...
if ($syndication == "oui" OR $syndication == "off" OR $syndication == "sus") {
	...
	afficher_syndic_articles(_T('titre_articles_syndiques')." PMB", array('FROM' => 'spip_syndic_articles_pmb', 'WHERE' => "id_syndic=$id_syndic", 'ORDER BY' => "date DESC"), $id_syndic);
