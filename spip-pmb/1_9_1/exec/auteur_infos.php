...
//récupérer identifiants pmb - retourne éventuellement un tableau vide
$auteur_pmb = spip_fetch_array(spip_query("SELECT * FROM spip_auteurs_pmb WHERE id_auteur=$id_auteur"));
$auteur['pmb_id'] = $auteur_pmb['pmb_id'];
$auteur['pmb_passwd'] = $auteur_pmb['pmb_passwd'];
...
if (strval($nom)!='') {
	...
	if ($new_login_pmb) {
		$auteur['pmb_id'] = $new_login_pmb;
		if ($new_passwd_pmb)
		{
			$auteur['pmb_passwd'] = $new_passwd_pmb;
		}
	  }
	...
	if (!$echec) {
		...
		$n = spip_query("UPDATE spip_auteurs SET $query_pass		nom=" . spip_abstract_quote($auteur['nom']) . ",						login=" . spip_abstract_quote($auteur['login']) . ",					login=" . spip_abstract_quote($auteur['login']) . ",						email=" . spip_abstract_quote($auteur['email']) . ",	bio=" . spip_abstract_quote($auteur['bio']) . ",				nom_site=" . spip_abstract_quote($auteur['nom_site']) . ",				url_site=" . spip_abstract_quote($auteur['url_site']) . ",				pgp=" . spip_abstract_quote($auteur['pgp']) .					(!$extra ? '' : (", extra = " . spip_abstract_quote($extra) . "")) .			" WHERE id_auteur=".$auteur['id_auteur']);
		if (!$n) die('UPDATE');
		
		$n = spip_query("SELECT * FROM spip_auteurs_pmb WHERE id_auteur=".$auteur['id_auteur']);
		if (!spip_num_rows($n))
		{
			$n = spip_query("INSERT INTO spip_auteurs_pmb (pmb_id, pmb_passwd, id_auteur) VALUES ( " . spip_abstract_quote($auteur['pmb_id']) . " , " . spip_abstract_quote($auteur['pmb_passwd']) . " , " . $auteur['id_auteur']. ")");
			if (!$n) die('INSERT');
		}
		else
		{
			$n = spip_query("UPDATE spip_auteurs_pmb SET pmb_id=" . spip_abstract_quote($auteur['pmb_id']) . ", pmb_passwd=" . spip_abstract_quote($auteur['pmb_passwd']) . " WHERE id_auteur=".$auteur['id_auteur']);
			if (!$n) die('UPDATE');
		}
	}
 }
...
//zone de saisie des identifiants pmb
	echo "<B>"."Identifiant PMB"."</B> ";
	echo "<BR>";
	echo "<INPUT TYPE='text' NAME='new_login_pmb' CLASS='formo' VALUE=\"".entites_html($auteur['pmb_id'])."\" SIZE='40'><P>\n";
	echo "<B>"."Mot de passe PMB"."</B> ";
	echo "<BR>";
	echo "<INPUT TYPE='password' NAME='new_passwd_pmb' CLASS='formo' VALUE=\"".entites_html($auteur['pmb_passwd'])."\" SIZE='40'><P>\n";
