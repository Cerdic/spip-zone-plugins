...
function optimiser_base_disparus($attente = 86400) {
	...
	//APA
	# même traitement avec les articles syndiques appartenant a des sites pmb effaces
	$res = sql_select("syndic_articles_pmb.id_syndic AS id",
		      "spip_syndic_articles_pmb AS syndic_articles
		        LEFT JOIN spip_syndic AS syndic
		          ON syndic_articles_pmb.id_syndic=syndic.id_syndic",
			"syndic.id_syndic IS NULL");

	$n+= optimiser_sansref('spip_syndic_articles_pmb', 'id_syndic', $res);
